...
**Fidelity.** Real content, never placeholder text. Use one consistent demo customer across every prompt: **Acme Robotics**, organization `acme`, workspaces `core-platform` (main repo `acme/platform`, production branch `main`, linked repos `acme/billing`, `acme/mobile`) and `finops` (main repo `acme/finops-agents`). Operators: Priya Natarajan (org owner), Marcus Bell (workspace owner, core-platform), Dana Okafor (billing role, finops). Agents: `acme.core.release-manager` (Claude Code), `acme.core.stella-ci` (Stella), `acme.core.triage` (Codex CLI), `acme.finops.invoice-bot` (OpenAI Agents SDK, holds a mandate), `acme.core.docs-writer` (custom, SDK-wrapped). Fifty agents in total in the fleet counts. Costs shown to the cent with their basis; token counts by class; cache hit rates; enforcement tiers; replay grades; verdicts from the closed vocabulary (`flipped`, `failing`, `unmoved`, `unsatisfied`, `tampered`, `unverified`, `waived`). Every number that is money shows its currency; the organization's display currency is USD.
