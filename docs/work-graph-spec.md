...
| **Pages** | `mockups/pages/tasks.md`, `task.md`, `work-order.md`, each with its audit prompt |
...
Each change lands in an existing page spec. Nothing gains a route.

### 11.1 Tasks (`tasks.md`)

- **Tiles.** Ready for work ("certified, open, and unblocked"), Drafts waiting on you (unchanged), Blocked
  by other tasks ("ready, and waiting on a task upstream", replaces Being drafted), In work orders
  (unchanged). Being drafted stays visible as the `drafting` readiness badge.
- **Tasks panel.** A **List** and **Graph** switch in the panel header. List adds the column **Blocked by**
...
1. **The word.** The wireframes say task. The request that asked for this spec said work item. The closed
   wedge (PR #90) renamed task to work item and was not adopted. The ARP brand spec bans task as a
   user-facing noun (`ARP-Brand-spec.md` line 37) and says work order, work request, or run. This spec
   keeps task and changes no page title. Mac decides whether it changes, and when.
