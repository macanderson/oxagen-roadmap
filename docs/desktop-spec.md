...
| **Canonical copy** | The canonical copy of this spec is `docs/specs/oxagen-desktop/spec.md` in https://github.com/macanderson/oxagen, which is newer: it carries Stella and custom agents in §6, and §13 (rev 2, the connected tier). This copy stops before §13. §14 says the same thing in both. |
...
2. **Enrollment writes the model base URLs into each harness**, Claude Code (`ANTHROPIC_BASE_URL`) and Codex (`OPENAI_BASE_URL`), with displace-and-restore: the value that was there is kept, and unenroll puts it back. Enrollment writes them only after the daemon is confirmed listening on loopback, so a harness is never pointed at a port nothing answers.
...
| Loopback model proxy | Passes Anthropic Messages and OpenAI Responses requests through to the vendor with streaming. Prompt bodies never leave the machine: only digests and usage go up. The vendor credential stays on the machine and Oxagen never holds it | Phase 4, in build on `gateway-model-proxy` |
