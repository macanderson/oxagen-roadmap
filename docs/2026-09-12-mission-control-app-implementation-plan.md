...
| **Inputs** | `2026-09-11-oxagen-mission-control-spec.md` (§3, §4, §14, §15, §19, App. A, B, F) · mockups `~/Documents/Oxagen/Mockups` (`mc.html` @ `main` 1b0634c, W1–W12) · `docs/feedback-mockups.md` · the repo at `origin/main` |
...
import { seed } from "./seed"; // ported from mc.html @ main 1b0634c, mapped to spec vocabulary
...
|---|---|---|---|
| **L1 contracts + ports + fixture** | `src/data/**` | All view-model schemas (§4.5) for the ten pages, every port interface, the fixture seed ported from `mc.html` @ 1b0634c with W3's vocabulary mapping, the integrity test (W4), and the `mc_state` state switch for dev/e2e | B0 |
...
| P3 | **Agents** (section "Agent IAM", list page "Identities" on mockup `main`) | list; detail tabs identity/definition/toolbelt/mandates/budgets/runs/enrollment; source editor + commit dialog; mandate detail (real `[mandate]` param, W4); register entry point | Editor is a client island |
...
- mockup baseline: ~/Documents/Oxagen/Mockups mc.html at commit 1b0634c, plus approvalCardSm
  (branch small-approvals-bottom-dock-scenarios) and the assistant flyout (branch worktree-assistant-sidebar-flyout)
