...
| **M3** Teach |  the records endpoint, the reflector, the promoter, steering prs through the github app, st… | 12 | 1 |
...
| **Steering** | Published records, proposals, open Steering PRs, effect metrics, and retirement c… | 11 |
...
| `list_records` | INHERIT | `steering.record.list.ts`<br>`agent.memory.list.ts`<br>`agent.memory_citation.list.ts`<br>`agent.memory_citation.stats.ts` | Steering | `pSteering`:5499 | none | M3 |
...
| `propose_record` | INHERIT | `agent.memory.promote.ts`<br>`steering.record.promote.ts`<br>`agent.memory_promotion.rationales.ts` | Steering | `pSteering`:5499 | none | M3 |
| `open_steering_pr` | INHERIT | `steering.record.publish.ts` | Steering | `pSteering`:5499 | `wrk.repositories` `wrk.workspaces` `audit.audit_events` | M3 |
...
| `register_agent` | milestone | M0 | M3 | §6.2 is unconditional: "Creating or changing an agent in Mission Control (register_agent, update_agent) does not write Postgres first. It opens a Steering PR on the main repo… Merge creates or updates the principal, the roles the definition asks for, and the toolbelt." §17 dates Steering-PR delivery to M3 ("Steering PRs through the GitHub App"); M0 delivers only "IAM" and M0/M1 have the App installation for repo binding, not PR authoring. The tool as specified cannot execute before M3. The row's own note names this dependency and still assigns M0. |
| `update_agent` | milestone | M0 | M3 | Same §6.2 mechanism ("a Steering PR changing the definition; identity and belt update on merge", Appendix E) and the same §17 M3 dependency on Steering PRs through the GitHub App. |
| `retire_agent` | milestone | M0 | M3 | Appendix E: "a Steering PR removing the file; principal retired, never deleted". §6.2: "Deleting an agent is a PR that removes the file." Same M3 dependency as its two siblings. |
