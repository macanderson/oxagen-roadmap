...
Ported on 2026-09-14. `mc.html` now has the **Skills** page (`pSkills()`: Catalog · Search · In the
...
the six beats above as seven rail steps (beat 4 is two: Fleet, then the run). `w13-in-the-loop.html`
is generated by `tools/build-w.mjs`; `pages/skills*`, `pages/run-interjection*` and their specs and
audit prompts by `tools/build-pages.mjs` and by hand. The `screens` index of the hand-drawn original
is replaced by the scenario rail and the state bar. Every W13 class in `mc.html` is `sx-` prefixed.
