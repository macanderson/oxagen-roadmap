...
> **Status of this section (2026-09-18).** This section carries the storage rule approved on 2026-09-18 (the steering, graph and gateway review of 2026-09-18, linked in this document's header; ADR "One assembler decides what reaches the agent, and records what it cut"). The rule is in force now. Two parts of it are targets: the steering index with its `steering_drift` check arrives with the assembler in Phase 1, and the graph projection of steering items arrives in Phase 3 (§17.2). Lines marked **Today** describe oxagen `main` at `02278c913`.
...
> **Status of this section (2026-09-18).** §7 describes the approved target (ADR "tachod grows into the gateway: a loopback model proxy and an MCP aggregator"; ADR "The tier ladder is four words, computed from what was routed"; ADR "Oxagen may contain the process that runs turns: the contained tier"). Earlier revisions of this section described the model proxy, base URL enrollment, run tokens and proxy budgets as present. None of them exists on oxagen `main` at `02278c913`. The hook adapter and the control channel are built. The tool gateway is built on the server and is registered only into Claude Desktop. The loopback model proxy and the MCP aggregator are Phase 4. The contained tier is Phase 5 (§17.2). Each table below says what is built.
...
|---|---|---|
| **Hook adapter** | Answers the harness's lifecycle hooks: delivers steering as additional context, refuses at the four blocking events, records the rest | Yes (`packages/tacho/src/collector/hook-handler.ts`) |
| **Loopback model proxy** | Passes Anthropic Messages and OpenAI Responses requests through to the vendor, with streaming. Enrollment writes the harness's base URL setting to point at it | No. Phase 4. There is no source hit for `ANTHROPIC_BASE_URL`, `OPENAI_BASE_URL` or `/v1/messages` |
...
| **Hook adapter** (the wrapper). A hook is a script the harness runs at set points in its lifecycle. | Harness lifecycle events: session start, prompt submit, pre-tool, permission request, stop, compaction, config change | Deliver steering as additional context, refuse at the four blocking events (`SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PermissionRequest`), record, detect tampering after the fact | `harness` |
| None of the above | Attested events only | Record and grade | `observe` |

**The tier ladder is four words, computed from what was actually routed: observe → harness → gateway → contained.**

| Tier | What is true of the run | Words the product may use | Built today |
|---|---|---|---|
| `observe` | Recorded only. Nothing was delivered and nothing could refuse. | "recorded" | Yes |
| `harness` | Hooks are installed. Steering is delivered, and the four blocking hook events can refuse. The hook runs inside a process the machine's owner controls, so every control is client-attested and fail-open. | "delivered", "recorded", "client-attested", "fail-open". Never "enforced" | Yes. This is the tier of every wrapped Claude Code, Codex and Stella run |
| `gateway` | Model and MCP traffic was routed through `tachod`. Metering is observed and budgets are enforced on routed traffic. | "observed" metering, "enforced" budgets, always with the scope "for traffic routed through Oxagen" | No. Phase 4 |
| `contained` | The agent runs under an OS sandbox whose only egress is the gateway (§7.2). The agent cannot reach a model or a tool server any other way, which turns every seam from attested to enforced. This is the only tier that earns the word "enforced" against the machine's operator. | "enforced" | No. Phase 5 |

The **enforcement tier is computed per run from what was actually observed**, never from what an adapter could do on paper. It is recorded on the run and shown word for word. The UI, exports, and attestation reports cannot show a stronger word than the tier allows. This is the rule from ADR-040 and the wrapper spec, and it must not change. A control claim always carries its scope: "for actions routed through Oxagen".

**Today.** The wrapped tier is a recorder plus a kill switch. The signed bundle carries empty permissions, `budget.mode = "observed"` and `context.system = null` (`packages/handlers/src/lib/tacho-host.ts:269-276`), so `PreToolUse` can deny only on host status or a paused run. A person can step outside the wrapper by deleting the hook entry, setting `disableAllHooks`, killing the daemon, going offline, adding another MCP server, or running the harness somewhere else. Detection is after the fact, and most failure paths answer the hook with `{}`, which the harness reads as allow. The code's tier enum is three words today (`TACHO_ENFORCEMENT_TIERS`, `packages/database/src/schema/tacho.ts:74`), and its `gateway` word is earned only by a Claude Desktop host connected through the MCP gateway, which routes tools and no model traffic (ADR-078). ADR "The tier ladder is four words, computed from what was routed" settles how those hosts read on the ladder.

### 7.2 Adapters and supported agents

Oxagen is vendor-neutral on the agent side as well as the model side. The target proxy speaks the **Anthropic Messages API** and the **OpenAI Responses API**, as a passthrough with streaming, so a harness that reads a base URL setting can be routed through it. The tool side is plain MCP, which every current harness speaks.
...
**Wrapping is file edits plus a daemon. It is not a supervisor.** Enrollment writes hook entries into the harness's settings file, installs `tachod`, registers it to start at login, and enrolls the host with a device key. No Oxagen command launches the agent. Earlier revisions of this specification call the daemon `oxagend` and the hook binary `oxagen-hook` (§2.1). That rename has not happened in the tree, and this section uses the names in the tree. From Phase 4, enrollment also writes the base URL setting and re-serves the harness's MCP servers through loopback, using the displace-and-restore logic `packages/tacho/src/host/mcp-config-writer.ts` already has. Where a vendor offers managed settings, the managed variant pins them. Unenroll restores every file it displaced. The installer that carries this to a laptop is Oxagen Desktop (the desktop spec).
...
| 5 | **The model request itself** | The volatile selection re-landed per turn, right after the cached system block (ADR-051) | Not available. It exists only once the gateway's proxy exists. Phase 4 |
...
> **Status of this section (2026-09-18).** Commands, their statuses and the hook adapter column are built, and they are what every wrapped run uses today: delivered at the next hook boundary, client-attested and fail-open. The model proxy column, run token revocation and automatic halts on a budget breach describe the `gateway` tier and arrive in Phase 4 (§17.2). In the tables below, "tool gateway" reads as the MCP aggregator for a wrapped harness.
...
|---|---|---|
| Context records | Postgres `agent.context_records` with versions, the promotions ledger and proposals. Mirrored in git as `.oxagen/rules/*.toml` | **No.** No run-time reader exists. ADR-051's injection path was removed by ADR-043 and never re-landed (issue #2592). Only a harness that reads the checkout itself sees them |
...
| `packages/engram` and `packages/context-provider` | A second memory system, and the only real token budgeter (`packWithinBudget`, `packages/context-provider/src/budget.ts:37`) | **No.** Neither has a production importer |
...
| `workspaces.promptConfig.additionalInstructions` | Postgres JSONB | In-app agent only. Appended to every prompt with no check against rules |
...
> **Status of this section (2026-09-18).** Not built. This is the contract Phase 1 implements (§17.2), decided by ADR "One assembler decides what reaches the agent, and records what it cut" and ADR "Steering and gating are two planes, authored on one surface and compiled twice". Phase 0 ships the smallest slice of it: active records with `force` of `must` or `should`, compiled into `context.system`. Its home is `packages/context-provider`, which has no production importer today.
...
| `kind` | `record` \| `skill` \| `memory` \| `ontology` \| `policy` \| `instruction` | `policy` is a gate notice. `instruction` is the workspace's additional instructions |
...
**Precedence, fixed in this one place:**
...
**Failure behaviour.** The hook tier is fail-open, and the assembler is written for that:
...
**The one open risk is a spike, not a decision, and it gates Phase 4 only:** whether Claude Code and Codex subscription logins, as opposed to API keys, pass cleanly through a base URL rewrite, and whether the vendors' terms allow it. It decides how much of the laptop population the proxy can cover. Phases 0 to 3 do not depend on it.
...
| Claude Code and Codex model calls | From Phase 4, enrollment points the harness's base URL at the loopback proxy in `tachod`, which earns the `gateway` tier for model calls. No proxy exists at 2026-09-18 | If a harness cannot point at the proxy, the run gets the `harness` tier and the record says so |
| **Open risk, 2026-09-18: subscription logins through a base URL rewrite** | Unverified. Whether Claude Code and Codex subscription logins, as opposed to API keys, pass cleanly through a base URL rewrite, and whether the vendors' terms allow it. This is a spike, not a decision. It gates Phase 4 only (§17.2) | The answer decides how much of the laptop population the proxy can cover. If subscription logins cannot be proxied, those hosts stay at the `harness` tier and the record says so |
...
The maintainer approved the steering, graph and gateway review of 2026-09-18 in full. Nothing in this table is a proposal or an open question except row 14, which is a spike. (§20, the decisions of 2026-09-15, is in the canonical copy of this specification.)

| # | Decision | Where | ADR |
|---|---|---|---|
| 1 | **One assembler.** `assembleSteering(run, budget)` is the one function where everything competes for Oxagen's slice of the agent's context. It returns a stable prefix, a volatile selection and a manifest, recorded as a `steering.manifest` frame | §10.5 | "One assembler decides what reaches the agent, and records what it cut" |
| 2 | **Storage stays plural, one writer per fact.** Git for what is published. Postgres for what must be transactional or money-grade. The graph for lineage, evidence and entity links. Only the assembler and its index are single. Nothing is collapsed into Neo4j | §4.2 | the same |
| 3 | **Two planes that never merge.** Steering is what the model reads. Gating is what the kernel refuses. One authoring surface, two compilations, and every gate emits a gate notice into steering | §10.5 | "Steering and gating are two planes, authored on one surface and compiled twice" |
| 4 | **One item type, `SteeringItem`**, with `kind` of record, skill, memory, ontology, policy or instruction, and `force` of `must`, `should`, `may` or `info` | §10.5 | "One assembler decides what reaches the agent, and records what it cut" |
| 5 | **Precedence, fixed in one place.** A gate beats everything. A published `must` beats recalled memory. Repository scope may narrow workspace scope and never widen it | §10.5 | the same |
| 6 | **Oxagen does not own the context window. The harness does.** Oxagen's injection points are exactly five | §7.3 | the same |
| 7 | **The index sits behind a port.** First on the Postgres registry. It moves to the graph in Phase 3, with Postgres as the fallback. Delivery never waits for the graph | §4.2, §10.5 | the same |
| 8 | **Skills are steering, and they are files.** Governed through a pull request, delivered by sync, loaded by the harness. The description line competes in the assembler. Skills live under Steering | §10.6 | amendments to ADR-008, and a check of ADR-090 |
| 9 | **One screen: Steering is the hub.** Records, Skills, Memory, Ontology, Policy, Proposals, Preview | §10.7, §14 | none. A page decision |
| 10 | **The gateway.** `tachod` grows into it: hook adapter, loopback model proxy, MCP aggregator, control channel. Prompt bodies never leave the machine. The vendor credential stays on the machine. Metering becomes observed. `session_limit_usd` is enforced. `interrupt` becomes real | §7.1 to §7.3, §12.5, §12.6 | "tachod grows into the gateway: a loopback model proxy and an MCP aggregator", and amendments to ADR-051 and ADR-056 |
| 11 | **The tier ladder is four words, computed from what was actually routed:** observe, harness, gateway, contained | §7.1 | "The tier ladder is four words, computed from what was routed" |
| 12 | **The sandbox is the top tier, not the only tier. Hooks stay.** `oxagen run -- <agent>`, aimed at CI, headless runs, cloud runners and managed devices first. The witness runner is built on the same launcher. ADR-043 is revised by one sentence | §1, §7.2 | "Oxagen may contain the process that runs turns: the contained tier", and amendments to ADR-043 and ADR-064 |
| 13 | **The refactor path is six phases, each ships alone.** Phase 0 carries a freeze on new governance ceremony until it lands | §17.2 | none. The implementation plan |
| 14 | **The one open risk, a spike and not a decision:** whether Claude Code and Codex subscription logins pass cleanly through a base URL rewrite, and whether the vendors' terms allow it. It gates Phase 4 only | §17.2, §18 | none |
