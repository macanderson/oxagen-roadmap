...
`observe` means Oxagen recorded the run and nothing more. It is the floor of the ladder and the default mode of a new bundle (`docs/oxagen/specs/tacho/plan.md:45`).
...
The MCP aggregator that re-serves a wrapped harness's MCP servers through loopback (ADR-094 part 3) is not on `main` (`oxagen` #3299, open). Today a wrapped harness reaches `gateway` through model traffic only, and Claude Desktop through its Oxagen MCP calls only. `mcp-gateway-plan.md` plans the route for Claude Code, Codex, Cursor, and Stella, and lists the MCP surfaces that can never route.
...
7. **The Tacho spec lags the code.** `docs/specs/tacho/spec.md` in `oxagen` defines `gateway` as "every tool routed" (`:278`), has no `contained` in §6.3, still refuses commands to `observe` runs with `observe_tier` (`:391`, reversed by ADR-163), and says `per_day_micros` is not signed (`:405`). The spec's status is still `Proposed`. `oxagen` #3315 tracks correcting it.
