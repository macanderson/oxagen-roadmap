# Scale back Mission Control: the mockup prompt
...
| **Date** | 2026-09-14 |
| **Owner** | Mac Anderson |
| **Related** | `scope-review.md` |

> This prompt drove the wedge approach (PR #25): the cut surfaces stayed in the mockup behind a release switch and an "Already built" group. It was not kept. The review's cuts are removed outright from the mockup, the spec and the plan, and `scope-review.md` records where each landed. The prompt refers to `mc.html` and its generated files, which no longer exist.
...
- **`mc.html` is the only file you edit.** `consolidated*.html`, `pages/**` and
  ...
  get this task wrong.
- **The graph cut is already done** (commits `48f5e8a`, `4052cdd`). There is
  no graph page slug, and `w7` is gone. The review's largest recommendation
...
> - Do not re-add connectors or the graph surfaces.
