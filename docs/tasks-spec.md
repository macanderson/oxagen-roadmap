...
Your work lives in GitHub issues, Linear, or Jira. Your agents are registered in Oxagen. Nothing
joins the two. To put an agent on an issue you copy the issue into a prompt, you guess which
repository it may touch, and nobody writes down what finished means. When the agent stops, you read
the pull request and decide whether it did the job, against a standard that exists only in your head.

This spec joins them. Oxagen reads your tasks from the tracker, drafts a definition of done for each
one, and waits for a person to certify it. A certified task is ready. You pick ready tasks and send
...
| Day 1 | Later |
|---|---|
| GitHub issues, Linear, and Jira Cloud | Jira Server and Data Center, Azure DevOps, ServiceNow, Zendesk, and other case systems |
...
| Term | Meaning |
|---|---|
| **Issue provider** | A tracker Oxagen reads tasks from: GitHub, Linear, or Jira. |
...
| **Task** | One issue, read into Oxagen with the thirteen fields. Its id is `tsk_<ULID>`, minted by Oxagen and stable across reconnects. |
...
1. **Provider.** GitHub, Linear, or Jira, each with its logo. A provider already connected is marked
   `connected` and can be connected again for another account.
2. **Authorize.** The permissions Oxagen asks for, in a table with the reason for each, then **what it
   still cannot do**: edit a subject or a description, delete an issue, act as you, read outside the
   scope you choose next, or assign a task to a person. Next is disabled until the provider returns a
   token.
3. **Scope.** The repositories, teams, or projects to import, and the filter: open issues and those
   closed in the last 30 days (the default), open only, or everything. Jira adds an optional JQL filter.
   The step estimates how many tasks the first read brings in.
4. **Fields.** The workspace's statuses, resolutions, and labels, each with the provider value the
   ...
   and can be changed. Then the four write-back switches (§5.5).
5. **People.** Every account on the tasks in scope, with a suggested workspace member where one matches
   (§7). Each can be confirmed, changed, or left not mapped. A bot is never mapped.
6. **Review.** The whole connection on one screen, then what happens when you connect: Oxagen stores
   the token and reads the tasks, the assistant drafts a definition of done for each open one, every
   draft waits for a person, and nothing is sent to an agent.
...
| Linear | OAuth 2.0 with `actor=app`, so every write is signed by the Oxagen app | `read`, `comments:create` | Long-lived. Linear has no refresh endpoint, so Oxagen re-authorizes on a 401. |
| Jira Cloud | OAuth 2.0 (3LO) on one Atlassian site, resolved through the accessible-resources call | `read:jira-work`, `read:jira-user`, `write:jira-work`, `offline_access` | Rotating refresh tokens |
...
- **Events.** GitHub `issues` and `issue_comment` webhooks through the App, Linear webhooks, and Jira
  dynamic webhooks registered by the OAuth app. An event is a hint: Oxagen reads the issue again and
  applies what it read.
...
- **Identity.** A task is unique by connection and provider id. GitHub keys by record type and
  database id, never by issue number (ADR-121), so a transferred issue stays the same task.
...
|---|---|---|
| Post the definition of done when a person certifies it | on | A comment with the items and a link to the task in Oxagen |
| Post a link when a work order is sent | on | A comment naming the work order and the agent |
| Move the status when a work order starts | off | The provider status mapped to `in review` or `open` |
| Close the task when you accept the work | off | Closed, with the resolution mapped to `Fixed` |

Oxagen never edits a subject or a description, never deletes an issue, and never assigns a person.
...
Day 1 reads thirteen fields and nothing else. The Fields tab shows this table.
...
Two derivations are worth naming. **Updated by** is not a field in any of the three, so Oxagen reads
the actor of the latest change. **Owner** is one person: GitHub allows several assignees, and day 1
reads the first. Custom fields are not read. Each field Oxagen adds needs a mapping on three
providers and a meaning in the brief, so custom fields wait for a schema of their own.
...
|---|---|---|---|
| Fixed | `state_reason completed` | Done | Done, Fixed |
| Won't fix | `state_reason not_planned` | Canceled with label `wontfix` | Won't Do, Won't Fix |
...
provider's labels, priorities, or issue types. Oxagen ships ten.
...
The colour is Oxagen's own. Oxagen never changes a label's colour in a provider. The label editor
...
- **Bots.** A bot account (`dependabot[bot]`, Automation for Jira) is never mapped to a person.
...
| Source | `from the issue`, `oxagen.assistant`, `you`, or `edited by you`. |
...
If the connection allows it, Oxagen posts the list as a comment on the issue. In a workspace whose
governance mode is `regulated`, the person who certifies a task cannot send it.
...
connection allows it, Oxagen posts a link on each issue. The sender lands on the work order page.
...
- **Close.** If the connection's close switch is on, Oxagen closes each task as `Fixed`. If it is off,
  the dialog says the task stays open in the provider.
...
| `update_issue_provider` | api, app, cli | Scope, filter, and write-back switches |
...
| ClickHouse | `task.imported`, `task.changed`, `dod.drafted`, `dod.certified`, `work_order.sent`, `stage.handed_off`, `stage.returned`, `item.claimed`, `work_order.accepted`, `work_order.stopped` |
...
| `issue_provider.connect` | Connect, edit, sync, and disconnect a provider | workspace owner |
| `task_fields.write` | Edit statuses, resolutions, labels, and colours | workspace owner |
...
- [ ] The ten default labels, five resolutions, and three status categories ship as specified.
