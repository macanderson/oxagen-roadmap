...
Billing is readable by a finance role and shows the plan, this period's lines, the meters and the invoices, with a row opening the Stripe-hosted invoice and Change plan going to Stripe checkout. The mockup still prints the per-run model (free runs, per-run tiers, a runs meter). The 2026-09-13 amendment (ADR-055) replaces it with governed action units: a monthly GAU bucket on the subscription, more bought in blocks at the customer's contracted rate, and the contracted rate printed on the page. Tokens are never marked up, and the customer-spend accounting on Spend is unchanged by the amendment. The billing decisions of 2026-09-15 (credit packs for the in-app agent, negotiated enterprise with every feature on for every tier, invoice billing, the Build and Scale upgrade through Stripe Checkout, suspension) are in [spec §12.10](mission-control-spec.md#1210-billing-decisions-of-2026-09-15).
