...
import { z } from "zod";
import { firstHeading, firstParagraph } from "./scripts/lib.mjs";
import { remarkSite } from "./src/lib/remark-site.mjs";

// The repo markdown carries no frontmatter: the title is its first heading and the description
// its first paragraph. Generated MDX may set both in frontmatter, which wins.
const schema = ({ source }: { source: string }) =>
  z
    ...
    .transform((data) => ({
      ...
      description: data.description ?? firstParagraph(source),
    }));
