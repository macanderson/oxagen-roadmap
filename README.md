...
mockups/w13-in-the-loop.html  the one hand-drawn flow not yet in the master
docs/missioncontrol-docs.html the documents page: every docs/*.md, one file
docs/*.md                     the spec, the DoD spec, the desktop spec, the plan, the scope review, the mockup records
...
```sh
npm install
npm run storybook        # every page in every state, desktop and mobile; every scenario; W13
...
| W13 | In the loop (hand-drawn, `mockups/w13-in-the-loop.html`) | Hand-drawn |

W7 (the ontology) and the assistant half of W11 were cut by the scope review of 2026-09-14
(`docs/scope-review.md`); W12 was a coverage audit, now `tools/check-mockup.mjs`. W14 is the
...
```sh
npm run build   # build-mockup, build-stories, build-docs
