...
| **Already built** (a nav group) | Skills — it ships off and needs turning on, so it cannot work on day one — then Audit — events, incidents, receipts, exports, keys; not legal holds, retention or assurance — and the Assistant, demoted from the sidebar footer and the top bar to this group. |
...
W5, W6, W8–W11 and W13 open everything built.
...
per state and theme, generated by `tools/build-badges.mjs` (`--check` guards drift). The words are
