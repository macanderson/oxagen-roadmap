...
You are auditing the **Onboarding · Organization** page of Oxagen Mission Control (`#/welcome/organization`) for conformance to its design. Be exact and adversarial: the design is the spec, and “close enough” is a fail. Do not summarise what you see; compare it.

## Inputs

1. The page spec: `pages/onboarding-organization.md` (read it first, in full).
2. The design, rendered: `pages/onboarding-organization-<state>.html` and `pages/onboarding-organization-<state>-mobile.html` for each state (loaded, loading, error, access denied); open them in a browser or with Playwright. `pages/index.html` links all of them.
...
2. **Header.** Eyebrow “Step 1 of 3”, h1 “Name your organization”. Actions present, in order, with the same labels: Cancel · Continue. Exactly one gold (primary) action on the screen.
...
7. **States.** Force each state and compare copy and controls with the design file:
   - **loading** (`onboarding-organization-loading.html`): the shell stays and the body is the skeleton; no data, no zeros, no stale rows.
   - **error** (`onboarding-organization-error.html`): Inline: “That namespace is taken. `<ns>` belongs to another organization. Pick a different 2–6 character namespace.” Namespace field marked bad.
   - **access denied** (`onboarding-organization-denied.html`): “You cannot see onboarding” — the roles the signed-in person holds on the organization do not include `org.create for <email>`. Copy explains an owner can grant it and that the grant is itself a governed action in the audit record. Actions: Request access (opens the request-access dialog), Back to Fleet. Below: *Signed in as* (name · role), *Needed* (the permission), *Decided by* (`pol_v41` · deny
...
9. **Mobile.** At 390 × 844 with a touch pointer: the card fills the width with 16 px gutters, buttons are full width and ≥ 44 px, inputs are 16 px, code inputs use the numeric keypad. Compare against `pages/onboarding-organization-loaded-mobile.html`.
...
11. **Permissions.** Read requires `authenticated`; each write (org.create; workspace.create) is gated server-side, not only hidden in the UI. Verify with a role that lacks the permission.
