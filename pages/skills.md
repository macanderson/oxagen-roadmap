...
| Design | `mc.html` → `pSkills()` (the single source; `consolidated.html` is the product build of it). The off-by-default gate is `skills-off.md`; the interjected run is `run-interjection.md`. |
| States | loaded · empty · loading · error · access denied |
| Files | `skills-loaded.html` / `skills-loaded-mobile.html`, `skills-empty.html` / `skills-empty-mobile.html`, `skills-loading.html` / `skills-loading-mobile.html`, `skills-error.html` / `skills-error-mobile.html`, `skills-denied.html` / `skills-denied-mobile.html` |
...
**Dialogs this page opens:** `skenable` (turn skills on: the switch rendered as the outcome, the pull request as the control; the config it writes; runs in flight finish without skills), `skcfg` (the config file and its five sources, on/off, with the skill count each contributes), `skill` (version · digest · kind · load cost · decision · owner · cited · proof rate; the SKILL.md body; **Send the digest for approval** on an unapproved marketplace skill), `skadd` (a skill is a file: the path shape, a branch, nothing resolves until it merges).
...
Legend: ✅ backed today · 🟡 partial · ❌ no store (fixture in dev, `NotBacked` in production). The *mockup collection* column names the constant in `mc.html` that the design renders from. Nothing on this page is in Appendix A yet; every target store below is a proposal the spec has to accept or rename.
...
- **empty** — “Skills are on, and there is nothing to resolve” — the config names five sources and none holds a skill; `search_skills` returns an empty result and the reason, not an error. Actions: **Add a skill to a-intel/platform** (→ `skadd`), **Open the config** (→ `skcfg`).
