...
You are auditing the **Two-factor** page of Oxagen Mission Control (`#/welcome/two-factor`) for conformance to its design. Be exact and adversarial: the design is the spec, and “close enough” is a fail. Do not summarise what you see; compare it.

## Inputs

1. The page spec: `pages/two-factor.md` (read it first, in full).
2. The design, rendered: `pages/two-factor-<state>.html` and `pages/two-factor-<state>-mobile.html` for each state (loaded, loading, error); open them in a browser or with Playwright. `pages/index.html` links all of them.
...
2. **Header.** Eyebrow “Step 2 of 2”, h1 “Two-factor authentication”. Actions present, in order, with the same labels: Verify. Exactly one gold (primary) action on the screen.
...
7. **States.** Force each state and compare copy and controls with the design file:
   - **loading** (`two-factor-loading.html`): the shell stays and the body is the skeleton; no data, no zeros, no stale rows.
   - **error** (`two-factor-error.html`): “That code is wrong. 2 attempts left before a 15-minute lockout.” Inputs cleared.
...
9. **Mobile.** At 390 × 844 with a touch pointer: the card fills the width with 16 px gutters, buttons are full width and ≥ 44 px, inputs are 16 px, code inputs use the numeric keypad. Compare against `pages/two-factor-loaded-mobile.html`.
...
11. **Permissions.** Read requires `authenticated (first factor)`; each write (auth.2fa) is gated server-side, not only hidden in the UI. Verify with a role that lacks the permission.
