...
| Design | `mc.html` → `pRun()` hands this run to `skRunPage()` |
| States | loaded (waiting · answered: link · answered: create) · loading · error · access denied |
| Files | `run-interjection-loaded.html` / `-mobile.html`, `run-interjection-loading.html` / `-mobile.html`, `run-interjection-error.html` / `-mobile.html`, `run-interjection-denied.html` / `-mobile.html` |
