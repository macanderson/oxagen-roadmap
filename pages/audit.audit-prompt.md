...
2. The design, rendered: `pages/audit-<state>.html` and `pages/audit-<state>-mobile.html` for each state (loaded, empty, loading, error, access denied); open them in a browser or with Playwright. `pages/index.html` links all of them.
...
2. **Header.** Eyebrow “Organization”, h1 “Audit”. Actions present, in order, with the same labels: Place a legal hold · Export evidence bundle. Exactly one gold (primary) action on the screen.
3. **Summary tiles.** 4 tiles: Events · 30 days · Denied · By a service principal · By the in-app agent. Each shows one number and one basis line; each number is a rollup of the rows under it (recompute it from the list and compare).
...
5. **Actions and dialogs.** Every button in the spec exists, opens what the spec says (`hold`, `newexport`, `incident`, `receipt`, `export (download)`, `rotate KEK`, `retention policy`, `erasure request`), and each write is a governed action: it passes IAM, produces an audit event, and shows a receipt or reference. A stub must say what the product would do; a control that silently does nothing is a FAIL.
...
7. **States.** Force each state and compare copy and controls with the design file:
   - **empty** (`audit-empty.html`): “No audit events yet” — events are written by the kernel on every governed action; empty means nothing has been done yet, not that recording is off. Action: Open Organization.
   - **loading** (`audit-loading.html`): the shell stays and the body is the skeleton; no data, no zeros, no stale rows.
   - **error** (`audit-error.html`): “Audit could not be loaded” — `503 audit_store_unavailable`. Nothing was changed. Runs kept recording while this page was down — frames are written by the gateway, not by Mission Control. Actions: Try again, Open an incident; a trace id, region and timestamp line.
   - **access denied** (`audit-denied.html`): “You cannot see the audit record” — the roles the signed-in person holds on the organization do not include `org.auditor or org.owner`. Copy explains an owner can grant it and that the grant is itself a governed action in the audit record. Actions: Request access (opens the request-access dialog), Back to Fleet. Below: *Signed in as* (name · role), *Needed* (the permission), *Decided by* (`pol_v41
...
11. **Permissions.** Read requires `org.auditor or org.owner`; each write (hold.place / release; export.create; key.rotate; incident.open / resolve; retention.edit; erasure.record) is gated server-side, not only hidden in the UI. Verify with a role that lacks the permission.
