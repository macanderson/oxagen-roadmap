...
You are auditing the **Verify email** page of Oxagen Mission Control (`#/welcome/verify`) for conformance to its design. Be exact and adversarial: the design is the spec, and “close enough” is a fail. Do not summarise what you see; compare it.

## Inputs

1. The page spec: `pages/verify-email.md` (read it first, in full).
2. The design, rendered: `pages/verify-email-<state>.html` and `pages/verify-email-<state>-mobile.html` for each state (loaded, loading, error); open them in a browser or with Playwright. `pages/index.html` links all of them.
...
2. **Header.** Eyebrow “Step 1 of 2”, h1 “Check your email”. Actions present, in order, with the same labels: Verify email. Exactly one gold (primary) action on the screen.
...
7. **States.** Force each state and compare copy and controls with the design file:
   - **loading** (`verify-email-loading.html`): the shell stays and the body is the skeleton; no data, no zeros, no stale rows.
   - **error** (`verify-email-error.html`): “That code has expired. Codes last 10 minutes. Send a new one below.” The inputs are cleared.
...
9. **Mobile.** At 390 × 844 with a touch pointer: the card fills the width with 16 px gutters, buttons are full width and ≥ 44 px, inputs are 16 px, code inputs use the numeric keypad. Compare against `pages/verify-email-loaded-mobile.html`.
...
11. **Permissions.** Read requires `public`; each write (auth.verify) is gated server-side, not only hidden in the UI. Verify with a role that lacks the permission.
