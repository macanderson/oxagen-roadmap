...
| Design | `mc.html` → `pAgentSource(r)` (the single source; `consolidated.html` is the product build of it) |
| States | loaded · loading · error · access denied |
| Files | `agent-source-loaded.html` / `agent-source-loaded-mobile.html`, `agent-source-loading.html` / `agent-source-loading-mobile.html`, `agent-source-error.html` / `agent-source-error-mobile.html`, `agent-source-denied.html` / `agent-source-denied-mobile.html` |
...
- **error** — “This file could not be loaded” — `502 git_read_unreachable`. Nothing was changed. Runs kept recording while this page was down — frames are written by the gateway, not by Mission Control. Actions: **Try again**, **Open an incident**; a trace id, region and timestamp line.
