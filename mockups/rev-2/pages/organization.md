...
- **People**: panel "People" with the badge "two-factor required" and **Invite**; filters Status (active, invited), Two-factor (TOTP, hardware key, passkey, passkey + TOTP), Rows; columns Person (name and email), Role, Workspaces, Two-factor, Last seen, Status; per row **Open** (opens `member`), **Change role** (opens `role`), **Remove** (opens `removemember`); a note that changing a role is a governed action. Beneath: **Delegation ceiling** (the intersection rule and the formula `effective = agent.grants ∩ operator.grants ∩ policy_bundle ∩ kill_switches`) and **Roles in use** (**Manage roles**; one row per human role with its holder count and description; a footer counts the agent and service roles on the Roles tab).
...
- **Model funding and routes**: **Model key** (the source badge; Secret prefix, Provisioned id, Key hash, Name on the account, Minted, Monthly cap, Reads, Engine; **Rotate**, **Revoke**, **Change source**; a Reconciliation block with OpenRouter reports, Our credit ledger, Difference; or, with no key, **Mint a key**). **Model routes** for Oxagen's own work: Tier, Provider, Route, Fallback, Use, Cost, **Edit** (opens `editroute`), and a Total row with basis `client_attested`. **In-firewall routes**: Tier, Endpoint, Dialect, Model served inside the network, **Edit**. **Funding source**: a Source select, Current, Cap, Key storage, "No call reads the environment", **Change funding source** (opens `funding`).
...
| Model key, funding, routes | `ORG_KEY`, `ORG_ROUTES` | `org.organizations.funding_source/model_routes`, `org.model_credentials` | `org.model_credentials`, `workspace.routing_policy` | 🟡 |
