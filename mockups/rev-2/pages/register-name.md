...
- Note under the grid: “Continue mints a one-time enrollment token for `a-intel.core.perf-watch`. Nothing is written to Postgres and no PR is opened until the first frame arrives; the smoke session then opens the Steering PR that adds the definition file.”
