...
3. **One gold action.** Write a steering record in the hub header is the one gold action. The chip is not gold.
...
5. **Actions.** Write a steering record opens the record wizard. No row on this tab is a control; nothing is edited here.
...
7. **States.** Force each state and compare copy and controls with the design:
   - **empty** (`state=empty`): the hub header, chip, and tabs stay; the body is “No ontology notes yet” with its sentences and Write a steering record.
