...
3. The product spec for context: `docs/mission-control-spec.md` §10 (the repository, steering and Steering PRs), §11.2 (GitHub: events in, code graph up to date), §14, Appendix F; `docs/creation-spec.md` (the one shape all five wizards share).
...
4. **Sections, tabs and tables.** For each item below, the build has it, with the same tab labels (and live counts where the design shows them), the same panel headings, and every table column named in the spec, in that order. Missing or renamed columns are FAILs; extra columns are noted.
   ...
   - Working copies: Directory · Repository · Branch · `.oxagen/` · Symlinks · Bundle · Last seen. The behind-is-not-behind banner is present when a copy is out of step. Panels: **Files to review**, **Sync**.
...
5. **Actions and dialogs.** Every button in the spec exists, opens what the spec says (`wz (init wizard)`, `linkdir`, `workcopy`, `repo`), and each write is a governed action: it passes IAM, produces an audit event, and shows a receipt or reference. Table rows open their dialog by click, Enter, and Space. A stub must say what the product would do; a control that silently does nothing is a FAIL.
...
14. **Permissions.** Read requires `repository.read`; each write (`repository.link`, `repository.admin`, `context.propose`, `context.review`, `workcopy.link`) is gated server-side, not only hidden in the UI. Verify with a role that lacks the permission. `repository.admin` (changing which repo is main) must additionally require approval and write a security event.
