...
| Scope | workspace |
| Spec | §10 (the repository, steering and Steering PRs); §11.2 (GitHub: events in, code graph up to date); §14 Mission Control; Appendix F |
...
- **`.oxagen/rules/governance.toml`** with the three modes: `solo` (the author may merge their own), `team` (a code-owner review is required; what a missing file means), `regulated` (a named approver from a role, hash-chained ledger). It is read when a pull request is opened **and again when it is merged**, so raising the mode takes effect on everything still open. A file that exists but names no mode refuses both. The workspace's current mode is the chip on Steering (`steering.md`), and changing it is a Steering PR.
