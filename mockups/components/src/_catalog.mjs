...
export default {
  ...
  findings: [
    ...
    { tag: "note", title: "Rules with no emitter", body: "`.srclink`, `.compbar`, `.legend`, `.mtr`, `.rl-tog`, `.fp-bar`, `.def-bar`, `.tid`, `.ctxi`, `.ctxg`, `.stack`, and `.agc .scs` are styled but nothing renders them. `.avat` has one caller, which overrides its radius inline. `approvalCardSm()` is never called, so the `.apsm*` rules are unused except `.apsm-clk`." },
