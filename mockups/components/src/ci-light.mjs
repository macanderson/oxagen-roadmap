...
export default {
  ...
  helpers: [
    ["ciLight(c, label)", "engine.js:7384", "Picks the state from a counts object and writes the dot, title, and count. An optional label appends a `.dim` caption."],
    ["ciCounts(list)", "engine.js:7374", "Tallies an array of per-check states into `{pass, fail, running, queued, total, done}`."],
    ["ciFromSt(def, st)", "engine.js:7379", "Builds a plausible per-check state list from a change's overall status, for fixture data with no real per-check log."],
    ["oxprCiLight(p)", "engine.js:7390", "The Repositories Changes table's own wrapper: counts a proposal's checks and calls `ciLight()`."],
  ],
  sourceNotes: [
