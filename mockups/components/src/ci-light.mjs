...
export default {
  ...
  a11y: [
    ...
    "The running blink and the failure pulse both stop under `prefers-reduced-motion: reduce` (engine.css 2138), leaving the light static but still correctly colored.",
  ],
  phone: [
  ...
  helpers: [
    ["ciLight(c, label)", "engine.js:7079", "Picks the state from a counts object and writes the dot, title, and count. An optional label appends a `.dim` caption."],
    ["ciCounts(list)", "engine.js:7069", "Tallies an array of per-check states into `{pass, fail, running, queued, total, done}`."],
    ["ciFromSt(def, st)", "engine.js:7074", "Builds a plausible per-check state list from a change's overall status, for fixture data with no real per-check log."],
    ["oxprCiLight(p)", "engine.js:7085", "The Repositories Changes table's own wrapper: counts a proposal's checks and calls `ciLight()`."],
  ],
  sourceNotes: [
    "The design rule, from the CSS comment at engine.css 2126 to 2127: blink blue while anything runs, a red ✕ the moment one job fails, pulsing while others still run, static green once every job has passed, static grey while everything queues.",
