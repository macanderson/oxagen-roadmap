...
export default {
  ...
  helpers: [
    ["listify()", "engine.js:13932", "After every render, runs `ltTable()` on each table in the page and the dialog layer, and `ltCards()` on each card list."],
    ["ltTable(table, prefix)", "engine.js:13827", "Reads the table, picks filters, makes headers sortable, and inserts the bar and pager."],
    ["ltBar(st, opts, apply)", "engine.js:13753", "Writes the control bar: search, filters, an optional Sort, and Rows."],
    ["ltMultiFacets(cols, rows, vals)", "engine.js:13797", "Reads each `data-facet=\"multi\"` column's `data-fv` values into options with their row counts."],
    ["ltMulti(st, f)", "engine.js:13714", "Writes a multi-select filter: the button and its popover list."],
    ["ltMultiWire(m, st, f, apply)", "engine.js:13724", "Places the list under its button, closes it on scroll, and applies each pick."],
    ["ltPager(st, total, onPage)", "engine.js:13681", "Writes the range and the page buttons, with ellipses past seven pages."],
    ["ltCards(cfg)", "engine.js:13901", "The same bar and pager for card lists, configured in `LT_CARDS`."],
    ["cardTables()", "engine.js:13998", "Turns tables into labelled cards in the phone shell."],
  ],
  sourceNotes: [
