...
export default {
  ...
  helpers: [
    ["avatarHtml(av, size, shape, extra, letters)", "engine.js:339", "The one avatar renderer: reads `{kind, icon|text|url, font, tone}` off a record and writes the `.avx` span. `letters` is the fallback when the record has no avatar."],
    ["personAv(key, size)", "engine.js:353", "Looks up a person by key and renders them round. 12 calls."],
    ["agentAv(a, size)", "engine.js:354", "Takes an agent object or key and renders it as a squircle. 7 calls."],
    ...
    ["openAvatar(target)", "engine.js:571", "Opens the one avatar editor for `person:<key>`, `agent:<key>`, `workspace:<slug>`, or `org`."],
  ],
  sourceNotes: [
