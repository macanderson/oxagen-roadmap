...
7. **No teaching panels.** Nothing follows the table: the `.oxagen/` layout and the CLI commands live in the component help. If a build names a CLI command anywhere, it uses the name the CLI ships, or marks it not available: `oxagen pull` does not exist today.
8. **`linkdir`.** One command with the workspace in it, the pairing code with its expiry (or "not issued yet" while no code is issued), the one-pairing hint, Close, and Copy command as the dialog's gold action. What the command writes and reads is in the component help, not on the dialog. There is no browse button and no path field.
9. **`workcopy`.** The facts (Repository, Remote, Branch, Machine, `.oxagen/`, Symlinks, Bundle, Last seen), one short note by state (in sync, behind, symlinks missing, uncommitted), the uncommitted file list where there is one, Disconnect, and Ask for a pull on a copy not in sync. No note claims the copy's state changes what a run is steered by.
10. **`copyoff`.** The confirm says Oxagen forgets the directory and nothing on disk is deleted, and warns when uncommitted edits sit under `.oxagen/`. Keep it and Disconnect it. Disconnecting opens no pull request, is a governed action, and lands in Audit.
...
14. **Mobile.** At 390 × 844 with a touch pointer: More is lit in the thumb bar; the table becomes labelled cards; the page never scrolls sideways; the dialogs are bottom sheets with full-width footer buttons; touch targets are at least 44 px.
