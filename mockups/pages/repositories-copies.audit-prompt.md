...
7. **Files to review and Sync.** The tree shows `workspace.toml` as committed and `workspace.json` as gitignored, with no note beneath. Sync lists the four commands with one line each and no note. A command the CLI does not ship is named as the CLI names it, or marked not available: `oxagen pull` does not exist today.
