...
1. **Header.** The id as eyebrow, the title as h1, "Sent by <name> on <time> to <target>." **Stop the work order** (red) while open. **Accept the work** is gold only when every item is claimed and the work order is not accepted; otherwise disabled with its reason. One gold on the screen. **Copy prompt** (plain) copies the prompt as sent, character for character, then the work order id, link, and digest, and every task's number, issue link, and Oxagen task id and link.
