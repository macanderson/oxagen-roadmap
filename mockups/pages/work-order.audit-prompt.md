...
8. **Handoffs.** Every stage run in order with its note, a return labeled “sent the work back”, and the note that a handoff is quoted evidence. Verify in the delivery code that a handoff note reaches the next agent inside a quoted-evidence frame, never as an instruction. A sealed run is never labelled “running”.
