...
3. The product spec: `docs/fleet-operations-wedge.md` (D4, D5, D8, D15), `mockups/pages/steering-source.md` (a memory's page) and `mockups/pages/steering-proposals.md` (the proposal a memory becomes).
...
3. **Order.** The tab reads, top down: Memories from this run, then Self-grade. Nothing else, and no lead note.
4. **No explainer text.** No sentence on the tab explains how Oxagen writes a memory, the seal, or the fold setting. That text lives in the component help, `mockups/help/run-memories.md` (Memories from this run). A sentence on the tab that explains the design is a FAIL.
...
7. **Empty and unsealed.** On `run_01K5RS7M2E8FJ3QW` the table is replaced by "No memories yet. This run is live." On `run_01K5RF2J7M3EDC5F` it is replaced by "This run wrote no memories." A build that writes a memory before the seal is a FAIL.
8. **Memory dialog.** A row opens the memory, with the memory's id as the subtitle: Class and force, Scope, Where it came from, Recalled, Cost, In force since, the position line ("Superseded by <source>.", "Yields to <record>, a published must." or "Competes in the per-prompt selection at force <force>."), the Sayings list and the fold line. A run saying links to that run's Memories tab. An imported saying shows its file and line with "imported". The fold line for an imported saying says it counts toward the sayings and never toward the runs. The footer holds Close, Forget, and one gold action: "Open the proposal" when the memory was proposed, otherwise "Promote to a record".
...
10. **Self-grade states.** Unsealed or ungraded: "Not captured yet. This run is <status>." Deleted (`run_01K4QJ9E4T6YUI1O`): "Deleted on 2025-12-29, 180 days after capture." Locked, as Marcus: "Reading a self-grade takes research.read" and "Marcus Bell does not hold it.", the Captured, Rubric, Model, Cost and Retention list, "Request access" (gold) and "How reflection works". Readable, as Priya: four question blocks with "self" and "the record" bars out of 5, the quoted answer, "the record says: …", "calibration gap" where self exceeds the record by 2 or more, and no research-only note, with no buttons. The research-only fence is explained in the component help (Self-grade).
