...
| Design | `mockups/src/engine.js` → `pTasks()`, `tkTaskTab()`, `tkWoTab()`, `tkWfTab()`, `dispatchButton()`, `dispatchMenu()`, `tkGraph()`, `tkBlockedBy()`, `tkWhyNot()`, `DLG_EXT.wo`, `DLG_EXT.wfview`, `DLG_EXT.wfnew`, built into `mockups/missioncontrol.html` by `tools/build-mockup.mjs`; data `mockups/fixtures/tasks.json` |
