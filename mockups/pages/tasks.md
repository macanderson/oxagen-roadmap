...
- **Tiles**: Ready for work ("certified and open"), Drafts waiting on you ("certify to make them ready", blue when above zero), In work orders ("sent to an agent or a workflow"), Being drafted ("oxagen.assistant is reading them").
...
- **Tasks** panel, subtext "Only a ready task can be selected and sent.", with "N selected" and **Clear** in its header while a selection exists. Search, facets and Rows come from the shared list bar. Columns: select · Task (provider logo, number in mono, subject) · Labels (colour chips) · Status (dot and word: Open, In review, Blocked, Closed) · Owner (the mapped member with avatar, or the provider handle with its logo and `not mapped` or `bot`) · Readiness (`drafting`, `draft`, `changed`, `ready`, `in a work order`, `accepted`, `closed`) · Updated.
- **Selection**. A checkbox is enabled only on a task whose readiness is `ready` and whose status category is `open`. A disabled checkbox carries the reason as its title ("Its definition of done is a draft. Certify it first.", "Blocked upstream. It can be sent when it is open again."). A selected row is tinted. A row click opens the task (`task.md`); the checkbox click does not.
...
2. **Sent to**: a select of the agents you operate and the published workflows. Below it the agent card (harness mark, avatar, name, "harness on host", tier, `operator: you`), or the workflow's stage chain ending in **Accept** by You, each stage with its harness and tier. Under it, **where it starts**, one line per agent (spec §9.6): "<agent> starts on <host>, your machine" or "<owner>'s machine", with the tier floor the owner set and the tier it runs at; "starts on <runner> through `repository_dispatch`" for a CI runner; or, for a host that has not opted in, that it "does not accept remote starts" and that a person there runs `oxagen work start`. Then "Oxagen starts the harness and hands it this brief. The harness runs every turn."
...
- **`wfview`**: the stage chain with what each stage owns and what happens on failure, the file as committed, and whether it is published or in a pull request. Footer **Close**, **Change it**.
...
| Where a run starts | `RT_START` | the host's local `accept_work_orders` and tier floor; `start_run` through the ADR-096 launcher | the daemon on wrapped hosts exists; the launcher is Phase 5 and unbuilt | ❌ |
