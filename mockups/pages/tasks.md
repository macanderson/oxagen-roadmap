...
| Scope | workspace |
| Spec | `docs/tasks-spec.md` §3, §8.6, §9, §10; `providers`, `fields` and `people` are specified in `tasks-providers.md` |
| Design | `mockups/src/engine.js` → `pTasks()`, `tkTaskTab()`, `tkWoTab()`, `tkWfTab()`, `dispatchButton()`, `dispatchMenu()`, `DLG_EXT.wo`, `DLG_EXT.wfview`, `DLG_EXT.wfnew`, built into `mockups/missioncontrol.html` by `tools/build-mockup.mjs`; data `mockups/fixtures/tasks.json` |
...
Actions: on the Tasks tab, **Connect an issue tracker** (plain) and **Send to an agent…** (gold, disabled until a task is selected). On Workflows, **Connect an issue tracker** (plain) and **New workflow** (gold). On every other tab, **Connect an issue tracker** (gold).

- **Tabs**: Tasks (N) · Work orders (N) · Workflows · Trackers (N) · Fields · People (N). Tasks counts tasks that need certification ("N tasks need certification" as the count's title). Work orders counts those waiting on you ("N work orders are waiting on you"). Trackers counts connections ("N trackers connected"). People counts matches that need confirmation ("N matches need your confirmation"). A tab with nothing waiting shows no count. The route key of Trackers stays `providers`.

### Tasks

- **Tiles**: Ready ("certified and open"), Needs certification ("drafts and tasks changed since certified", blue when above zero), In a work order ("sent to an agent or a workflow"), Drafting ("oxagen.assistant is reading them").
- **Banner** when a certified task changed in its tracker: **Changed since certified**, "N task(s) changed since certified", "<number> was edited in <tracker> on <time>. It’s no longer ready. Certify its definition of done again to send it.", **Review changes**.
- **Tasks** panel, with "N selected" and **Clear** in its header while a selection exists. Search, facets and Rows come from the shared list bar. Columns: select (no label; `aria-label="Select task"`) · Task (provider logo, number in mono, subject) · Labels (colour chips) · Status (dot and word: Open, In review, Blocked, Closed) · Owner (the mapped member with avatar, or the tracker handle with its logo and "(not mapped)" or "(bot)") · Readiness (Drafting, Needs certification, Changed since certified, Ready, In a work order, Accepted, Closed) · Updated.
- **Selection**. A checkbox is enabled only on a task whose readiness is `ready` and whose status category is `open`. A disabled checkbox carries the reason as its title ("Certify this task’s definition of done to select it.", "Blocked upstream. It can be sent when it is open again."). A selected row is tinted. A row click opens the task (`task.md`); the checkbox click does not.
...
**Send to an agent…** opens a menu under the button. Header "Send N tasks to", "agents where you are the registered operator". A search field narrows both groups.

- **Agents you operate**: every agent in the workspace whose operator is the signed-in person, each row with its harness mark (Claude Code, Codex, Cursor, Stella, the Agent SDK), its avatar, its name, "harness · host", and its tier badge. The four seeded workflow agents come first, then the rest by name; twelve are shown and the rest are counted.
- **Workflows**: every published workflow, each with its stages' harness marks in order and "N stages then you".
- An agent somebody else operates is never listed. Picking a row opens the work order dialog.

### The work order dialog (`wo`)

Title "Work order", subtitle "N tasks to <agent or workflow>". Six sections.

1. **Tasks**: one chip per task (provider logo, number, subject), removable while more than one remains. Hint: "Each task is tagged to this work order, and each shows it on its own page."
2. **Sent to**: a select of the agents you operate and the published workflows. Below it the agent card (harness mark, avatar, name, "harness on host", tier, `operator: you`), or the workflow's stage chain ending in **Accept** by You.
...
6. **Spend cap**: a USD field, default 12.00, and the hint on per-run budgets. When a stage runs on Cursor, a warning: "Cursor is not metered."

Footer: "needs `work_order.send` on core-platform", **Cancel**, **Send to <target>** (gold). Send is disabled until the repositories are confirmed, at least one is checked, and the prompt is not empty. Sending tags the tasks `in a work order`, clears them from the selection, opens the work order page (`work-order.md`), and toasts "Work order <id> sent to <target>. It starts when <host> picks it up."

### Work orders

The **Work orders** panel, subtext "Each one carries tasks, their definition of done, a prompt, and the repositories it may change." Columns: Work order (title and id) · Tasks (logo and number, one per line) · Sent to (harness marks and name, or the workflow) · Stage ("2 of 4 Validate", "1 of 1", or "done") · Items claimed (claimed / total) · State (`sent`, `in progress`, `waiting on you`, `returned`, `parked for you`, `stopped`, `accepted`) · Sent by (avatar, name, time). A row opens the work order. Note: "An agent claims an item with evidence. A person accepts it. A work order is done when you accept every item, and nothing merges without a person." With none sent, the panel says so and points at the Tasks tab.

### Workflows

Two columns. Left, the **Workflows** panel, subtext "A workflow is a file in .oxagen/workflows that names its stages in order." Columns: Workflow (name and file path) · Stages (harness mark and role, arrows, ending in You) · State (`published` with its commit, or `pull request open` with its number) · Work orders. A row opens `wfview`. Right, **How a workflow runs**: five numbered facts (a stage is its own run; a stage owns items by tag; a handoff note arrives as quoted evidence; returns are bounded and then the work parks for you in Approvals; the last stage is a person), and the note that every agent in a workflow must be one you operate.

- **`wfview`**: the stage chain with what each stage owns and what happens if it fails ("If it fails: stop and ask you", or "If it fails: send back to <role> (up to N times)"), the file as committed, and whether it is Published or In review. Footer **Close**, **Edit workflow**.
- **`wfnew`** (the builder): **In your own words** with the wand, **Name**, **Stages** (each: number, role, agent select of agents you operate with the harness mark, up, down, remove; Owns checkboxes for code, test, docs, review; On failure: stop and ask you, or return to an earlier stage, with at most 1 to 3), the fixed **Accept** stage for You, **Add a stage**, the TOML file as it will be committed, and "A workflow is a file. It exists when the pull request merges, and a reviewer can stop it there." Footer "needs `context.propose`", **Cancel**, **Open pull request** (gold, disabled without a name). The wand turns "A bug fixer passes a fix to a validator, which passes it to a documenter, which passes it to an architect for final review." into four stages in that order.
...
- Only a ready task whose status category is `open` can be selected.
- Work reaches an agent only in a work order, and only an agent the sender operates.
...
- Read: `task.read`
- Writes, each a governed action in Audit: `work_order.send` (send a work order to an agent you operate), `context.propose` (open a workflow's pull request)
