...
- **Agent**: a select labelled "Agent". Each option reads "<name> · <harness>": "Release manager · Claude Code", "Stella CI · Stella", "Triage · Codex CLI", "Docs writer · Custom (SDK-wrapped)". An agent named in the address that is not among the standing ones is added to the list ("Pr Reviewer · Claude Code"). Under the select, the agent's tier badge and "works in a-intel/platform". Changing the agent writes `/steering/compiler/<slug>` into the address.
...
- Caption: "44 SteeringFrames for release-manager in a-intel/platform. Select a type to filter."
...
- Each block is a table. Columns, in order: Type, Frame, Source, Force and Tokens.
  - **Type**: the frame-type badge, with the type's description as its title.
  - **Frame**: the body. A frame a gate enforces adds "enforced by <gate>" under it ("enforced by gate.never-merge", "enforced by decision rule", "enforced by mandate", "enforced by kill switch").
...
- The table shows its first six rows and "Show all 23". Rows are ordered by reason, then by type, force and id. With nothing excluded it reads "Nothing was excluded."
...
- How the envelope resolves:
  ...
  4. The `may` and `info` candidates are ranked by relevance to the brief and packed under the volatile budget. A candidate with relevance 0 is `below_relevance_floor`; one that does not fit is `over_budget`, with its rank, its relevance, its cost and what was left.
  ...
  7. The belt adds one `capability` per tool it shows the model. A harness's own tools are not frames. A tool the belt denies is excluded as `overridden_by_gate`, with the rule.
...
- The source adapters beyond Steering records: documents, skills and their bundles, memory, glossary terms, gate notices, instructions, mandates as delegation frames and the belt as capability frames (#3830, #3880, #3904).
- The volatile selection and its budget, and the exclusion reasons past `tier`, `budget` and `superseded`.
...
- Plain nouns. A heading names the thing, a caption states one fact, and no label carries a comma, a mid-dot, or a not/never contrast. Subtext under a heading is one sentence or nothing; the mockup's two-sentence captions under Envelope and Exclusions, and the mid-dot in each agent option, are design defects, not patterns to copy.
