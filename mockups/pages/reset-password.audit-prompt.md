...
2. The design, rendered: the `reset-password` stories in Storybook (`npm run storybook`), one per state (loaded, loading, error, access denied), desktop and mobile; or `mockups/missioncontrol.html?product=1&state=<state>&mobile=<0|1>#<route>` in a browser or with Playwright.
...
7. **States.** Force each state and compare copy and controls with the design file:
   ...
   - **error** (`state=error`): “The two passwords do not match. Retype the confirmation.” Confirmation field marked bad.
   - **access denied** (`state=denied`): Full-card state: “This reset link has expired” — links last 60 minutes and can be used once; when this one was issued. Action: Request a new link.
