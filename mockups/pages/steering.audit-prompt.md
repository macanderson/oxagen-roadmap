...
3. **Header.** Eyebrow is the workspace name, h1 "Steering", and no subtext (the page's purpose is in its component help). The actions, left to right: the governance chip "Governance: <mode> (<review rule>)", such as "Governance: team (code-owner review)", the skills chip "Skills settings", its title naming the settings version (only where skills are on; absent on FinOps), Import Markdown (plain, on every tab), and New source. Neither chip nor Import Markdown is gold.
...
6. **Panel heading and caption.** The heading is "All sources" or the picked kind's label. The caption states the row count and how many emit nothing, in one sentence. The type strip sums the Emits column of the listed rows. A number that disagrees with the rows is a FAIL; name both values.
...
11. **Dialogs.** `govmode`: title, subtitle, three cards with the one in force marked "· now", the file the pick would write (the comment `draftGovernanceToml` writes, no dash-joined line), no note, Cancel and Open the pull request. Confirming calls `set_governance_mode` and reports the pull request, or the commit under `solo`; picking the mode in force reports that nothing changed. `skcfg`: title, subtitle, the file, the Sources list with their counts and notes, and Close; nothing links to a Reflection tab. `newsrc`: the four cards with Write one, Register one, Add one and Define one, no note, and Cancel. `srcreg`: the registration, the note naming the pull request that last changed the file, and Open the pull request. Each dialog is `role=dialog`, `aria-modal`, with a labelled close.
...
20. **Markdown import wizard.** Walk it with the sample directory.
    ...
    - Step 1: the drop zone and its three buttons; the Found table with Include, File, Lines, KB, Tokens and Import as, in that order; Import every file as; and Skipped, grouped by reason, with each reason's paths under it. The cost note is verbatim. Parse with stella is disabled while no file is included.
    ...
    - Publishing opens the pull requests (each described "Imported from <file>"), writes the memories, records `steering_imported` in Audit, and changes no Markdown file. It lands on `/steering/proposals/prs` with the first pull request selected, or on `?kind=memory` when it opened none. A skipped directory is never opened, and no file over 200 KB is read.
21. **Nothing extra.** List anything on the built view that is not in the spec: a Library section row, a Memory, Ontology or Gates tab, a Skills console view, a frame list on Sources. Each is a finding.
