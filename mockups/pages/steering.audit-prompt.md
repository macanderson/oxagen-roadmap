...
6. **One gold action.** Write a context record is the one gold action, and a tab that holds its own primary action takes the gold from the header. The chip is not gold. Import Markdown is plain wherever it appears. In the empty state the header holds only the chip, and the gold, where the state has one, is the empty-state button. The Memory shelf's empty state holds no gold.
7. **The Library, All shelf.** The stat strip has Items, By kind, Compiled size, and Carry a grant, each with the basis line the spec gives. The lead note is verbatim. The **Everything written down** panel is badged with the item count and carries **Who receives it**, which opens Assignments. Columns: Item, Kind, Force, Scope, Compiles to, Token cost, Source, in that order. Kind is the Records shelf's badge with its icon and hue, never a grey “record · rule” chip. A skill has “skill” under the badge, a memory its class, and an ontology note its kind. An ontology note, an instruction, and a gate notice use the neutral hue, so the six hues stay the six kinds. Order is the assembler's: record, instruction, skill, definition, memory, and within a kind by force. No gate notice appears here. The Items tile, the All chip, and the Library tab badge carry the same number, and that number is the sum of the shelf chips beside All. The Compiles to chip is a button only where the item carries a grant, and it opens Gates.
...
11. **States.** Force each state and compare copy and controls with the design. In **empty**, the empty state is the tab's own, and on the Library it is the shelf's: the All and Records shelves read “Nothing steers this workspace yet” with Import Markdown (plain) and Write a context record (gold), Memory reads “Nothing has been recalled yet” with Import Markdown alone, and Ontology reads “No ontology notes yet”. In **loading** the shell stays and the body is the skeleton. In **error**, `503 record_index_unavailable` with the three sentences, Try again, Open an incident, and the trace line. In **access denied**, `steering.read on core-platform` with Request access, Back to Fleet, Signed in as, Needed, and Decided by.
...
15. **Permissions.** Read requires `steering.read`, checked server-side. Each write (`context.propose`, `context.review`, `context.retire`) is gated server-side. Lowering the governance mode requires an org-owner approval and writes a security event. The Markdown import's footer names `steering.write · memory.write`, and the build checks both server-side before it writes. The mockup has no refused state for the import, so record what the build shows as a note.
16. **Markdown import entry points.** Import Markdown sits in the hub header between the chip and Write a context record, in the Proposals panel header, in the ⌘K group Create, and as the Markdown import card in the `create` dialog. The All, Records, and Memory empty states offer it too. Each opens the same wizard, and none is gold.
17. **Markdown import wizard.** Walk it with the sample directory.
    ...
    - Step 1: the drop zone and its three buttons; the Found table with Include, File, Lines, KB, Tokens, and Import as, in that order; Import every file as; and Skipped, grouped by reason, with `node_modules/`, `.git/`, `docs/img/topology.png`, and `CHANGELOG.md (412 KB)` under their reasons. The cost note is verbatim. Parse with stella is disabled while no file is included.
    - Step 2: the four tiles; Accept all, Reject all, Import everything as, and the count. A duplicate starts rejected, and Accept all leaves it rejected. Each row shows its source, the reason stella gives, its notes, and its controls. A memory row shows its force as a badge, never must or should. Review what publishes is disabled until a line is accepted.
    - Step 3: one pull request per source file on `context/import-<file slug>`, the Memories field and its hint, and the gold button's wording.
    - Publishing opens the pull requests, writes the memories, records `steering_imported` in Audit, and changes no Markdown file. A skipped directory is never opened, and no file over 200 KB is read.
18. **Nothing extra.** List anything on the built page that is not in the spec. Each is a finding.
