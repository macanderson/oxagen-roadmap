...
3. **Header.** Eyebrow is the workspace name, h1 "Steering", subtext "Every source that can steer an agent here, and the frames it emits." The actions, left to right: the governance chip "Governance: <mode> (<review rule>)", such as "Governance: team (code-owner review)", the skills chip "Skills: <version>" (only where skills are on; absent on FinOps), Import Markdown (plain, on every tab), and New source. Neither chip nor Import Markdown is gold.
