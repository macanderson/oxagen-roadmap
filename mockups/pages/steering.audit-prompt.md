...
3. **Header.** Eyebrow is the workspace name, h1 "Steering", and no subtext (the page's purpose is in its component help). The actions, left to right: the governance chip "Governance: <mode>", such as "Governance: team", with no review claim, the skills chip "Skills settings", its title naming the settings version (only where skills are on; absent on FinOps), Import Markdown (plain, on every tab), and New source. Neither chip nor Import Markdown is gold.
