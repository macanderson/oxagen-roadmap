...
4. **Tiles.** Ready for work, Drafts waiting on you, In work orders, Being drafted, each a rollup of the rows below, never typed twice.
5. **Tasks table.** Columns in order: select · Task · Labels · Status · Owner · Readiness · Updated. Labels render as colour chips in the configured colours. Owner shows a mapped member with avatar, and an unmapped account as its provider handle with its logo and `not mapped`, a bot with `bot`. Readiness uses exactly the seven words of the spec.
6. **Selection.** Only a `ready` task with an `open` status category has an enabled checkbox. Every disabled checkbox carries the reason. Selecting a blocked, draft, changed, sent, or closed task must be impossible in the DOM **and** refused by the server. A row click opens the task; a checkbox click does not.
...
8. **The work order dialog.** Six sections in order: Tasks, Sent to, Definition of done, Prompt, Repositories, Spend cap.
   ...
   - Sent to: the select holds agents you operate and published workflows only. The agent card says `operator: you`. A workflow shows every stage with its harness and tier and ends in Accept by You.
   - Where it starts: one line per agent naming the host and whose machine it is, the owner's tier floor and the tier the run gets, `repository_dispatch` for a CI runner, and the `oxagen work start` fallback for a host that has not opted in. Verify against spec §9.6 that the host pulls a signed, expiring, single-use order; that the opt-in and the floor are read from the machine and cannot be set from the control plane; that the launcher starts only a harness binary in a registered working copy with the brief as a file and no permission-bypass flag; and that Oxagen opens no inbound connection to the host. A path that starts a process on a host that did not opt in is the most severe FAIL available on this page.
