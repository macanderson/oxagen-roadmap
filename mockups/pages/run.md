...
**Dialogs this page opens:** `pause`, `steer`, `forkreplay`, `bisect`, `runexport`, `evidence` (a pinned finding), `approve` / `deny` (from a parked approval frame on Governed actions), `request-access` (from denied), `incident` (from error).
