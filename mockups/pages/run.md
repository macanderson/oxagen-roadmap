...
**Component help.** With component help on, every page part carries a `?` that opens its section in `mockups/help/run.md`: the header, the Summary, the stat row, the tab bar, each panel, and each dialog. The Decision trace sections, the trace record line, the Skills block, the Outputs spine and the pause banner are not panels, so each carries a `data-help` key. The explanations of how each part works live there and never on the page.
...
- One row: the compact agent card (harness mark, avatar, `a-intel.core.release-manager`, "Claude Code · 212 runs 30d · $612.48", a link to the agent), the status dot and word ("live"), the tier badge (`gateway`), the work order chip "work order `wo_01K5RS7M4N`" (a link to the work order, title "The work order this run belongs to", " · direct" after the id on a direct work order), and the task chip "task a-intel/platform#482".
- The rig strip: the harness and its version ("Claude Code 2.1.4"), the model id in mono (`claude-opus-5`), "effort medium" (or "effort not captured"), and one badge per fit reading: "Model fits this work" (green) and "Less effort than needed" (approval colour). A reading the page could not make draws no badge. The readings are argued on the Cost tab (`run-cost.md`).
...
**Pause banner**, while pausing or paused, above the columns: one line, "Pausing" or "Paused" at turn N · step M, then "takes effect at the next checkpoint" or "by <name> at <time>", and the reason in quotes. Then "▶ Resume run" when paused, and "Open the pause frame". What a pause holds is in the Pause banner help section (`mockups/help/run.md`).
...
**Summary.** Eyebrow "Summary" and the badge "generated · not the record". The badge breaks the label rule below with its mid-dot and its contrast. It is the honesty label the app ships (`apps/app/messages/run.json:948`), and the conflict is open. Who was involved: the agent card (the harness mark, "a-intel.core.release-manager", "Release manager"), "on behalf of →", and the operator ("Marcus Bell", "operator · workspace.owner · core-platform", a link to that operator's spend). The generated prose. The line "Summary by claude-haiku-4-5 turn 7 · regenerated at each turn boundary · from 186 frames · cost included in this run", and "Check it against the frames", which opens the frame dialog on frame 0. The Summary is the one model-written text on the run page. It says what happened, is labelled generated, and never appears inside the Decision trace.
...
| Prompts | 2 | "1 follow-up prompt"; "No follow-up prompts" at one prompt; the approval colour above two |
| Cost | $4.13 | the basis, "Observed by gateway" |
| Unproductive | $1.75 | "1 follow-up prompt"; the listed waste cause; "None" at zero; the critical colour above zero |
...
**Intro.** One line: "186 frames, 1 steering manifest, and the send of `wo_01K5RS7M4N`. Tier `gateway`." The send clause appears only for a dispatched work order. The tier is the badge. What each tier means for the record, and that Oxagen has no access to the model's hidden reasoning, are in the Trace record help section (`mockups/help/run.md`).

Each section has a number and a heading. Sections 1 to 5 carry one data line under the heading. Sections 6 and 7 carry none. Each section's explanation is its help section in `mockups/help/run.md`, keyed by the section title.
...
- Each point is a table with the columns Type · Frame · Source · Force · Tokens.
  ...
  - Tokens: the frame's token cost, or a dash when it has none.
...
A withheld skill is excluded before ranking. Its row body reads "Withheld before ranking. The agent is told the count and the reason, never the name." and its tokens read as a dash.

**3 Choices.** "13 tools on the toolbelt, 11 calls." The tool count is the number of capability frames in the Tool list. The call count is the rows below. A harness adds its own tools, which the Choices help section says. One table with the columns At · Call · Answer · Decided by · Frame:
...
- Frame: "frame N", which opens the frame dialog, or a dash.
...
Under the table, the **Skills** block, marked future-only: "Skills" and the config it resolved, "skills.toml @ skl_v7 (commit a4c91e2)", then one row per skill with a state badge: "synced" (`a-intel.release-notes-from-prs@2.1.0` and `a-intel.rollback-a-bad-release@1.4.2`, "in the checkout"), "withheld" (`a-intel.changelog-bot@1.4.0` and `a-intel.mobile-release-notes@2.0.1`, each with its reason only, `unapproved_digest` or `out_of_scope`. What the agent is told is in the Skills help section) and "loaded" (`a-intel.release-notes-from-prs@2.1.0`, "09:14:19 · Claude Code read SKILL.md from the checkout after matching its description").

**4 Frames.** "186 recorded, 16 in view." The Timeline panel: heading "Timeline", "16 of 186 frames shown", and a legend by class with counts (model calls 6, tool calls 3, governance 4, context 1, operator 1, lifecycle 1). Turn bands ("TURN 1", "TURN 2 · AFTER STEER"), one tick per frame (taller for operator frames, approval requests and decisions that route to a person), the marks "steer" and "parked for approval", the axis "09:14:02" to "+49 s · 09:14:51 · live", and a foot that shows only "2 turns in view". How to read the ticks is in the Timeline help section. Every tick opens its frame in the frame dialog and shows the frame on hover. Under the panel, the class counts again and "Open the transcript".
...
**6 Self-reported uncertainty**, only when the agent reported a doubt in a structured field. No line under the heading. That the quote is the agent's own and Oxagen does not estimate confidence is in its help section. Marked future-only. One quote per doubt, verbatim ("#481 changes the default decision for unmatched untainted calls to approve. I listed it under Breaking, but I could not confirm whether any existing rule depends on the old default."), with the footer `oxagen__report_status` · 09:15:39 · `rep_01K5RS9Q`.
...
**7 Evidence.** No line under the heading. Four rows:
...
- **Outputs.** Eyebrow "Outputs", the tally "3 artifacts · 2 reads · 1 gate", and "Hide reads" (then "Show reads"). One node per thing the run produced, in frame order, each with its glyph, name, state badge, where it landed, a note and a "Frame N" chip that opens the frame: the task (`a-intel/platform#482`, "linked"), a read mark ("read CHANGELOG.md, 38 merged pull requests", "read once, never modified · since 4.10.3"), the branch (`release/4.11.0-notes`, "created"), the file (`RELEASE-4.11.0.md`, "written", +118 −0), the gate (`github__create_release`, "awaiting", "oxagen · rule rg_0093", with "Review the approval" and "the run is stopped here until someone answers"), and the dashed withheld node ("4.11.0 release", "withheld", "not published; main untouched"). More than three durable nodes of one kind in a row fold to the first two and a count. Foot: "In frame order.", as the app draws it. What each note means is in the Outputs help section.
...
- Body: the frame's summary line, then the panel for its kind. `policy_decision`: the decision badge ("allow" or "approve"), "decided in 6 ms", "Conditions evaluated" (Condition · Value · Result, with no caption) and the "Decision" record. `approval_request`: "Parked.", the facts Approval, Call, Rule, Approvers and Waited, and "Approve" and "Deny", which open the `approve` and `deny` dialogs. `control.steer`: "Delivery", the quoted steer, Issuer, Injected into (a link to the `model.request` that carried it) and Digest, with no note under them. Every note that explained a panel is in the Frame help section. `context.assembled`: Budget, Used, Context frames, Assembled by, the table Frame · Kind · Evidence · Tokens · Use, and "Open the steering.manifest frame". `model.request`: Provider · model, Parameters, Tools offered, Context frames, Prompt tokens, "Prompt composition" and "Message stack". `agent_start`: Run, Agent, Operator, Workspace, Task, Policy version and Enforcement tier. A kind with no panel shows its frame hash, `prev_hash`, where its body is kept, and who attested it.
...
- `pause`: "Pause this run", the facts Run, Position and Recorded as, a Reason field the model reads on resume, and "Cancel" and "Pause at the next boundary". No lede, no note and no footer text. What a pause holds, and how it differs from cancel, is in the Pause this run help section.
- `steer`: "Steer this run", a "Steering text" field, Address ("this run", or every live run of this agent) with the hint "To steer every live run in the workspace at once, use Steer on the Agents page.", Delivery ("At the boundary" or "Interrupt", each with its one-line description and no hints), and "Cancel" and "Steer". No note. How interrupt behaves by tier and the frame a steer writes are in the Steer this run help section.
- `cancelrun`: "Cancel this run?", one warning, "Nothing is rolled back. The run seals where it stopped.", and "Let it run" and "Cancel the run". What cancel revokes is in the Cancel this run help section.
- `runexport`: "Export this run", with no subtitle, Scope and Format selects, the facts Segment and Seal, and "Cancel" and "Build bundle". No callout. What the bundle carries is in the Export this run help section.
...
| Stat row: prompts, wall clock split | `runPrompts()`, `runMetrics()` | the transcript | counted in the app from `get_run_transcript` (`apps/app/src/features/run/metrics.ts`); "follow-up" is the design's reading of any prompt after the first | 🟡 |
| Stat row: unproductive | `runWaste()` | none | not recorded (`apps/app/src/features/run/stats.tsx` renders it as not recorded) | ❌ |
...
| 1 Envelope: meters | `assembleSteering().tok`, `STEERING_PREVIEW.budget` | the manifest's budget and spend | `budget_tokens`, `spent_tokens` (`wire.ts:666-677`). The shipped prefix budget is 2,000 tokens (`packages/steering-assembler/src/assemble.ts:93-100`); no per-prompt selection is delivered | 🟡 |
...
- "frame N" in Choices, every tick in the Timeline, each "Frame N" chip on the Outputs spine and "Check it against the frames" open the frame dialog. "Previous" and "Next" step through the frames in view.
...
- The intro states the run's tier as a badge. On `gateway` and `contained`, calls routed through Oxagen were checked and recorded. On `observe`, the run was recorded and not enforced. The Trace record help section says so. The page shows only the badge.
...
- In the mockup the header actions are 40 px tall, and the type buttons, "frame N" links, source links, "Show all" links and "Frame N" chips are 17 to 29 px. A build gives each a hit area of at least 44 px.
...
- Frame types, per-frame provenance (source kind, id, version, hash, injection point, body) and the per-prompt selection on `steering.manifest`, and a capability that resolves without delivering (#3879).
...
- Headers are rollups of the rows beneath them: a point's tally is the sum of its rows, each meter is the sum of its point (the session-start meter adds the compile header), the type strip counts the frames in the Envelope, the Exclusions count is its row count, and the tab counts read the same record as the tabs.
