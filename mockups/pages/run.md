...
- **First prompt** (`firstPrompt`): eyebrow “First prompt”; byline “<operator> · <started> · N tok written, M tok sent”; the prompt text; a badge, **one-shot** or **N corrective prompts after it**; buttons **What reached the model** (opens the Context tab) and **Show the prompts** (Transcript filtered to prompts; only when there is more than one).
...
- **Tabs** (`runTabs`; counts are live; a tab routes from the hash `/runs/<id>/<tab>`; the old proof, dod, and ladder keys land on Cost): **Transcript** (entries) · **Governed actions** (policy decisions; “Player” with the frame count when a run has none; a dot when a call is parked) · **Cost** ($) · **Policy** (decisions; a dot when parked) · **Context** (context frames) · **Chain and seal** (live or sealed). Transcript is the default.
...
**Side column** (`runSide`, aria-label “The work”): **Repository** (edge chip; Repository · Branch · Main) · **Pull request** (state badge; Pull request · Checks · Diff · Release; “No pull request yet.”) · **Files in the diff** (badge “+N −N”; one row per file with its stat and note; **Open the diff in the transcript**) · **Task** (edge chip “stated”; ref and title) · **Summary** (eyebrow “Summary”, badge “generated · not the record”, who was involved, the prose, “generated by <model> · turn N · regenerated at each turn boundary · read N frames · light tier · cost accounted to this run”, **Check it against the frames**) · **Outputs** (eyebrow “Outputs”, tally “N artifacts · N reads · N gate”, **Hide reads**; one node per thing produced in frame order with a state badge and an `fr N` chip; a read is a hairline tick; a governed gate sits where it stopped the run with **Review the approval**, followed by a dashed *withheld* node; “In frame order.”).

**Dialogs this page opens:** `pause`, `steer`, `forkreplay`, `bisect`, `runexport`, `evidence` (a pinned finding), `approve` / `deny` (from the Approvals panel), `request-access` (from denied), `incident` (from error).

**Shell.** Sidebar (organization switcher, workspace switcher, Workspace nav: Fleet · Agent IAM · Tools · Steering · Repositories · Spend; Organization nav: Organization · Billing · Audit; foot: the assistant launcher, agent count · data plane, connection badge). Top bar: hamburger, breadcrumbs (… / Fleet / <run id>), ⌘K search-or-run, notifications with unread dot, the approvals button (left of the avatar, count of everything waiting on you across the organization; opens the drawer described in `fleet.md`), account avatar → user menu. No assistant button in the top bar.
...
| Side column (repository, pull request, files, task) | `RUNGRAPH` | `FOR_TASK`, `OPENED_BY`, code graph | none | ❌ |
