...
5. **The enforcement note.** The build's note says where each ceiling is checked and at which tier: the kernel's admission gate for the organization and workspace ceilings, and the loopback model proxy for an agent's per-run ceiling, both for calls routed through Oxagen. A note that claims a check at a hook boundary, or a stop on the `observe` or `harness` tier, is a FAIL.
...
11. **Future-only fields.** Another workspace's ceiling, the operator scope, the daily period, soft mode, Remove and the "Current highest run this month" hint are future-only. None may render as if backed.
