...
5. **Enforcement claims.** The panel carries no enforcement note; where each ceiling is checked lives in the component help (`mockups/help/spend-budgets.md`, Budgets). Any claim the build does show (a mode label, a warning, a toast) names only what ships: the admission gate for the organization and workspace ceilings, and the loopback model proxy for an agent's per-run ceiling, both for calls routed through Oxagen. A claim of a check at each checkpoint, or of a stop on the `observe` or `harness` tier, is a FAIL.
