...
14. **Valid Cedar.** Every rule sample the page or its dialogs draw parses as Cedar: effects are `permit` or `forbid` only, annotations are `@name("value")`, and conditions use Cedar operators and methods (`has`, `like`, `in`, `.contains()`), with no invented statement or method.
