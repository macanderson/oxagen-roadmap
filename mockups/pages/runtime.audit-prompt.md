...
3. **Host panel.** The host's name as the title, the caption with its kind, operating system and what starts the collector, and the health badge in the panel header. The key-value list in order: Workspace, Owner, Harness, Collector, Hook binary, Hooks installed, Model surface, Settings, Tier, Last checkpoint, and Note only where the host has one. The Hooks installed line matches the count as the spec gives it, and no line explains a hook or the model surface; those explanations live in the component help (`mockups/help/runtime.md`, Host).
4. **Agents on this host.** The count badge, the list controls, and the columns Agent · Operator · Tier · Principal · Runs 30d in order. The Agent cell is the agent card, led by the harness mark. The Operator cell names the person, not a key. A row opens the agent and is operable by keyboard. The panel carries no note. A host with no agent reads the spec's sentence.
5. **One host, one tier.** Every agent row's tier agrees with the host's tier. An agent on a host at a different tier than the host shows is a FAIL, whichever of the two the build got wrong.
...
7. **Unenroll this host from the CLI.** The panel shows the unenroll command, and the command matches the shipped CLI (`oxagen agent unenroll`, with `--host` taking an enrollment id and no `--restore-settings`). The panel carries no note. Run a test session either starts a recorded session or says it is a stub and what it would do (#3819). Unenroll is danger and opens the dialog.
8. **Unenroll.** The dialog carries the one-sentence note and the warning verbatim, Keep it enrolled and Unenroll it. Unenroll revokes the enrollment through `revoke_tacho_enrollment`, lands in Audit, and the page shows the host as not enrolled afterwards. It passes IAM and a refusal changes nothing.
...
16. **Permissions.** Read requires `runtime.read`, checked server-side. `runtime.unenroll` is gated server-side (today `revoke_tacho_enrollment`, an org Owner or Admin) and lands in Audit. A test session requires `agent.run`.
