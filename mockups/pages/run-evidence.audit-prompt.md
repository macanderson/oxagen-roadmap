...
15. **Accessibility.** "Details" carries `aria-expanded`; the tables have headers; the countdown has an accessible name; state is never colour alone; dialogs are `role=dialog aria-modal` with a labelled close; the tab works by keyboard end to end.
