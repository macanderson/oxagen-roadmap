...
7. **Linked work.** The eyebrow, the legend of three edge chips ("observed", "stated", "inferred") each with its count and its meaning in a tooltip, the Repositories and Pull requests and artifacts panels with their counts, edge chips and forge links, their empty texts, and Files changed with the stat, "as the harness reported them" and one row per file that opens its diff. An inferred item never stands beside a record it contradicts.
...
12. **Rollups.** "<N> in this session" equals the issue rows. Each Linked work count equals its rows. The three legend counts sum to the Linked work rows, and the inferred count equals the inferred rows. The Checkpoints count equals its rows. The Files changed stat equals the sum of the file rows and the Changes panel's Diff.
