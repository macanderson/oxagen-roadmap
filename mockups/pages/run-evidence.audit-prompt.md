...
6. **Issues.** The heading with "<N> in this session", the Status filter, Rows and the pager, and the table Issue · Status · Relation · Edge · link. Each edge reads "stated", "observed" or "inferred · <N>%", with a chip per cited frame that opens it. Each "View ↗" opens the tracker's page. The empty text, verbatim, and no note under the table. The issues table appears once on the page.
7. **Linked work.** The eyebrow, the legend of three edge chips ("observed", "stated", "inferred"), each with its short meaning as the app draws it, and the inferred count "<n> of <N> rows.", the Repositories and Pull requests and artifacts panels with their counts, edge chips and forge links, their empty texts, and Files changed with the stat, "as the harness reported them" and one row per file that opens its diff. An inferred item never stands beside a record it contradicts.
8. **Chain and seal.** Hash chain with its badge, Frames, Rule, `telemetry_gap` frames, Checkpoints ("<N> · every 20 frames") and Completeness gaps, with no note. Seal and attestation: "Not sealed yet. The run is still recording." on a live run; on a sealed run, Merkle root, Over, Archive segment, Signature, Signs over, Enforcement tier (the badge alone) and "Export the bundle". Checkpoints with its table or "No checkpoint frame is shown." The gap badge reads the chain: force a run with a missing sequence and confirm the badge changes.
...
12. **Rollups.** "<N> in this session" equals the issue rows. Each Linked work count equals its rows. The legend's inferred count equals the inferred rows, over all the Linked work rows. The Checkpoints count equals its rows. The Files changed stat equals the sum of the file rows and the Changes panel's Diff.
...
18. **Nothing extra.** List anything on the built tab that is not in the spec. Each is a finding. A sentence that explains how Oxagen works or why the tab looks this way is a finding too: that text belongs in the component help, `mockups/help/run-evidence.md`.
