...
1. **Route and shell.** The build serves the route with the gate shell only: brandmark, the signed-in email, a Cancel button, the three-step rail (steps 1 and 2 done and clickable, step 3 current with `aria-current="step"`), and no caption under the card. There is no sidebar, no topbar, and so no approvals button or drawer. An app shell on this route is a FAIL. The document title names the page.
