...
**Flip banner.** After a switch is turned on, a banner sits between the header and the tab bar on every Tools tab: the badge "Switch on", then "<target> is blocked." (with "across the organization" for a class or organization switch and "in <workspace>" for a workspace switch), then "<blast radius>. Every affected call is blocked from the next tool call. Kill-switch generation is now <n>.", and **Clear it** (opens `switch`). Turning on the switch on `a-intel.core.triage` reads "a-intel.core.triage is blocked." and "18 tool versions · 1 agent · 2 runs in flight. Every affected call is blocked from the next tool call. Kill-switch generation is now 119." The banner carries `data-help="tools/flip-banner"`, and its component help is `mockups/help/tools.md`, Flip banner.
...
**Each card.** The title is the target, and the caption its level and scope. The toggle carries `role="switch"`, `aria-checked`, and the label "Flip the switch on <target>" or "Clear the switch on <target>". It reads "Off (calls allowed)" or "On (calls blocked)" and opens `switch` rather than flipping in place. Then a key-value list: Blast radius ("7 tool versions · 2 agents · 2 mandates · 1 run in flight"), then Flipped by (the person over the time) and Reason for a switch that is on, then Takes effect, "On the next tool call". A flip made in this session shows its time, "2026-09-11 09:16 UTC". A class card carries `data-help="class-switch"` and a scoped card `data-help="scoped-switch"`, and the run-token backstop is in their component help (`mockups/help/tools-switches.md`). A switch created in this session also carries **Edit** (opens `switchedit`) and **Remove** (danger, opens `switchdel`). A card whose switch is on takes the denied color on its border, and the headline class switch (moves_funds) a faint gold one.
...
- `switch`, flip or clear. Title "Flip the switch on <target>" or "Clear the switch on <target>", then the level and scope.
  - Turning on: a critical banner, the badge "blast radius", the blast radius in bold, and "Every affected call is blocked from the next tool call." What happens to runs in flight, and how a class switch matches a tool imported later, are in the component help (`mockups/help/tools-switches.md`, Switch flip).
  - Clearing: the badge "restores", and "Blocked calls are allowed again from the next tool call."
  ...
  - Footer, turning on: Cancel and **Block calls now** (danger). Clearing: Cancel and **Allow calls again** (gold).
...
- `switchnew`, "Create a kill switch", subtitle "An agent, the device it runs on, or everything one operator answers for".
  ...
  - A blast radius banner that recomputes as the scope or the target changes ("18 tool versions · 1 agent"), with "A new switch starts off (calls allowed)."
  - Why it exists, with the placeholder "Held ready for an incident on this host" and the hint "One sentence."
  - No note. Which switches are permanent and which you can edit or remove is in the component help (`mockups/help/tools-switches.md`, Switch create).
...
- `switchdel`. On a switch that ships with the workspace it refuses: "a-intel cannot be removed", "a-intel ships with the workspace and cannot be removed. Turn it off instead.", with Close. On a switch that is on: "Clear it before you remove it", the warning "The switch on <target> is on right now. Removing it would let every blocked call through with no record of who allowed it.", Cancel and **Clear it** (gold, opens `switch`). On a switch that is off: "Remove the switch on <target>?", "It is off, so it blocks nothing and no run changes.", **Keep it** and **Remove** (danger: "Removed the switch on <target>.").
...
- **empty**: "No provider is registered", with **Add provider** (gold) and **Add a connection**. The Kill switches tab has no empty panel of its own: the organization, workspace and class switches exist from the day the workspace does.
