...
**Banner**, when a linked repository carries no `.oxagen/`: the badge "1 linked repository carries no .oxagen/", the bold line "A run on a-intel/mobile is steered by the main repo and by nothing of its own.", and **Add .oxagen/**, which opens the init wizard on that repository. Why a repository-scoped record has nowhere to go until the tree exists is in the component help (`mockups/help/repositories.md`, Banner).

**Repositories panel.** Heading "Repositories", and no subtext. A small **Add .oxagen/** sits at the right of the heading (not gold). The shell's list tools sit above the rows: "Search this list", the filters "Any role" (Linked, Main repository, Not linked), "Any production branch" (Main, Release) and "Any .oxagen/" (No .oxagen/, Present), Rows (5, 10, 25, 50, All), sortable headers and a pager over the six rows.
...
No note closes the panel. What main and linked repositories hold, who can move main, and how the production branch follows GitHub's default are in the component help (`mockups/help/repositories.md`, Repositories).
...
- **`repo`**, titled with the repository, subtitled "main repository", "linked repository" or "not linked to this workspace" with the language. Facts: Production branch ("main at a4c91e2"), Visibility, `.oxagen/` (the badge, then "· 9 files at a4c91e2"), Issues ("enabled · 3,907 imported"), Events, Code graph ("118,402 symbols · indexed 2 min ago", or "Not indexed"), Data layer ("3 data-layer findings"), Working copies ("2 on 2 machines", or "None").
  - On a governed repository, "Records published here". For main: "Scope is workspace. These steer every run in Core platform." For a linked one: "Scope is repository. These steer only runs on this repository." The narrow-and-never-widen rule is in the component help (`mockups/help/repositories.md`, Repository).
  - With no tree, the note "No .oxagen/ here." followed, for a linked repository, by "Runs on this repository get steering from a-intel/platform only." and, for one not linked, by "This repository is not linked to Core platform either. Adding .oxagen/ links it."
...
- **`repounlink`**, "Unlink <repository> from Core platform?". The note: "Its issues and events stop reaching this workspace, and new runs here can no longer use it." On a governed repository, the warning "Records published in this repository stop steering runs in Core platform." With working copies, the warning "1 working copy is linked through it." Footer: **Keep it linked**, **Unlink it** (red). Unlinking leaves the repository in the table as not linked and reports "<repository> unlinked from Core platform." Linking reports "<repository> linked to Core platform." What stays untouched, and what recorded runs keep, is in the component help (`mockups/help/repositories.md`, Repository unlink).
- **The init wizard**, "Connect repository", subtitle "Adds .oxagen/ to <repository>" on every step. Why the directory comes first is in the wizard's component help (`mockups/help/steering.md`, Repository wizard). A rail of five steps, the current one `aria-current="step"`: Repository · Branch & governance · Permissions · Review · Pull request. The footer names the permission it needs, "needs repository.admin on core-platform".
  1. **Repository.** No lead. The Repository select lists only the repositories with no `.oxagen/` (a-intel/mobile · Swift · private, a-intel/ledger-service · Go · private, a-intel/docs-site · MDX · public), with the hint "Only repositories the installation can already reach, and only the ones with no .oxagen/ yet." Two cards: **Make it the main repository** ("The workspace’s steering and configuration live here. A workspace has exactly one.") and **Link it** ("Agents work on it. Its records steer only runs on this repository."). The note reads "a-intel/platform is already this workspace’s main repository, so this one is linked.", or, with main chosen, "a-intel/platform is this workspace’s main repository today. Merging this makes <repository> the main repository instead." **Next**.
  2. **Branch & governance.** No lead. Production branch, with GitHub's default first and the hint "GitHub’s default branch is release, which is the suggestion." Governance mode, three cards that say what the merge gate enforces: `solo` ("Any workspace member merges, the author included."), `team` ("An org Owner or Admin, or a workspace Owner, other than the author merges."), `regulated` ("An org Owner or Admin other than the author merges, recorded as the accountable approver."), and no hint. What the production branch governs, and when the mode is read, are in the wizard's component help. **Next**.
  3. **Permissions.** A field labelled "What oxagen will be able to do in a-intel/mobile": Permission · Level · What it is for. Contents, read and write, the branch and the file every pull request carries. Pull requests, read and write, opening one and re-reading its head before a merge. Checks, write, the check runs on the head commit. Metadata, read, mandatory. Issues, read, task references so spend rolls up to an issue. What Oxagen still cannot do (push to the production branch, merge on its own, read a secret, grant authority) is in the wizard's component help. **Draft the files**.
  4. **Review.** `.oxagen/workspace.toml` and `.oxagen/rules/governance.toml` in full, each under its path. **Next**.
  5. **Pull request.** "Add .oxagen/ to a-intel/mobile", with the lead "The pull request puts the directory in a-intel/mobile." The pull request header names the repository being added and the branch `oxagen/init`. Six files: `.oxagen/workspace.toml`, `.oxagen/rules/governance.toml`, `.oxagen/rules/.gitkeep`, `.oxagen/proposals/.gitkeep`, `.oxagen/agents/.gitkeep` (each added), and `.gitignore` (changed, to ignore `.stella/private/` and the local `.oxagen/workspace.json`). "What the checks will assert": `schema`, `layout`, `governance`, `secret_pii_scan`, `no_authority`. **Open pull request** (gold) reports "Opened a-intel/mobile#118 · Add .oxagen/".
...
- **error**: "Repositories could not be loaded". "The control plane answered 503 installation_unreachable. Nothing was changed. Runs kept recording while this page was down." Actions: **Try again** (gold) and **Open an incident**. The trace line: "trace 01K5RSXQ7F2E · us-east-1 · 2026-09-11 09:16:04Z".
- **access denied**: "You cannot see this workspace’s repositories". "Your roles on Anderson Intelligence Corp. do not include repository.read on core-platform. An organization owner can grant it." Actions: **Request access** (gold) and **Back to Work**. Beneath: Signed in as "Marcus Bell · workspace.owner · core-platform", Needed "repository.read on core-platform", Decided by "pol_v41". The app today reads "Back to Fleet" and names the organization's policy. The wedge makes the way back Work.
