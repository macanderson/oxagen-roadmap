...
- **The init wizard**, "Connect repository", subtitle "Adds .oxagen/ to <repository>" on every step. Why the directory comes first is in the wizard's component help (`mockups/help/steering.md`, Repository wizard). A rail of five steps, the current one `aria-current="step"`: Repository · Branch & governance · Permissions · Review · Pull request. The footer names the permission it needs, "needs repository.admin on core-platform".
  ...
  2. **Branch & governance.** No lead. Production branch, with GitHub's default first and the hint "GitHub’s default branch is release, which is the suggestion." Governance mode, three cards: `solo` ("The author may merge their own."), `team` ("A code-owner review is required."), `regulated` ("A named approver from a role must approve, and the promotion ledger is hash-chained."), and no hint. What the production branch governs, and when the mode is read, are in the wizard's component help. **Next**.
