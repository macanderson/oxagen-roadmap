...
| 2026-09-01 | `stripe__create_payment@4` | $0.00 | released | “dispatch failed · released” | `rcp_01K4V7D3N` |
...
- `budget`, “Set a budget”: Scope, Period (per run, daily, monthly), Limit (USD, “Stored as integer micro-USD.” with the current highest run), Mode (hard or soft), **Cancel** and **Set it**. Opened here, its agent scope names this agent. The mockup's dialog names `a-intel.core.triage` and triage's highest run from every agent's tab, which is a defect.
- `mandateedit`, “Edit mnd_7K2ETQ4” with the agent key: Per call (USD), the per-period limit (labelled “Per monthly (USD)” in the mockup), Approval above (USD) (“A call above this parks for a human, and no rule elsewhere can release it.”), Valid to, and the note that lowering a ceiling below what is reserved applies from the next call and that widening one needs the second approver. **Cancel** and **Save**.
