...
2. **Work orders.** The subtext counts the work orders and runs in view and says nothing more; what a direct work order holds is in `mockups/help/agent-activity.md`, Work orders. Columns in order: Work order (id over title) · Kind (`direct` or `dispatched`) · Runs (each a link) · Status · Cost · Started. A row opens the work order; a run id opens the run. Search, the Status facet, Rows and a pager. An agent with no run in view shows the one-line panel with its 30-day count. There is no Verdict column and no Fleet link.
3. **Token accounting.** No subtext, and "30 days" in the header. Columns Class · Tokens 30d · Rate · Cost, with the four input and output classes and the Tool definitions row counted as input. The note names the cache hit rate and the tool-definition share of input, and nothing more. The removed subtext and the findings-job sentence are in `mockups/help/agent-activity.md`, Token accounting.
4. **Last 30 days.** No subtext; Open on Spend. Rows Runs, Spend with its basis, Productive ratio without a sub-line, Tokens with the cached share. The ratio's definition is in `mockups/help/agent-activity.md`, Last 30 days. Each finding open against the agent is a card with its kind, the money at stake, why, Evidence and Fix; with none, "No finding is open against this agent."
5. **Tamper incidents.** One panel per incident: the kind in mono, the caption (when, detected by, agent, host), severity and status badges, What happened, What it stopped, Closed or Owner, Incident, Open on Audit and the runs affected. No closing note or panel follows the incidents. With none, "No tamper incident recorded" with its subtext, the badge and Open the incident register, and no detector paragraph. The closing note and the detector list are in `mockups/help/agent-activity.md`, Tamper incidents.
...
11. **Trust language.** Money carries its basis, and a figure the harness reported says so. An incident's window stays labelled as observed. Nothing says proven, verdict, score or percentile.
