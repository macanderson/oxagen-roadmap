...
1. **Route and shell.** The build serves the route and resolves the mandate the route names; the sidebar (Workspace: Fleet, Agent IAM, Tools, Steering, Repositories, Spend; Organization: Organization, Billing, Audit; the assistant launcher at the foot), breadcrumbs (… / Agent IAM / <slug> / <mandate id>), ⌘K search, notifications, the approvals button, and account are present and match the spec’s shell. No assistant button in the top bar. The document title names the page.
...
5. **Actions and dialogs.** Every button in the spec exists, opens what the spec says (`mandateedit` for Change limits, `mandaterevoke` for Revoke, `receipt` from a ledger row, `request-access` from denied, `incident` from error), and each write is a governed action: it passes IAM, produces an audit event, and shows a receipt or reference. Both mandate dialogs carry the id the route names: a button that opens the grant wizard, or that acts on a different mandate, is a FAIL. Revoke says what is reserved and what already settled, and keeps the ledger. A control that reports a write it did not make is a FAIL.
