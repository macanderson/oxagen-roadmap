...
- The notifications dialog (`notifs`, from the top bar) lists `NOTIFS`. Each item shows its tone icon, title, body, kind in mono, and time. An unread item is a button (`data-notif`, aria-label “Mark read: <title>”): a click, Enter, or Space marks that one read, and focus moves to the first unread item still in the list, or to the dialog's close button when none is left. A read item is plain text. The footer reads “<N> unread · select one to mark it read”, or “All read” once nothing is unread. **Mark all read** marks the rest read, closes the dialog, and shows “All notifications marked read. Audit records who read each one.” It is disabled when nothing is unread. The top bar's dot and its aria-label “Notifications, <N> unread” follow the count. Each item marked read, one at a time or through Mark all read, writes one `notification_read` audit event with the viewer as actor, the summary `Read “<title>”`, and the item's kind as the reference.
