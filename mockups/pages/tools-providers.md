...
**Providers panel.** Title "Providers". Caption: "19 providers hold 703 tool versions." Both numbers are counted from the rows. What a provider and a transport are is in the component help (`mockups/help/tools-providers.md`, Providers). The panel header carries **Add provider** (gold, opens `import`).
...
**Credential grants log** panel. Title "Credential grants log". Caption: "45,722 grants in 30 days across 16 connections." Badge: "4 most recent shown". This table carries no list controls. Columns in order: Grant · Tool version · Agent and run · Connection · Scope · TTL · State.
...
- *Scope*: what the minted credential could reach, in mono ("repos [a-intel/platform]"). A parked call adds "Waiting on approval apr_01K5RN9T4." A settled one adds "receipt" and a link to its receipt.
- *TTL*: "5m", "2m", "10m".
- *State*: Expired, Not issued (a dot, approval colour) or Settled (a dot, allowed colour).
- No footer. How a TTL is set, where each grant is recorded, and why the agent never sees the credential are in the component help (`mockups/help/tools-providers.md`, Credential grants log).
...
- `serveredit`, "Edit <system>", no subtitle. Fields: Endpoint, Transport (`mcp`, `http`, `graphql`, `sdk`, `cli`, `native`, `local` or `rpc`), Wire (`streamable-http`, `https`, `stdio`, `in-process` or `hook`) and Connection ("none" or a connection by name). No note: that an endpoint change does not re-import is in the component help. Footer: Cancel, **Save** ("Saved. GitHub is reached at https://mcp.github.com/a-intel from the next call."). An empty endpoint is refused with "A provider needs an endpoint."
- `serverdel`, "Remove <registry name>?". Warning: "81 tool versions leave the registry. 3 toolbelts lose a tool and the calls they cover are denied as unknown_tool from the next call boundary." For a provider no toolbelt reaches: "No toolbelt reaches it, so nothing in flight is denied." No note: what is kept for replay, and how to restore, is in the component help (`mockups/help/tools-providers.md`, Provider removal). Footer: Cancel, **Remove it** (danger).
- `oauth`, "Connect <system>" or "Reconnect <system>", subtitle "OAuth". On an expired token, the warning "The token expired <time>. Calls through this connection are blocked until you authorize it again." Fields: Client id, Client secret, Authorization URL, Scopes (with "Ask for the narrowest set the tools need."), and the read-only Redirect URL `https://engine.oxagen.sh/v1/oauth/callback` with "Register this with <provider> before you authorize." Note: "Authorizing opens <provider> in a new tab." What Oxagen does with the code and the token is in the component help (`mockups/help/tools-providers.md`, OAuth connection). Footer: Cancel, **Authorize with <system>** (gold), which refuses without a client id and an authorization URL ("Enter a client id and an authorization URL first. <provider> issues both when you register oxagen as an app."). Success reads "<provider> authorized as Marcus Bell. The token expires 2026-12-20 09:00 UTC." Refresh reads "Refreshed with the provider. The new token expires 2026-12-20 09:00 UTC." Disconnect reads "Disconnected <connection>. Every call through it is blocked until someone authorizes it again."
- `conn`, one connection. Title: its name. Subtitle: its id and kind. A passed review date adds "The review date passed on <date>. It has issued <n> grants since. Marking it reviewed sets the next date 90 days out." Then Owner (a financial connection adds "finance role"), Providers, Downscope, Issued per call, Grants 30d, Reviewed ("2026-08-14 · next 2026-11-14"). Then "Recent grants" (Tool version · Scope · TTL · State). No note: that no agent holds the credential is in the component help. Footer: **Revoke** (danger), **Mark reviewed** (when due: "Reviewed. The next date is 2026-12-10, recorded with your name on it."), **Edit** (opens `connedit`).
- `connedit`, "Edit <connection>", no subtitle. Fields: Name, Owner (a person), Next review, Replace the secret. Footer: Cancel, **Save**. An empty name is refused with "A connection needs a name." How a replacement is tested is in the component help (`mockups/help/tools-providers.md`, Connection edit).
- `connrevoke`, "Revoke <connection>?". Warning: "Every credential it issued stops working at its next use. It issued 23 grants in 30 days. Calls to stripe are blocked until another connection covers them." A financial connection adds "This connection is financial. Revoking it leaves its mandates in place with nothing to draw on." Footer: Cancel, **Revoke it** (danger).
- `connection`, "Add a connection", no subtitle. Fields: Name, Kind (`oauth`, `api_key`, `cloud_role`, `github_app`), Owner (a person), Review date, Secret ("It cannot be read back after you save it."), and the `two_person` check ("Required for any connection whose tools carry a financial effect."). Footer: "Downscoping for this provider: restricted key.", Cancel, **Test and save** ("Tested against the provider, then saved.").
