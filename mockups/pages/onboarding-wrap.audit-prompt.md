...
2. **Header.** Eyebrow “Step 2 of 3”, h1 “Wrap an agent”, lead as quoted in the spec with the key `a-intel.core.release-manager`. Footer actions present, in order, with the same labels: Cancel · Back · I have already installed it — continue, with the caption “Nothing completes until a frame arrives.” before the last. Exactly one gold action on the Claude Code and Codex CLI tabs (Download for <OS>) and none on the SDK tab; the continue button is not gold.
