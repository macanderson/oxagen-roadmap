...
1. The page spec: `pages/onboarding-wrap.md` (read it first, in full).
2. The design, rendered: `pages/onboarding-wrap-<state>.html` and `pages/onboarding-wrap-<state>-mobile.html` for each state (loaded, loading, access denied); open them in a browser or with Playwright. `pages/index.html` links all of them.
...
2. **Header.** Eyebrow “Step 2 of 3”, h1 “Wrap an agent”. Actions present, in order, with the same labels: Cancel · Back · I have already installed it — continue. Exactly one gold (primary) action on the screen.
...
9. **Mobile.** At 390 × 844 with a touch pointer: the card fills the width with 16 px gutters, buttons are full width and ≥ 44 px, inputs are 16 px, code inputs use the numeric keypad. Compare against `pages/onboarding-wrap-loaded-mobile.html`.
...
11. **Permissions.** Read requires `authenticated`; each write (enrollment.create) is gated server-side, not only hidden in the UI. Verify with a role that lacks the permission.
