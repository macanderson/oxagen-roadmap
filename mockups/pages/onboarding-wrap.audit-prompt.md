...
2. The design, rendered: the `onboarding-wrap` stories in Storybook (`npm run storybook`), one per state (loaded, loading, access denied), desktop and mobile; or `mockups/missioncontrol.html?product=1&state=<state>&mobile=<0|1>#<route>` in a browser or with Playwright.
