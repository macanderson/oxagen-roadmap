...
2. **Header.** Eyebrow is the workspace name, h1 "Runtimes", the one-sentence subtext verbatim, and Enroll a runtime as the one gold action. Enroll a runtime opens Register agent at its first step.
3. **Stat strip.** Four tiles in order: Runtimes, Agents hosted, Highest tier earned, Degraded, each with the line the spec gives. Highest tier earned is a tier badge, not a bare word. Where the spec marks a design line as breaking the plain-noun rule, the build states the same fact in a caption that keeps it. Each tile is a count over the rows: recompute all four. A Degraded tile that counts a not-enrolled host is a FAIL.
4. **Enrolled hosts.** The panel carries the runtime count, the list controls (search, filters, Rows, pager) and the columns Runtime · Kind · Harness · Model surface · Tier · Agents · Collector · Hooks · Health · Last checkpoint, in that order. The Harness cell shows the harness mark before the name. The Agents cell lists the agent keys, or says that an enrolled host has no agent assigned. The Hooks cell reads "N of 5" and, below five, adds a line saying that some calls are recorded but not decided. The design's wording of both lines breaks the plain-noun rule, so the build's wording may differ. A host that is not enrolled is not presented as enrolled. A row opens the host and is operable by keyboard.
5. **The tier is the seam's.** The panel note is verbatim. The tier ladder shows the four rungs in order, `observe`, `harness`, `gateway`, `contained`, each with the spec's line, and no rung is marked current. Its note says only `contained` earns the word enforced.
6. **Nothing here enrolls a host.** No control on the page installs or writes a hook. Enroll a runtime leads to the flow whose Wrap step shows the command. The empty state's Show the CLI path shows the CLI path and the command, not an agent registration form.
...
8. **States.** Force each state and compare copy and controls with the design:
   - **empty** (`state=empty`): the header stays, then "No runtime is enrolled" with its two sentences, Enroll a runtime and Show the CLI path. Exactly one gold action on the screen.
