...
5. **The tier is the host's.** The Hosts panel carries no note, and the page draws no tier ladder panel; the four rungs and the explanation live in the component help (`mockups/help/runtimes.md`, Hosts). Each tier badge shows its word and carries its rung's text as a tooltip. No claim on the page calls any tier but `contained` fully enforced.
...
13. **Accessibility.** A clickable row is reachable and operable by keyboard and names what it opens. State is never colour alone. Focus is visible.
