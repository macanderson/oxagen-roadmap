...
5. **The tier is the host's.** The Hosts panel carries no note. The tier ladder shows the four rungs in order, `observe`, `harness`, `gateway`, `contained`, each with the spec's line of what it needs, no rung marked current, and no note beneath; the explanation lives in the component help (`mockups/help/runtimes.md`, Tier ladder). No claim on the page calls any tier but `contained` fully enforced.
