...
3. **Tiles.** At stake, Findings, Evidence and Basis with their captions. Recompute At stake as the sum of the listed savings and Findings as the count of cards. The open and in-work split must agree with the cards. A typed figure is a FAIL.
...
7. **Evidence dialog.** Title “Evidence · <kind>” and the subtitle with the id, level, window and basis. The four figures, the runs table (Run · Task · Started · Cost · Wasted · What was wasted) with its total row, How we know with the counterfactual, Who is involved, and the footer with **Close** and **Fix**. Recompute the total row from the rows: a run listed twice must not count its cost twice. Each row opens its run, and each run is sealed.
8. **Fix dialog.** The shape follows the finding’s kind. The pull request shape shows the branch, the Steering record file with the finding as evidence, why a pull request, the six checks, and **Open the pull request**, which opens the pull request through the proposal path with the finding as supporting evidence. The help article shape shows why it costs money, the before and after code, the steps, the three figures and the kind’s action, which records `record_finding_fix`. Verify the record: the finding becomes applied with the request id its audit row carries.
