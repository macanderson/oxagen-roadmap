...
2. **Header.** Eyebrow the workspace name and h1 “Work”, with no subtext (what Work is for is in `mockups/help/work-backlog.md`, Header). On Backlog the actions are **Intake** (plain) and **Send to an agent…** (gold), the second disabled with the title “Select one or more ready work items” while nothing is selected. A second gold control anywhere on the screen is a FAIL.
3. **Tabs.** Backlog · Work orders · Workflows · Findings, in that order, with `role=tablist` and `aria-selected`, each a path segment that a reload keeps. Counts: Backlog the drafts and changed certifications, Work orders the work orders waiting on you, Findings the open findings nobody picked up, Workflows none. A zero shows no number.
4. **Tiles.** Ready to send (“certified and unblocked”), Waiting on you (“N to certify · N to accept”, in the approval colour above zero), In work orders (“sent to an agent or a workflow”), Live work orders (“work orders with a live run”, with “· N parked on a person” only while a run is parked). Recompute each from the rows and from the work orders: a figure that is typed or disagrees with its rows is a FAIL. Live now is a button that opens the Work orders tab.
...
12. **Future-only fields.** Every field the spec lists as future-only (the tiles, the Work order column, and every other Work field) renders `not recorded` until its contract ships, and a control whose capability does not exist is left out or says what it will do. A control that silently does nothing is a FAIL.
