...
17. **Memories tab.** Open `/runs/<id>/memory` on three runs and compare each with the design:
    - The lead note comes first on every run and reads “**oxagen writes these after the seal.** Each lesson becomes a memory, or joins a memory that already says the same thing. At 3 sayings from 2 runs, a memory becomes a steering proposal that cites every saying.” Change the fold setting on Steering's Memory tab, come back, and confirm both numbers follow it. A typed 3 or 2 is a FAIL.
    - A live run (`run_01K5RS7M2E8FJ3QW`) reads “Nothing is written until the seal. This run is live. When its chain seals, oxagen reads it and writes what it learned here.” and lists no memory.
    - A sealed run that wrote none (`run_01K5RF2J7M3EDC5F`) reads “This run wrote no memories. oxagen read the sealed run and found nothing another run would need, and no operator steered it.”
