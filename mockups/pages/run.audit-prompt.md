...
4. **Sections, tabs and tables.** For each item below, the build has it, with the same tab labels (and live counts where the design shows them), the same panel headings, and every table column named in the spec, in that order. Missing or renamed columns are FAILs; extra columns are noted.
   ...
   - Context — first the `steering.manifest` frame as a spine in assembly order (gate notices, stable prefix, volatile selection with ranks, then cut items dashed with one of four reasons and a why), each node linked to its item, the tally “N rendered · N cut · N tok” equal to the `steering.compiled` band, Open in Preview, and “Assembled, not delivered” on an observe tier run. Then the window as it was sent (block by block), `context.frames` (Kind · Frame · Tok · Score · Cited), Walk the window, Retrieval in numbers (Candidates scored · Admitted · Held back · Below the floor · Headroom left · Composition digest).
...
7. **States.** Force each state and compare copy and controls with the design file:
   ...
   - **error** (`state=error`): “This run could not be loaded” — `502 frame_store_unreachable`. Nothing was changed. Runs kept recording while this page was down. Frames are written by the collector on each host, not by Mission Control. Actions: Try again, Open an incident; a trace id, region and timestamp line.
