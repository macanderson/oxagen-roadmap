...
4. **Sections, tabs, and tables.** For each item below, the build has it, with the same tab labels (and live counts), the same panel headings, and every table column named in the spec, in that order. Missing or renamed columns are FAILs; extra columns are noted.
   - Tabs: Transcript (N) · Issues (N) · Governed actions (N) · Cost ($) · Policy (N) · Context (N) · Memories (N, blank when the run wrote none) · Chain and seal (live/sealed). Transcript is the default and is open above the fold. No Proof tab, no Done tab.
...
5. **Actions and dialogs.** Every button in the spec exists, opens what the spec says (`pause`, `steer`, `forkreplay`, `bisect`, `runexport`, `evidence`, `approve` / `deny` from a parked approval frame, `memory` from a row on Memories, `request-access` from denied and from the self-grade lock, `incident` from error), and each write is a governed action: it passes IAM, produces an audit event, and shows a receipt or reference. A stub must say what the product would do; a control that silently does nothing is a FAIL.
...
16. **Permissions.** Read requires `run.read`, and reading a self-grade requires `research.read`; each write (run.command (pause/resume/cancel/steer); approval.resolve; run.export; run.fork (Series A)) is gated server-side, not only hidden in the UI. Verify with a role that lacks the permission.
17. **Memories tab.** Open `/runs/<id>/memory` on three runs and compare each with the design:
    - The lead note comes first on every run and reads “**The reflector writes these after the seal.** Each lesson becomes a memory, or joins a memory that already says the same thing. At 3 sayings from 2 runs, a memory becomes a steering proposal that cites every saying.” Change the fold setting on Steering's Memory tab, come back, and confirm both numbers follow it. A typed 3 or 2 is a FAIL.
    - A live run (`run_01K5RS7M2E8FJ3QW`) reads “Nothing is written until the seal. This run is live. When its chain seals, the reflector reads it and writes what it learned here.” and lists no memory.
    - A sealed run that wrote none (`run_01K5RF2J7M3EDC5F`) reads “This run wrote no memories. The reflector read the sealed run and found nothing another run would need, and no operator steered it.”
    - A sealed run that wrote some (`run_01K5RK7C2V8BNM3X`) shows the panel “Memories from this run” with a count badge equal to its rows, the Memories tab count equal to the same number, and the table Memory · Frame · Fold · Class · In the assembler with rows in ascending frame order. Each Fold cell reads “new” with “this run started it” or “joined” with “saying N of M”, and a memory that became a proposal adds “proposed as <id>”. Each row opens the `memory` dialog.
    - No row on any run's Memories tab is a saying imported from a Markdown file. An imported saying has no run, so one listed here is a FAIL.
    - The Self-grade panel sits below, with the chip “research only”. As a person without `research.read` it shows the lock “Reading a self-grade takes `research.read`”, Request access, and How reflection works, and no rubric answers. As a person with it, the four questions each show a “self” bar and a “the record” bar, “calibration gap” appears only where self exceeds the record by two or more, and the note “**Research only.** A self-grade never enters a context frame and has no promote action.” is present. Any control that promotes a self-grade into a memory, a proposal, or steering is a FAIL.
18. **Nothing extra.** List anything on the built page that is not in the spec (tiles, tabs, columns, buttons, copy). Each is a finding; the reviewer decides whether it stays.
