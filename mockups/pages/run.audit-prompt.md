...
12. **Section 7, Evidence.** No line under the heading. Rows Outputs, Definition of done, Approvals and Record, with the fallbacks "none recorded", "none: a direct work order" and "none waiting". "Open Evidence" opens the Evidence tab.
13. **Side column.** An `aside` labelled "The work" with two panels. Changes: Pull request, Base, Checks, Release when one exists, Diff, the changed files and "Open the diff in the transcript". Outputs: the tally, "Hide reads", one node per output in frame order with its state badge and a "Frame N" chip that opens that frame, a read as a tick rather than a card, the gate with "Review the approval", and the dashed withheld node. The foot reads "In frame order."
...
15. **Run dialogs.** `pause`, `steer`, `cancelrun` and `runexport` open from the header with the titles, facts and buttons the spec lists. `pause` has no lede, note or footer text. `steer` has no note, and its Delivery field has no hints. `cancelrun` shows one warning, "Nothing is rolled back. The run seals where it stopped." `runexport` has no subtitle and no callout, and shows no replay grade. Each write is a governed action: it passes IAM, writes an audit event and shows a receipt or reference.
