...
- `belt` (wide). Title: the belt's name. Subtitle: what it is for.
  ...
  - Owner (the team, with "last changed <time> by <person>"), Providers (badges), Agents assigned (one button per agent, each opening that agent's Toolbelt tab, or, for a belt nobody carries, "unassigned" and a line saying the belt is a catalogue entry until an agent carries it), and Gates on its tools.
...
| Gates on its tools, Availability today, the unavailable warning | `beltGates()`, `beltAvailability()` over `toolGateKind()` | the gate each version meets today | The kill switch half is on each version's `gate` (`tool.version.list.ts:9-15`). Approval and mandate outcomes are not, and the belt does not exist | ❌ |
