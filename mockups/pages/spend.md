...
**Tabs**: Findings (N), Tokens, Coaching (N), By operator, By agent, By cost center, By model, By tool, Wasted spend (N), Budgets (N).
...
- **By cost center** (`spendCostCenters()`): panel "By cost center" with the subtext "Every agent’s runs this month, each charged to the agent’s cost center, or to its workspace’s when the agent names none." and **Export the chargeback statement** (opens `ccexport`). Columns: Cost center, Runs, Spend, Share of spend. Each label is mono, a deleted label carries a `deleted` badge, and spend with no label is one "No cost center" row, always last. A Total row closes the table. A note beneath: "Every run lands on exactly one row, so the rows sum to the runs’ spend. Share is a row’s spend divided by that total. The strip above also counts oxagen’s own routed work, which no cost center is charged for. The chargeback statement covers every workspace."
...
**Dialogs this page opens:** `spendexport`, `ccexport`, `budget`, `evidence`, `fix`, `incident` (error state), `request-access` (denied state).
...
| By cost center | `SPEND.byAgent` rolled up by `ccReady()` into `CC.rolled`, labels from `FIXTURES.COST_CENTERS` | the `cost_center` group key over `cost.run_totals`, the label resolved per run (ADR-142) | `cost.cost_centers`; `export_cost_center_statement` | ✅ |
...
- By cost center charges each agent's runs to the agent's label, else its workspace's, else the No cost center row. The rollup is frozen when it is read, in cents: a label changed later moves new runs, never rolled-up ones. The rows sum to the Total row, and the Total row equals the month's spend and runs over every agent (`SPEND.spend`, `SPEND.runs`).
- **Export the chargeback statement** opens `ccexport`: a Month select (September 2026 to date, August 2026, July 2026), the columns the CSV carries (`line`, `cost_center`, `runs`, `unpriced_runs`, `cost_micros`, `cost_cents`, `currency`, `basis`, `run_ids`), and a note that the file holds one line per cost center, one for spend with no cost center, and the organization total. **Export CSV** writes `cost_center_statement_exported` to Audit and toasts "Exported cost-centers-<month>.csv. In the product this downloads the file. A mockup writes nothing to disk."
...
- Read: `spend.read`
- Writes (each a governed action recorded in Audit): `budget.set`, `spend.export`, `cost_center.export` (an organization Owner, Admin or Billing member; anyone else is refused with a toast that names who holds the role), `incident.open`, a coaching note sent to an operator
