...
15. **Notifications.** Open the notifications dialog from the top bar. Each unread item is a button labelled “Mark read: <title>”, and a read item is plain text. Mark one read by click, then another by Enter and a third by Space: each goes plain, the footer count falls by one, and focus moves to the first unread item left. Mark the last unread item read and record where focus lands; it should stay in the dialog. The footer then reads “All read”, Mark all read is disabled, and the top bar's dot is gone. On a fresh load, Mark all read closes the dialog with “All notifications marked read. Audit records who read each one.” and the audit log holds a read event for each item.
16. **Nothing extra.** List anything on the built page that is not in the spec (tiles, tabs, columns, buttons, copy). Each is a finding; the reviewer decides whether it stays.
