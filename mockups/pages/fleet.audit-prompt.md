...
15. **Notifications.** Open the notifications dialog from the top bar. Each unread item is a button labelled “Mark read: <title>”, and a read item is plain text. Mark one read by click, then another by Enter and a third by Space: each goes plain, the footer count falls by one, and focus moves to the first unread item left. Mark the last unread item read: focus lands on the dialog's close button. The footer then reads “All read”, Mark all read is disabled, and the top bar's dot is gone. On a fresh load, Mark all read closes the dialog with “All notifications marked read. Audit records who read each one.” and the audit log holds one `notification_read` event for each item it marked, and one for each item marked by click or key.
