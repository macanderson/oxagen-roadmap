...
| Source, version and frame hash | `srcCell()`, `frameOf()` | `source.kind`, `source.id`, `source.version`, `hash` | None on a frame. A Steering record's own fields ship: version, checksum, commit, path (`list_records` at `steering.records.list.ts:16`, `get_record` at `steering.records.get.ts:126`) | ❌ |
