...
**Header**: eyebrow "Organization", h1 "Audit", subtext "What happened, who allowed it, under what authority, and what it cost." A mono line beneath: "Retention: control-plane events 7 years · run ledger forever · bodies 7 years (default)".
...
- **By a service principal**: count, "Terraform, CI, exports, and the audit archive".
...
- **Events**: panel "Control-plane events" with the caption "admin actions, IAM changes, repository links, plane changes, key rotations", Actor and Range selects, **Export CSV** (opens `exportevents`); a search field "Search events, receipts, actors, external ids"; filters Result (allowed, denied), Severity (critical, info, warning), Rows; columns When, Event, Actor (name and kind: human, agent, service), What, Result, Severity, Reference; a note that a call the harness reported is labeled as such and can never be shown as decided by Oxagen.
- **Incidents**: tiles Open, Critical open, Median time to resolve, Money moved without a receipt; a critical banner while one is open, with **Open the incident**; panel "Incidents" with the caption "raised by the policy engine, the gateway and the verifier", **Open an incident** (opens `incident`); columns Severity, What happened (title, kind and scope), Opened, Detected by, State; per row **Open** (opens `incidentview`). Kinds: `mandate.exception`, `taint_raised`, `receipt_modified`, `chain_break`, `hooks_removed`, `credential_probe`. A note that any external transaction on a governed connection with no receipt is a critical exception.
...
- **Keys**: panel "Keys" with the caption "one key-encryption key per organization; data-encryption keys per object and per subject", **Rotate KEK** (opens `rotatekek`); columns Key (name and id), Algorithm, Gen, Valid from, Valid to, State (active, retiring, retired, expired), What it covers; **Rotate** on the active KEK row; a note on rotation and a re-wrap progress bar.
- **Retention**: panel "Retention" with **Edit policy** (opens `retention`): Body retention, Hot window, A compacted run, Per-workspace override, Storage price (the Billing figure, included for 13 months, then $0.10 per GB-month); **Archive tiers**: Tier, Where, What it holds, Retention, Held today (Ledger, Frames, Bodies and segments, Control-plane audit); **Redaction** (before write, never after).
...
- A receipt for a call the harness reported exists too. Its authority Decision reads "observed (oxagen did not decide this call)", and the note under the receipts table says harness-reported calls say **Recorded** where calls routed through oxagen say **Decided by oxagen**. A tier is shown as recorded: `observe` renders "recorded only", `harness` renders "routed through oxagen".
