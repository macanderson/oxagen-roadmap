...
- **Exports**: a callout on what a bundle is; one card per export (the title, a ready or building badge, **Verify bundle**, **Download**; Export id, Range, Contents, Size, Created, Signature, Key ids; the verifier output once verified).
...
- **Retention**: panel "Retention" with **Edit policy** (opens `retention`): Body retention, Hot window, Replay of a compacted run, Workspace opt-down, Cold storage cost.
