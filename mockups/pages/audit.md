...
Control-plane audit events, incidents, receipt search, exports, keys and KEK rotation, and retention. The record is written by Oxagen, never by an agent.

## What is on the page

**Header**: eyebrow "Organization", h1 "Audit", and no subtext sentence; what the page answers is in the component help (`mockups/help/audit.md`, Page header). A mono line beneath the h1 states the policy: "Retention: control-plane events 7 years · run ledger forever · bodies 7 years (default)".
Actions: **Export evidence bundle** (gold; opens `newexport`).

**Summary tiles** on the Events tab (one number and one basis line each, every one a count of the last 30 days of events, `auditRecent()`, through the same actor classifier the table uses):
- **Events · 30 days**: count, "allowed and denied".
- **Denied**: count, "of N events".
- **By a service principal**: count, "of N events".
- **By an agent**: count, "of N events".

**Tabs**: Events (N, the last 30 days), Incidents (N open), Receipts (N), Exports (N), Keys, Retention.

- **Events**: panel "Control-plane events" with no caption, Actor (All actors, Humans, Agents, Services) and Range (Last 30 days, Last 7 days, Last 48 hours) selects that filter the table, **Export CSV** (opens `exportevents`); a search field "Search events, receipts, actors, external ids"; filters Result (allowed, denied), Severity (critical, info, warning), Rows; columns When, Event, Actor (name and kind: human, agent, service), What, Result, Severity, Reference; "No event matches this actor and range." when a filter matches nothing. No note sits beneath the table.
- **Incidents**: tiles Open, Critical open, Median time to resolve, Money moved without a receipt; a critical banner while one is open, with **Open the incident**; panel "Incidents" with no caption, **Open an incident** (opens `incident`); columns Severity, What happened (title, kind and scope), Opened, Detected by, State; per row **Open** (opens `incidentview`) and, while open, **Resolve** (opens `incidentclose`, not gold). Kinds: `mandate.exception`, `taint_raised`, `receipt_modified`, `chain_break`, `hooks_removed`, `credential_probe`. No note sits beneath the table: the money-exception rule is in the component help (Incidents). The tiles are computed from `INCIDENTS`: Open and Critical open count the open rows, and Median time to resolve reads the resolved ones.
- **Receipts**: panel "Receipt search" with no caption or store badge; a search input, **Search**, example chips (stripe, harness, observe, deny, an agent key, an external effect id), **clear**; columns Receipt, When, Agent and operator (the agent key led by its harness mark), Tool version, Decision, Amount, External effect, Tier; a row opens `receipt`; an empty search shows "No receipt matches" with "Search by agent key, tool version, external effect id, call digest or approver." and **Clear the search**; a footer "N of N receipts shown". The nav count on Audit is the open critical incidents.
- **Exports**: one card per export, with no callout above them (what a bundle is lives in the component help, Export card): the title, a Ready, Building or Expired badge, **Verify bundle**, **Download**; Export id, Range, Contents, Size, Created, Signature, Key ids; the verifier output once verified); panel **Verifier** (the offline `oxagen-verify` transcript, with no badge or note); panel **Outbound events** (the subscribed event kinds, Dead-letter view, Delivery; no paragraph above the kinds).
- **Keys**: panel "Keys" with no caption, **Rotate KEK** (opens `rotatekek`); columns Key (name and id), Algorithm, Gen, Valid from, Valid to, State (active, retiring, retired, expired), What it covers; **Rotate** on the active KEK row; a re-wrap progress bar. The rotation rule is in the component help (Keys), not in a note.
- **Retention**: panel "Retention" with no caption and **Edit policy** (opens `retention`): Body retention ("7 years from the seal"), Hot window ("13 months, then frame rows are compacted into the segment"), Per-workspace override, Storage price (the Billing figure, included for 13 months, then $0.10 per GB-month); **Archive tiers**: Tier, Where, What it holds, Retention, Held today (Ledger, Frames, Bodies and segments, Control-plane audit); **Redaction**: Detectors ("run before each body is written") and Archive edit path ("none").
...
- A receipt for a call the harness reported exists too. Its authority Decision reads "observed (oxagen did not decide this call)". A tier is shown as recorded, through `tierBadge()`, in the table and in the receipt dialog alike: `observe`, `harness`, `gateway` or `contained`, never a stronger word.
...
- Every badge that describes trust (enforcement tier, replay grade, attestation, cost basis) shows the recorded value and nothing stronger; a call the harness reported is labeled as such and is never rendered as decided by oxagen.
