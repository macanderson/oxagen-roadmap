...
- **Keys**: panel "Keys", store badge "kms + postgres", **Rotate KEK** (opens `rotatekek`); columns Key (name and id), Algorithm, Gen, Valid from, Valid to, State (active, retiring, retired, expired), What it covers; **Rotate** on the active KEK row; a note on rotation and a re-wrap progress bar.
- **Retention**: panel "Retention" with **Edit policy** (opens `retention`): Body retention, Hot window, Replay of a compacted run, Workspace opt-down, Storage price (the Billing figure, included for 13 months, then $0.10 per GB-month); **Archive tiers**: Tier, Where, What it holds, Retention, Held today (Ledger, Frames, Bodies and segments, Control-plane audit); **Redaction** (before write, never after).
