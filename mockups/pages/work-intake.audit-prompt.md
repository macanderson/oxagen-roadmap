...
4. **Nothing under the cards.** Trackers ends with the cards. A How imports work panel, or any list of rules about events, reconcile, fields, writes and creates, on the page is a FAIL: those rules live in `mockups/help/work-intake.md`, Trackers. The build must still keep them: verify on the server that a write outside the card’s switches, an edit to a subject or description, a reply to a requester, and a rename or delete of a provider value are all refused.
