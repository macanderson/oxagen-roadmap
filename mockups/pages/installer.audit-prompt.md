...
8. **Trust language.** The Connected screen says `harness` and “countersigned on ingest”, and nothing stronger. The Download screen claims only what the package does (user account, no sudo, no kernel extension, frames as the only egress). Nothing on the page mentions a witness, a proof, a verdict, a definition of done, a trust or spend score, or a per-run price.
