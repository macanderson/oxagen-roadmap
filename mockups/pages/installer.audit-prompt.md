...
2. The design, rendered: the `installer` stories in Storybook (`npm run storybook`), one per state (loaded), desktop and mobile; or `mockups/missioncontrol.html?product=1&state=<state>&mobile=<0|1>#<route>` in a browser or with Playwright.
...
7. **States.** Force each state and compare copy and controls with the design file:
   - only the loaded state exists for this page
...
9. **Mobile.** At 390 × 844 with a touch pointer: the card fills the width with 16 px gutters, buttons are full width and ≥ 44 px, inputs are 16 px, code inputs use the numeric keypad. Compare against `pages/installer-loaded-mobile.html`.
...
11. **Permissions.** Read requires `installer token`; each write (enrollment (by the package)) is gated server-side, not only hidden in the UI. Verify with a role that lacks the permission.
