...
2. The design, rendered: the `accept-invitation` stories in Storybook (`npm run storybook`), one per state (loaded, loading, error, access denied), desktop and mobile; or `mockups/missioncontrol.html?product=1&state=<state>&mobile=<0|1>#<route>` in a browser or with Playwright.
...
7. **States.** Force each state and compare copy and controls with the design file:
   ...
   - **error** (`state=error`): “This invitation has already been accepted. It was used on <date> at <time>. Log in instead.”
   - **access denied** (`state=denied`): Full-card state: “This invitation is for a different account” — sent to X, you are logged in as Y. Action: Log in as someone else.
