...
2. **Header.** Eyebrow "Organization", h1 "Billing", no subtext (fail if the build adds a sentence describing the page; that text is component help, `mockups/help/billing.md`). One action with the same label: Change plan. Exactly one gold (primary) action on the screen.
...
4. **Sections, tabs and tables.** For each item below, the build has it, with the same panel headings and every table column named in the spec, in that order. Missing or renamed columns are FAILs; extra columns are noted.
   ...
   - Meters: Meter, This period, Note; the rows lead with Governed actions, then Sealed runs with at least one model call, Retained evidence, Runs oxagen halted before any model call, Runs of the in-app agent; no note beneath the table.
   - Invoices: Invoice, Period, Governed actions, Amount, Status, Paid, Open in Stripe.
   - Price list: the seven rows of the spec (Free; Governed actions, blocks of 10,000; Negotiated agreement; Invoice billing; Evidence retention; Tokens oxagen buys for you; Enterprise, annual), each description in sentence case, and no footer.
...
7. **States.** Force each state and compare copy and controls with the design file:
   ...
   - **error** (`state=error`): "Billing could not be loaded", `502 stripe_unreachable`. Nothing was changed. Runs kept recording while this page was down. Actions: Try again, Open an incident; a trace id, region and timestamp line.
