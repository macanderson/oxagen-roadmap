...
7. **States.** Force each state and compare copy and controls with the design file:
   - **empty** (`state=empty`): "Nothing billable yet"; you pay per governed action, a call Oxagen decided, delivered and recorded; the free tier has every governance feature on, an included monthly allowance, thirty days of evidence and three seats. Action: Back to Fleet.
   ...
   - **access denied** (`state=denied`): "You cannot see billing"; the roles the signed-in person holds do not include `org.billing` (plan and invoices are readable only by a finance role); an owner can grant it and the grant is a governed action in the audit record. Actions: Request access, Back to Fleet. Below: Signed in as, Needed, Decided by.
