...
2. **Header.** Eyebrow the workspace name, h1 "Repositories", the one-sentence subtext. Action: Add .oxagen/, gold on this tab. The gold moves off the header on Working copies and on a selected pull request that can merge, and nowhere else. Two golds on one screen is a FAIL.
