...
4. **Sections, tabs, and tables.** For each item below, the build has it, with the same tab labels (and live counts), the same panel headings, and every table column named in the spec, in that order. Missing or renamed columns are FAILs; extra columns are noted.
   ...
   - Policy: Policy versions (Version · State · Author · When · Rules · Tests · What changed; a row carries Open and then Edit, Activate or Restore by its state); the activation note; Conditions a rule may test; the Sequence rule.
...
5. **Actions and dialogs.** Every button in the spec exists, opens what the spec says (`import`, `wz`, `connection`, `tool`, `schema`, `server`, `oauth`, `policy`, `switch`, `request-access` from denied, `incident` from error), and each write is a governed action: it passes IAM, produces an audit event, and shows a receipt or reference. A stub must say what the product would do; a control that silently does nothing is a FAIL.
...
14. **Permissions.** Read requires `tools.read`; each write (tools.import; tools.schema.approve; connection.add; connection.authorize; connection.revoke; policy.edit / policy.activate; switch.flip) is gated server-side, not only hidden in the UI. Verify with a role that lacks the permission.
