...
4. **Sections, tabs, and tables.** For each item below, the build has it, with the same tab labels (and live counts), the same panel headings, and every table column named in the spec, in that order. Missing or renamed columns are FAILs; extra columns are noted.
   ...
   - Connections: Connections (Connection · Kind · Owner · Servers · Downscope · Grants 30d · Reviewed · Next review · Status; Add a connection); Credential grants log (Grant · Tool version · Agent and run · Connection · Scope · TTL · State); Broker selection (Provider capability · What the broker mints · Here); Financial connections carry extra rules (Named human owner · Mandate per agent · Full-scope keys).
   ...
   - Policy: Policy versions (Version · State · Author · When · Rules · Tests · What changed; Edit); the activation note; Conditions available to policy; the Sequence rule.
   - Kill switches: Deny is available at every level; Class switches and Scoped switches; each switch with its toggle, Blast radius · Takes effect · Flipped by · Reason.
