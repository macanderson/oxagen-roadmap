...
4. **How a workflow runs.** The five facts, verbatim, with no note under them. The rule that every agent in a workflow is one you operate is in `mockups/help/work-workflows.md`, How a workflow runs. The panel stays pending the lead’s decision on whether a build drops it.
...
13. **Mobile.** At 390 × 844: the thumb bar (Work, Agents, Tools, Spend, More); **New workflow** under the h1; the panels stack, the list first; the table is labelled cards; the dialog and the builder are bottom sheets with full-width footer buttons and a stacked stage chain; the file preview scrolls inside its block and the page never scrolls sideways; touch targets at least 44 px; inputs 16 px.
