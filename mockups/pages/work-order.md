...
- **Runs**: “1 run. Each is a child record of this work order.” Columns: Run (the id as a link and the run’s title under it) · Stage (the role, with “returned” beside a stage run that returned the work) · Agent (harness mark, avatar and agent key) · Status (dot and word) · Tier · Cost (USD, the basis under it) · Started. A row opens the run. With none: “No run yet. The runtime starts the first one with `oxagen work start wo_01K6TF1169Q`.”
