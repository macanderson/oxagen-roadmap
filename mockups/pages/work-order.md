...
- **Handoffs**: every stage run in order, as “<role>” or “<role> sent the work back”, its run id, and its note (“Returned: the backwards pair still passes. Item 2 is not met.”), or “running” while that run is live. No note sits under the list. The explanation is in `mockups/help/work-order.md`, Handoffs. A direct work order has one run and no handoff. The mockup still draws the panel there, with the run and “running” even once the run is sealed, and a build leaves the panel out. A stage that needed two stages shows the note from each, with its stage and run.
