...
4. **Sections and tables.** The build has each item below with the same headings and every column named, in that order.
   ...
   - Context PRs: Context PRs with Pull request · Branch · Opened by · State; the selected pull request with its file, body, the six named checks with per-check result text, the merge bar, Merge pull request, Close without merging on an operator's, Merge effects or promotion_event.
