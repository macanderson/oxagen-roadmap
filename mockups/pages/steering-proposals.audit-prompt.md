...
4. **Sections and tables.** The build has each item below with the same headings and every column named, in that order.
   ...
   - Context PRs: Open and recent Context PRs with Pull request · Branch · Opened by · State; the selected pull request with its file, body, the six named checks with per-check result text, the merge bar, Merge pull request, Close without merging on an operator's, Merge effects or promotion_event.
...
6. **Actions.** Review opens the proposal. Open a Context PR opens `ctxpr` with Concern, Kind, Scope, Constraint effect, and Supporting evidence. Merge is blocked until every check reports, re-runs every predicate, and publishes exactly once. Close without merging leaves nothing behind. A merged pull request cannot be closed. Write a context record ends on this tab with the new pull request selected.
