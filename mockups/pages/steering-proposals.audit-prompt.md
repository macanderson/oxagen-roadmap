...
3. **The list.** The panel "Proposals" with no caption, and the card list controls (Sort, Rows, pager). Each card: the kind badge, the force, the state badge, the checks badge where a pull request is open, Review, the statement as the headline, and the meta line (from, support, id, scope, lineage). Only this workspace's proposals are listed.
4. **States and checks.** Each state badge maps a contract status (proposed, pr_open, checks_running, checks_passed, checks_failed, merged, rejected) to the design's words. A checks badge counts out of six for every Steering record pull request. No older name for a pull request appears in the view's copy.
5. **Review.** Review shows one proposal and All proposals returns to the list. The three tiles (Supporting runs, Distinct agents, and Confidence or, for a person's proposal, Checks) with their sub-lines. Proposed record with "steers nothing yet". Promoter evidence. Supporting runs with Run, Frame, Outcome and Record, in that order. The Pull request panel with Target, Branch, File, Governance and the one action the spec gives for the proposal's state. If it publishes with Reaches, As, Costs and Baseline.
...
8. **Open a pull request.** Offered only while the proposal has none, and gold then. `ctxpr` shows the callout when one is open, the fields with no hints beyond "One concern per pull request.", no note, and Open the pull request (or Go to the open one). Opening calls `open_steering_pr`, pushes `context/<lineage>` with one record file, queues the six checks and lands on Pull requests with it selected. The Kind and Scope options carry no dash-joined labels. A constraint effect is offered for a constraint only.
...
10. **Data sources.** For each row of the spec's data-source table, find the adapter or query that feeds it. ✅ rows are wired to the named contract (`list_proposals`, `get_steering_pr`, `open_steering_pr`). 🟡 rows show the counts that derive from the support and render "not recorded" for the measure. ❌ rows render "not recorded" or are absent. A fixture reaching production is a FAIL.
...
17. **Permissions.** The read is refused server-side without the Steering read. `open_steering_pr`, `propose_record` and `dismiss_proposal` are gated server-side, not only hidden. Verify with a role that lacks them.
