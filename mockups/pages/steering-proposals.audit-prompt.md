...
2. The design, rendered: the stories `Oxagen / Steering / Proposals` in Storybook (`npm run storybook`): Loaded and Loaded · mobile. Or `mockups/missioncontrol.html?product=1&state=loaded&mobile=<0|1>#/a-intel/core-platform/steering/proposals`; press Review on each proposal. The scenario `#/a-intel/core-platform/scenarios/learned-approved-changed/2` shows the promoter's proposal before its pull request opens.
...
15. **Accessibility.** Tabs use `role=tablist` and `role=tab` with `aria-selected`; the two view buttons use `aria-pressed`; the dialog is `role=dialog`, `aria-modal`, with labelled fields and a labelled close; state is a dot and a word; focus is visible; the view is operable by keyboard end to end.
16. **Permissions.** The read is refused server-side without the Steering read. `open_context_pr`, `propose_record` and `dismiss_proposal` are gated server-side, not only hidden. Verify with a role that lacks them.
17. **Nothing extra.** List anything on the built view that is not in the spec. A Dismiss control is a finding for the reviewer: the capability ships and the design has none.
