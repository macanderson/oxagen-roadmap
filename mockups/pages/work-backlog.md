...
Component help for this page lives in `mockups/help/work-backlog.md`, one section per page part and one for the work order dialog.

**Header.** Eyebrow: the workspace name (“Core platform”). h1 “Work”. The header has no subtext. What Work is for is in `mockups/help/work-backlog.md`, Header. On the Backlog tab the actions are **Intake** (plain; opens the Intake dialog on Trackers) and **Send to an agent…** (gold, with the agents icon and a ▾). The gold button is disabled with the title “Select one or more ready work items” until a work item is selected. It opens the send menu.
...
**Tiles**, four across, one number and one caption each. The mockup outlines the row as future-only.
...
No note sits under the table. How a work item becomes ready and how it reaches an agent is in `mockups/help/work-backlog.md`, Backlog.

### The send menu

**Send to an agent…** opens a menu under the button (`role=menu`, `aria-label` “Send to”). Header “Send 2 work items to”, with no sub. The rule on which agents are listed is in `mockups/help/work-backlog.md`, Send menu. A search field (“Search agents and workflows”) narrows both groups.
...
- **Workflows**: every published workflow whose stages are all agents you operate (`wfSendable()`), each with its stages’ harness marks in order and “4 stages then you”. A workflow still in a pull request is not listed, and neither is one that names an agent somebody else operates. With no match: “No published workflow matches.”
...
|---|---|---|
| The tile row | “work items and work orders” | `not recorded` in each tile. No `/work` route exists in `apps/app` today; the workspace root renders Fleet (`apps/app/src/app/[org]/[ws]/(fleet)/page.tsx`), whose tiles count runs, not work |
