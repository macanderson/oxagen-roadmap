...
| | |
|---|---|
| Route | `#/a-intel/core-platform/tools/policy`. `#/…/steering/gates`, `#/…/steering/policy`, `#/…/steering/settings` and `#/…/steering/freshness` land here: a 308 in the app, and the mockup rewrites the hash with `history.replaceState`. `fleet-operations-routes.md` also lands `#/…/tools/autoapprovals` here in place |
| Scope | workspace |
...
| Design | `mockups/src/engine.js` → `pTools()`, the `t==="policy"` branch, with `DLG_EXT.policynew`, `DLG_EXT.policyactivate`, `DLG_EXT.policydiscard`, `DLG_EXT.policyrestore`, `policyDraft()`, `polNewDraft()`, `policyNextV()`, `polGate()`, `policyActivate()`, `policyDiscard()` and `policyRestore()`. Counts come from `polSaved()`, `polParse()` and `polTestsLabel()` over `FIXTURES.POLICY_RULES`. One version opens on its own page (`tools-policy-version.md`). The gate notices are drawn on Steering › Sources by `steeringSources()` and `gateHome()` in `mockups/src/wedge.js`. Built into `mockups/missioncontrol.html` by `tools/build-mockup.mjs` |
...
**Policy versions** panel. Title "Policy versions", with no caption. The panel header carries **Draft new version** (plain, opens `policynew`).

- **List controls** (`ltTable()`): "Search this list", the filters "Any state" and "Any author", **Rows** (10 by default) and the pager (`1–9 of 9`).
- **Table**, columns in order: Version · State · Author · When · Rules · Tests · What changed, then an unlabelled actions column.
  - *Version*: the version id in mono (`pol_v41`), a link to the version page.
  ...
  - *When*: a timestamp.
  - *Rules*: how many rules the version's saved source holds.
  - *Tests*: "<pass> / <total> pass" from the last run against the saved source ("42 / 42 pass"), a failed badge when a test fails, or "not run yet" when no run matches the saved rules, as on a fresh draft.
  - *What changed*: one sentence.
  - *Actions*: **Open** (the version page), then by state. Active: **Draft new version** (opens `policynew` based on it). Draft: **Edit** (the version page), **Activate** (gold, `policyactivate`) and **Discard** (danger, `policydiscard`). Superseded: **Restore** (`policyrestore`).
  - The demo holds nine versions: `pol_v41` active (Priya Natarajan, 2026-09-08 16:22, 38 rules, "42 / 42 pass", "Raises any tainted write to approval. Adds AWS Billing to the financial class."), `pol_v42` a draft (39 rules, "44 / 44 pass", "Would require approval for every irreversible tool call in core-platform."), and seven superseded, `pol_v40` and `pol_v39` down to `pol_v34`. The rows keep the fixture's order, so the draft sits first.
- No note under the table. How a version is activated, and why a draft is the only version you can edit or discard, are in the component help (`mockups/help/tools-policy.md`, Policy versions).

**Where a version lives** panel, a key-value list of facts. Each row's explanation is in the component help (`mockups/help/tools-policy.md`, Where a version lives):

- *Store*: "oxagen, one stored version with its rules and tests"
- *In regulated mode*: ".oxagen/policy/ in a-intel/platform, changed by pull request" (the workspace's main repository)
- *Compiled from*: "The rules on this page, the enforcement grants on each agent, and the role grants on each operator"
- *Who reads it*: "The gateway, on every tool call"
- *What it writes*: "One policy.decision frame per call"

**Conditions a rule may test** (`data-help="conditions"`): an eyebrow over 18 chips: tool version, risk, side effect, egress, financial class, amount by path, counterparty, repository, path prefix, recipient domain, taint and its sources, time window, rate, sequence, operator role, enforcement tier, budget position, mandate position.

**Sequence rule** (`data-help="sequence-rule"`): an eyebrow, then the plain sentence "This rule denies a payment unless the same run already priced it.", then the rule source as the active version stores it: the comment `// A payment is denied unless the same run already priced it.`, `@id("rg_0044")`, and a `forbid` on `stripe__create_payment` with `unless { context.run.prior_calls.contains("stripe__list_prices") }`. The tab names no policy language.

**Dialogs this tab opens.**

- `policynew`, "Draft a policy version", subtitle "A draft decides nothing until an approver activates it". Based on: a select of every version that is not a draft, each option reading the version and its state (`pol_v41 (active)`), defaulting to the active version or the one the dialog was opened from, with the hint "The draft opens as a copy of this version’s rules and tests." What changes: one sentence, with the placeholder "Raises any egress call to approval" and the hint "One sentence." No note. Footer: Cancel, **Create the draft** (gold). With no sentence it refuses: "Say what the version changes before you create it." Otherwise the next version (`pol_v43`) lands at the top of the list as a draft holding a copy of the base's rules and tests, with tests "not run yet". Its version page opens on Rules, and the toast reads "Drafted pol_v43 from pol_v41."

- `policyactivate`. It lists four checks, each ready or blocked: the draft is saved, the saved rules have no problems, the tests ran against the saved rules, and every test passes ("44 tests pass."). With any check blocked: "pol_v42 is not ready", the checks, the note "Fix what is blocked on the version page, then activate it.", and Close with **Open pol_v42** (gold). With every check ready: "Activate pol_v42?". Note: "pol_v41 is superseded. From the next tool call, every policy decision names pol_v42." The checks, then Rules and What changes. Reason: a required text field with the hint "Recorded with the activation." Warning: "This is a governed action. It records your name." Footer: Cancel, **Activate it** (gold). With no reason it refuses: "Say why you are activating it." Activating supersedes the active version, records the reason, bumps the kill-switch generation, and reads "pol_v42 is active. Kill-switch generation is now 119." On a version that is not a draft: "pol_v41 cannot be activated".
- `policydiscard`. On a draft: "Discard pol_v42?", a warning that quotes the draft's sentence and says it goes with its rules and its tests. No note. Footer: **Keep it**, **Discard** (danger: "Discarded pol_v42."). From the version page, discarding returns to this tab. On any other state it refuses: "pol_v40 cannot be discarded", "pol_v40 is superseded, so it is kept. Supersede it with a new version instead.", with Close and **Draft new version** (gold).
- `policyrestore`, "Restore pol_v40?". Note: "This copies pol_v40’s rules and tests into a new draft above pol_v41." A warning quotes what the restored version changed and says the restore undoes it. Footer: Cancel, **Draft the restore** (gold). The new draft reads "restores pol_v40", its version page opens, and the toast reads "Drafted pol_v43 from pol_v40." Why a restore never moves the pointer back is in the component help (`mockups/help/tools-policy.md`, Policy restore).
...
| Draft, edit, activate, discard and restore | `policyDraft()`, `polSave()`, `policyActivate()`, `policyDiscard()`, `policyRestore()` | `policy.draft`, `policy.edit`, `policy.activate`, `policy.discard` | None, #3920 | ❌ |
| The rules the gateway evaluates on every call | `FIXTURES.POLICY_RULES`, the rule source of each version | the active version | The decision-rules gate runs after IAM and entitlement and before the handler (`packages/rules/src/gate.ts:1-10`). A rule compares facts reached by path with a fixed operator set (`packages/rules/src/types.ts:50-73`), and its effect is `allow`, `deny` or `require_approval` (`:86`) | 🟡 one rule set, no versions |
...
The renderer puts no `data-future` mark on this tab, and the catalog gives it no future-only story. Every policy version row, every version dialog, the tab count and each gate notice's frame have no contract today all the same (#3920). A build keeps the panel's heading, its caption and Draft new version, and says the versions are not recorded in place of the rows, as `apps/app/src/features/tools/policy.tsx` does. Where a version lives, the conditions and the sequence rule describe that store as it is specified, and a build says so in one line.
...
- Every decision cites the version that made it, so a version that has decided anything is kept. A draft is the only version that edits or discards. Editing or discarding an active or superseded version refuses and offers Draft new version.
- A draft decides nothing until it is activated. Activation is a governed action with approval, and a pull request in regulated mode. Activating supersedes the active version and bumps the kill-switch generation, so every live run token is re-checked at its next call.
...
- **empty**: "No provider is registered", with **Add provider** (gold) and **Add a connection**. The Policy tab has no empty panel of its own.
...
- An address for one version, so a notice's source link opens `pol_v41` rather than the tab. The design has one, `#/…/tools/policy/<version>`, and a gate notice links to `?rule=<id>` on it.
...
- Every enforcement claim states the tier. A gate refuses a call for calls routed through Oxagen. On the `harness` tier the hook refuses a harness-native call, reported by the harness and fail-open. On `observe` nothing refuses.
...
- Plain nouns: a heading names the thing, a caption states one fact, and no label carries a comma, a mid-dot, or a not/never contrast. Subtext under a heading is one sentence or nothing. The Policy tab names no policy language.
