...
**Component help.** With component help on, every panel on this tab and the instruments grid (`data-help="instruments"`) carry a `?` that opens their section in `mockups/help/run-cost.md`. The explanations this tab used to print live there.

**Model fit**, first. Heading "Model fit" and the badge "Generated estimate". The reading is computed from the record, not written by a model. Two cards.

- The model card. A fit reading: the title "Model fits this work" and one line ("Opus 5 fits this work. No change suggested."). An over or under reading: the title "Heavier model than needed" or "Lighter model than needed", its badge, what the record showed and what the run would have cost a rung down or up, and "Move this agent to <model>".
- The effort card. A reading: the title "Less effort than needed" (or "More effort than needed"), its badge, the line ("This run needed 1 extra prompt at effort medium. At effort high the model reasons before it acts, which costs less than a correction.") and "Set effort to high". A fit reading, "Effort fits this run", states the effort and why it fits. Where the effort was not read, the title "Effort not captured" and the reason it was not captured, with no action. Which tiers capture the effort is in the component help (`mockups/help/run-cost.md`, Model fit).
- The footer note is the reading alone: "2 prompts · 7 turns · 36,711 output tokens, 34% of them reasoning · no tool call failed". That a reading changes nothing until its pull request merges is in the component help (`mockups/help/run-cost.md`, Model fit).

The action opens `fitchange`: the title ("Set effort to high"), the argument, the facts Agent, File (`.oxagen/agents/release-manager.toml`), Today, Proposed and "Effect per run" ("$0.56 more"), the note "The first run after this pull request merges uses the change.", and "Cancel" and "Open the pull request" (gold). The toast reads "Opened <pr>.". That sealed runs keep the settings they ran with is in the component help (`mockups/help/run-cost.md`, Change the model or effort). Opening it writes a pull request against the agent definition and never changes a run.
...
| Turns and steps | "16 of 186 frames shown" | 7 turns · 41 steps · 186 frames | no definition line (what a step and a frame are is in the component help, Instruments); steps per turn split into model and tool calls; "1.6 tools per batch · 0.86 tool calls per model call" |
...
| Productive ratio | "steps that advanced the task" | 71%, "+9 pts vs 30-day 62%" | a bar of steps that advanced against steps that did not, with this agent's 30-day ratio marked; "12 steps did not advance the task: 7 retries, 3 re-reads, 2 waits." |

Each column and bar segment shows its figures on hover and on focus.

**Spend by area.** Heading "Spend by area" and "$4.13 · Observed by gateway". Seven meters, each with its cost and tokens: Initial prompt, Follow-up prompts, Context retrievals, Tool definitions, Tool calls, System prompt and Model output. Then the eyebrow "Most expensive tools", up to eight tools with their calls and cost ("github__get_commit 3 calls · $0.19"). No note. How the cost splits across the areas is in the component help (`mockups/help/run-cost.md`, Spend by area).

**Tool calls.** Heading "Tool calls" and "19 calls · 12 batches · 2 families". Three parts:

- "Calls by family": Family · Calls · a bar · Share · Wall clock · Failed. Each family row names its family and its count of distinct tools. No note. What a family is is in the component help (`mockups/help/run-cost.md`, Tool calls).
- "Tools per batch": a histogram of batch widths, then Batches ("12 · 6 ran more than one tool"), Widest batch ("3 tools at once") and Time saved by batching ("7s · 20s in parallel instead of 27s one at a time").
- "Speculative prefetch": "9 of 12 used" with a bar of used against discarded, then Eligible ("16 read-only calls"), Hit rate ("75%") and Time saved by prefetch ("4s"). There is no Billed row and no note. How prefetch works, that writes are never prefetched, and that discarded reads are billed are in the component help (`mockups/help/run-cost.md`, Tool calls).

**Waterfall.** Heading "Waterfall", the badges "4 findings" and "7 turns · $4.13 · Observed by gateway". The chart: one bar per turn at its cost on the left scale, labelled with the cost, the turn ("T1") and its cache hit; a dashed line of the cost so far ending at "$4.13 total"; a diamond for each Spend finding that names this run, on the turn its pattern points at. The legend: "turn cost", "turn carrying a finding", "cost so far". No caption under the legend. How to read the bars, the line and the diamonds is in the component help (`mockups/help/run-cost.md`, Waterfall). The table: Turn · Steps · Frames · Cache hit · Cost · Running total · Pinned, with each pinned finding as a badge, and a total row ("total 41 186 83% $4.13 of $4.13 recorded"). Under it, when the frames in view carry no turn: "This run’s frames do not record their turn, so frames per turn are estimated from steps."

**Spend by token class** beside **Prompt composition**.

- Spend by token class: "768,981 tokens", then Class · Tokens · Cost · Share over `input_uncached`, `cache_read`, `cache_write_5m`, `output` and `reasoning`, and a total row (768,981, $4.13, 100.0%). A class with no tokens shows a dash for its cost. The rollup records a sixth class, `cache_write_1h`, which a build lists after `cache_write_5m`. No note. How each class is priced is in the component help (`mockups/help/run-cost.md`, Spend by token class).
- Prompt composition: "33,285 tokens per request, on average", then meters for Conversation, Context frames, Tool definitions, Steering and System, then Effective input price ("$1.88 per million across all input classes"), Cache write cost share, Basis (the basis chip alone, "Observed by gateway", with the key in its title; how the basis was counted is in the component help, Prompt composition) and Productive ratio ("71% · 29 of 41 steps advanced the task").
...
| Most expensive tools | `runAreas()` | per-tool cost | `byTool` names and call counts only (`run.cost.ts:78-85`); a tool call carries no price | 🟡 |
...
- One derivation feeds the stat row, the instruments, Spend by area, Tool calls, the waterfall and the token classes, so no two panels can disagree. The Tokens tile equals the stat row's Tokens and the total row of Spend by token class. The Turns and steps tile's steps and frames equal the waterfall's total row.
...
- The model reading's "small run" test reads the run's own steps (`R.steps`). This change fixed it: it read a `steps` field that `runMetrics()` does not return, so only the turn count decided.
...
- The tab reads the record. The Model fit panel is a reading, labelled "Generated estimate", and argues for a pull request. It is never a verdict, a score or a model-written account of why the run behaved as it did.
