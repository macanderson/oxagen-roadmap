...
7. **States.** Force each state and compare copy and controls with the design file:
   ...
   - **error** (`state=error`): “This agent could not be loaded” — `503 iam_principals_unavailable`. Nothing was changed. Runs kept recording while this page was down — frames are written by the gateway, not by Mission Control. Actions: Try again, Open an incident; a trace id, region and timestamp line.
