...
4. **Sections, tabs, and tables.** For each item below, the build has it, with the same tab labels (and live counts), the same panel headings, and every table column named in the spec, in that order. Missing or renamed columns are FAILs; extra columns are noted.
   ...
   - Toolbelt: Belt computation (Searchable belt / Full belt), Model view, Belt search (five queries, Run), Per-tool decision rules (By category / Flat, What the categories mean): Tool · Category · Decision · Hazard · Egress · Financial · Schema digest.
   - Mandates: Mandates held (Mandate · Effect · Per call · Per period · Remaining · Expires · Status), or No mandate with Request a mandate.
   - Budgets: Budgets (the lead sentence; Mode · On a breach · Spend 30d · Tokens 30d · Productive ratio · Runs 30d · Cost per run; Set budget), Token accounting (Class · Tokens 30d · Rate · Cost, derived from the agent, never typed), Findings (Evidence, Fix, Open on Spend). No proven spend row.
   - Runs: Run · Status · Tokens · Cost · Frames · Started; Open the audit record. No Verdict column.
   - Enrollment: Host (Device · Device key · Collector · Hook binary · Hooks written · Model proxy · Oxagen MCP server · Settings · Tier earned · Last checkpoint; Run a smoke session, Unenroll, and the hooks_removed note); an agent with no host offers Wrap it and Show the CLI path.
   ...
   - Definition in git: Identity · Model and budget · Tools · Instructions · Harness · Source (Repository · `definition_digest` · At commit · Generated beside it; Open the source) and the note that deleting an agent is a pull request. Save changes and Discard appear while the form is dirty, Open a pull request while a branch is pending.
