...
2. **Header.** Eyebrow "Agent". The h1 is the agent card: avatar, agent key in mono, harness label, and no score. One row of badges: the status as a dot and a word, the tier as recorded, "operator <name>". Then the description. Actions in this order: Edit avatar, Rotate credential, Suspend (danger), Deregister (danger). No action in the header is gold. No replay badge and no replay grade anywhere on the page.
3. **Tab bar.** Eight tabs in this order: Overview, Identity, Steering, Toolbelt, Runtime, Permissions, Activity, Source. `role=tablist`, each `role=tab` with `aria-selected`. Each is a path segment, so a tab is linkable and back moves between tabs. Counts: Toolbelt equals the tools on the belt, Permissions the mandates held, Activity the tamper incidents recorded; a zero draws nothing. There is no Definition in git tab.
...
6. **Coaching.** The badge reads "<N> from the token record". At most three items, ordered by the money at stake, each with its title, its signal line, "$<N> a month at stake" where there is a figure, and one action that opens where the change is made. The empty copy is verbatim: "Nothing to change. Every share is inside the workspace norm and the cache holds." The foot links to Spend › Optimization at its canonical path, not an old one.
7. **Composition.** Subtext "One principal, and a reference to every other object it uses." The health badge. Rows Identity, Steering, Toolbelt, Runtime, Owner and Permissions with the values and sub-lines the spec gives; every row but Owner has Open, which opens the tab that owns it. Each named toolbelt links into Tools › Toolbelts. No row restates a fact its registry owns.
...
9. **Definition in git.** Subtext "Identity is in Postgres; the definition is a file, and the file is the source of truth." Open the file opens the Source tab at `/source`. Rows Path, Repo, Commit, `definition_digest` and Generated beside it.
10. **Dialogs.** Edit avatar opens the avatar editor. Rotate credential opens "Rotate the credential on <key>?" with Cancel and Rotate it. Suspend opens "Suspend <key>?" with Cancel and Suspend it. Deregister opens "Deregister agent" with the confirmation checkbox. Each write reaches its contract (`update_agent_def`, `rotate_agent_credential`, `suspend_agent`, `retire_agent`), passes IAM, writes an audit event and shows a receipt or reference. A control that silently does nothing is a FAIL.
...
12. **Future-only fields.** The six input composition bars, Coaching, the Composition Steering row, the named toolbelts and the host kind render as not recorded until their contracts ship. Any of them drawn with invented values is a FAIL. Output, Reasoning, the cache rate and the tool count may render from their contracts.
...
14. **States.** Force each state and compare copy and controls:
    - **empty**: "This agent has never run", the sentence about no frame and the belt computed at run start, and Back to Work.
...
15. **Trust language.** Every tier, health word and cost basis shows the recorded value. `observe`, `harness`, `gateway` and `contained` appear only where recorded. A client-attested figure says so in its Basis line. Nothing says proven, verdict, score or percentile.
