...
9. **Figures reconcile.** Recompute and compare: every tab count equals the rows of its table; each human role's people count under Held by on the Roles tab equals the People rows left when you filter Role to that role; the Model routes Total equals the sum of its rows and the Cap's "used" figure; the Model key Difference equals OpenRouter reports minus Our credit ledger. A figure typed by hand is a FAIL.
