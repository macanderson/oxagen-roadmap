...
2. **Header.** Eyebrow "Organization", h1 "<organization name>", subtext "People, roles, invitations, workspaces, cost centers, model funding and routes, and API keys." Actions present, in order, with the same labels: Edit avatar, Invite, Create a workspace. Exactly one gold (primary) action on the screen.
