...
2. **Header.** Eyebrow "Organization", h1 "<organization name>", no subtext under the h1 (fail if the build adds a sentence describing the tabs; that text is component help, `mockups/help/organization.md`, Page header). Actions present, in order, with the same labels: Edit avatar, Invite, Create a workspace. Exactly one gold (primary) action on the screen.
...
8. **Trust language.** Every tier, attestation and cost basis on the page shows the recorded value. The ladder is `observe`, `harness`, `gateway`, `contained`, each rendered only as recorded; the routes' Total carries "reported by harness" (`client_attested`); the data plane's Gateway row names where the gateway runs and claims nothing about how metering is observed. Money always carries its basis. Nothing on the page says proven, verdict, score, witness or definition of done.
