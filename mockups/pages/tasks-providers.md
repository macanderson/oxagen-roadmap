...
- **Resolutions**, "A closed task carries one resolution.", **Add resolution**: Resolution · one column per connected provider. Done, Won't do, Duplicate, Cancelled, Other. A task closed as `Done` carries the status `Closed`, and the close write-back closes the task in the provider as `Done`. A provider's own word stays in its mapping column (Jira's `Done, Fixed`, Zendesk's `Solved`). A row opens `resedit`.
- **Labels**, "A label has a colour and a mapping to each provider's own labels, priorities, types, or tags.", **Add label**: Label (colour chip) · Colour (swatch and hex) · Group · one column per connected provider · Definition of done items (`later`). P0, P1, P2, P3, Bug, New Feature, Improvement, Documentation, Test, Chore. A row opens `lbledit`.
- Note: field settings are workspace settings, each change a governed action in Audit applied to the next read; "A value a provider lacks can be created there from its editor."; later a label carries definition-of-done items, and those templates live in `.oxagen/` as files.
...
Title "Connect an issue provider" (or "Edit <provider>"), subtitle "Import tasks from an issue tracker or a help desk". A six-step rail with the current step marked `aria-current="step"`. Footer "needs `issue_provider.connect` on core-platform".

1. **Provider**: "Choose where the tasks come from. You can connect more than one, and more than one account of the same provider." Six cards with the providers' logos in two groups, **Issue trackers** (GitHub, Linear, Jira) and **Help desks** (ServiceNow, Salesforce, Zendesk), one line each, `connected` on those already connected. **Next** waits for a choice.
...
Connecting creates each value you chose (`create_provider_value`), adds the card to Providers, adds the accounts to People, and toasts "<provider> connected. connect_issue_provider recorded. …".
...
As `tasks.md`: loaded (GitHub and Linear connected; Jira, ServiceNow, Salesforce, and Zendesk offered), empty ("No issue provider is connected to this workspace"), loading, error (`503 issue_index_unavailable`), access denied (`task.read on core-platform`).
