...
- The strip is derived from the rows beneath it by `stgMemoryAgg()`. Memories is the row count; folded is the count of sayings across the rows; Sources is the distinct runs across provenance and sayings, split into operator steers and reflections by each saying's `by`, with imported sayings counted apart; Recalled 30d is the sum of the recalls column; tokens delivered is the sum of token cost times recalls; By class counts rows per class. A build that types the numbers twice is a FAIL.
