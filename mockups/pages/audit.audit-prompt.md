...
3. The product spec for context: `docs/specs/mission-control/spec.md` §14 (Mission Control), §13.3 (retention tiers), Appendix A (target tables), Appendix E (contracts), Appendix F (the pages that survive); the data mapping in `docs/implementation-plan.md` §3.
...
4. **Sections, tabs and tables.** For each item below, the build has it, with the same tab labels (and live counts where the design shows them), the same panel headings, and every table column named in the spec, in that order. Missing or renamed columns are FAILs; extra columns are noted.
   ...
   - Exports: the callout; one card per export with its badge, Verify bundle, Download and the seven facts.
   - Keys: Rotate KEK; Key, Algorithm, Gen, Valid from, Valid to, State, What it covers; Rotate on the active KEK; the re-wrap bar.
   - Retention: Edit policy; Body retention, Hot window, Replay of a compacted run, Workspace opt-down, Cold storage cost.
...
7. **States.** Force each state and compare copy and controls with the design file:
   ...
   - **access denied** (`state=denied`): "You cannot see the audit record"; the roles the signed-in person holds do not include `org.auditor or org.owner`; an owner can grant it and the grant is a governed action in the audit record. Actions: Request access, Back to Fleet. Below: Signed in as, Needed, Decided by.
...
8. **Trust language.** Every tier, attestation and cost basis on the page shows the recorded value. A receipt at `observe` renders "recorded only" and its authority group says `recorded`, never `decided on the server`; a receipt at `harness` says "routed through Oxagen"; the ladder `observe`, `harness`, `gateway`, `contained` renders only as recorded. No incident kind, tile or column names a witness, a verdict, a proof or a definition of done. Money always carries its basis.
