...
3. **Summary tiles.** On Events: Events · 30 days, Denied, By a service principal, By an agent. On Incidents: Open, Critical open, Median time to resolve, Money moved without a receipt. Each shows one number and one basis line; each number is a count of the rows beneath it through the same classifier the table uses (recompute and compare).
4. **Sections, tabs and tables.** For each item below, the build has it, with the same tab labels (and live counts where the design shows them), the same panel headings, and every table column named in the spec, in that order. Missing or renamed columns are FAILs; extra columns are noted.
   ...
   - Events: Actor and Range selects, the search field, the Result and Severity filters, Export CSV; When, Event, Actor, What, Result, Severity, Reference; the note.
   ...
   - Retention: Edit policy; Body retention, Hot window, A compacted run, Per-workspace override, Storage price (the Billing figure, included for 13 months, then $0.10 per GB-month); Archive tiers with Tier, Where, What it holds, Retention, Held today; Redaction.
