...
2. **Header.** Eyebrow "Organization", h1 "Audit", no subtext sentence, the mono retention line beneath. One action with the same label: Export evidence bundle. Exactly one gold (primary) action on the screen.
3. **Summary tiles.** On Events: Events · 30 days, Denied, By a service principal, By an agent. On Incidents: Open, Critical open, Median time to resolve, Money moved without a receipt. Each shows one number and one basis line. The Events tiles count the last 30 days, the rows the table shows on Last 30 days and All actors, through the same classifier the table uses; the Incidents tiles count the incidents rows (recompute and compare). A basis line states data ("allowed and denied", "of N events"), never a rule.
4. **Sections, tabs and tables.** For each item below, the build has it, with the same tab labels (and live counts where the design shows them), the same panel headings, and every table column named in the spec, in that order. Missing or renamed columns are FAILs; extra columns are noted.
   ...
   - Events: no caption; Actor and Range selects that filter the rows; the search field, the Result and Severity filters, Export CSV; When, Event, Actor, What, Result, Severity, Reference; no note beneath the table.
   ...
   - Exports: no callout; one card per export with its badge, Verify bundle, Download and the seven facts; Verifier; Outbound events with Dead-letter view and Delivery.
   - Keys: Rotate KEK; Key, Algorithm, Gen, Valid from, Valid to, State, What it covers; Rotate on the active KEK; the re-wrap bar.
   - Retention: Edit policy; Body retention, Hot window, Per-workspace override, Storage price (the Billing figure, included for 13 months, then $0.10 per GB-month); Archive tiers with Tier, Where, What it holds, Retention, Held today; Redaction with Detectors and Archive edit path.
...
7. **States.** Force each state and compare copy and controls with the design file:
   - **empty** (`state=empty`): "No audit events yet"; Oxagen writes a control-plane audit event on every governed action; empty means nothing has been done yet, not that recording is off. Action: Open Organization.
   - **loading** (`state=loading`): the shell stays and the body is the skeleton; no data, no zeros, no stale rows.
   - **error** (`state=error`): "Audit could not be loaded", `503 audit_store_unavailable`. Nothing was changed. Runs kept recording while this page was down. Actions: Try again, Open an incident; a trace id, region and timestamp line.
...
8. **Trust language.** Every tier, attestation and cost basis on the page shows the recorded value. A receipt's tier badge, in the table and in the receipt dialog, reads the recorded tier through the same badge; a receipt at `observe` has the authority Decision "observed (oxagen did not decide this call)" and nothing on it says oxagen decided the call; the ladder `observe`, `harness`, `gateway`, `contained` renders only as recorded. No incident kind, tile or column names a witness, a verdict, a proof or a definition of done. Money always carries its basis.
