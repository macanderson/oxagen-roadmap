...
2. **By scope.** The heading "By scope" with no caption, and a table with Scope, Sources, Frames by type and Agents, in that order. Rows in the spec's order: Organization, Workspace, each repository in name order, One agent each with its sub-line, Named agents with its sub-line. A scope with no emitting source has no row.
...
4. **By agent.** The heading "By agent" with no caption, the shared list controls (search, the Tier filter, Rows, pager), and the columns Agent, Tier, Sources, Frames by type and Resolve, in that order. The Agent cell is the agent chip with the key in mono. Resolve is a Compiler link.
