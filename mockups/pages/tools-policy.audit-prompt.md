...
1. The page spec: `mockups/pages/tools-policy.md`. Read it first, in full. The header and tab bar it shares are specified in `mockups/pages/tools.md`; the list of gate notices is on Steering › Sources (`mockups/pages/steering.md`).
2. The design authority: `docs/fleet-operations-wedge.md` (Emissions, the Policy row; Exclusion reasons; Cuts, Steering › Gates; D4; D17), `docs/fleet-operations-routes.md` (Steering and Tools), `docs/fleet-operations-collapse.md` (Steering › Gates) and `docs/mission-control-spec.md` §6.12.
...
3. **Policy versions panel.** Title "Policy versions", the store name `tools.policy_versions` as a badge, Draft a version. Columns in order: Version · State · Author · When · Rules · Tests · What changed, then the actions. A row carries Open and, by state, Draft a change (active), Edit, Activate and Discard (draft), or Restore (superseded). The note under the table is verbatim.
...
6. **Where a version lives.** The five rows in order, Store · In regulated mode · Compiled from · Who reads it · What it writes, with the spec's copy.
...
8. **Dialogs.** `policyver`, `policynew`, `policyedit`, `policyactivate`, `policydiscard` and `policyrestore` open from the controls the spec names and carry its fields and copy. `policynew` refuses without a sentence. `policyactivate` says the active version is superseded and kept and that every `policy.decision` frame names the new one from the next call boundary. `policyrestore` drafts a new version and never moves the pointer back. A draft's `policyver` subtitle does not call it superseded. Each write is a governed action: it passes IAM, produces an audit event, and shows a receipt or reference. A stub says what the product would do.
9. **Activation.** Activate a draft. The previous version reads superseded, the new one active, the deny generation advances, and activation required approval.
