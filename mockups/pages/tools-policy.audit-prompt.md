...
4. **Tests badge.** A version whose tests have not run against its saved rules reads "not run yet" and never a pass. Create a draft and confirm it. Edit a draft's rules, save, and confirm the badge falls back to "not run yet" until the tests run again.
5. **Only a draft edits or deletes.** Discard on an active or superseded version refuses with the spec's copy and offers Draft new version. The version page opens an active or superseded version read-only. A version that has decided anything is never edited or removed.
...
8. **Dialogs.** `policynew`, `policyactivate`, `policydiscard` and `policyrestore` open from the controls the spec names and carry its fields and copy. `policynew` refuses without a sentence, and its Based on options carry no comma. Creating a draft and drafting a restore both open the new version's page. `policyactivate` says the active version is superseded and that every policy decision names the new one from the next tool call. `policyrestore` drafts a new version and never moves the pointer back. Each write is a governed action: it passes IAM, produces an audit event, and shows a receipt or reference. A stub says what the product would do. The version page, its editor, its tests and its dialogs are audited by `tools-policy-version.audit-prompt.md`.
9. **Activation.** Try to activate a draft with unsaved changes, with a problem in its rules, with tests that have not run, and with a failing test. Each is refused with the check that blocks it. Then activate a ready draft without a reason, which refuses, and with one. The previous version reads superseded, the new one active, the kill-switch generation advances, the reason is recorded, and activation required approval.
10. **Gate notices.** On Steering › Sources filtered to Policy, each gate's notice is a `constraint` from a source of kind `policy`, and its Managed in link opens the place its gate is edited: the version page at its rule (`/tools/policy/pol_v41?rule=rg_0093`) for a decision rule, Kill switches for a switch, the record for an enforcement grant, Agents for a missing mandate. A notice's frame names its version (`pol_v41`). Removing a notice leaves its gate in force. While notices have no contract they render as not recorded.
