...
3. **Parts.** A group of four buttons with `aria-pressed`: Wasted spend, Tokens and cache, Agents (N), Operator habits (N). Each count equals its list.
4. **Wasted spend.** By cause lists each cause with its run count, amount, bar and one line saying what the frames show; the amounts sum to the Wasted tile. Runs with waste shows one card per run with the run link, agent, operator, start, wasted of cost, badges, the work order's title, the frame, step, cache and model line, Open the run and Show the frames. The closing note says wasted is a claim about frames and that accepted work is never counted.
