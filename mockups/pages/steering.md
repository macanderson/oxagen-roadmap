...
- **Tabs**: Records (N) · Proposals (N) · Context PRs (N). The Context PRs count includes every open operator pull request as well as the promoter's.
...
- **Context PRs** — Pull request · Branch · **Opened by** · State. A row is selectable; the selected pull request is shown below it. Two things open one, and the table says which: the **promoter**, out of runs it aggregated into a proposal, and **a person**, out of the record wizard. The lifecycle is the same for both — the same six checks, run one at a time, the same merge, the same promotion event, the same bundle bump — and only the pull request body differs, because a promoter argues from runs and a person argues from the person.
  - Several operator pull requests may be open at once. "One concern per pull request" limits what one carries, not how many a person may have in flight; each keeps its own state, its own file and its own checks, and selecting a row switches between them.
  ...
  - **A check can fail, and a failed check stops the run where it stopped.** State is `failed`, the failing row says why, the checks behind it stay `queued`, merge stays disabled and nothing is published. A check that can only report "pass" is not a check.
  - **Every check with a predicate is re-run at merge.** A pull request that went green is not evidence that it is still green: another one may have merged in between and taken its lineage. On a stale check the state flips to `failed` at the merge click and nothing publishes.
  - The lineage check counts **published records and competing open pull requests**. Two descriptions can slug to one lineage; the pull request opened first keeps the claim and the later one fails, naming the one that took it. Without both halves — the predicate and the re-run — two pull requests open at once both reach `passed` and the second publishes a duplicate under a published id: two rows, two compiled rules, the tokens counted twice, and the check claiming otherwise on screen.
  - Publishing compiles a bundle version and **derives that version's digest from the rules it contains**. A version with no digest is one the promotion panel claims was re-signed while rendering nothing.
  - The file the pull request carries is serialised as TOML, not interpolated into it. A statement containing a quote or a backslash must not be able to produce a file that fails to parse while the Schema check says it is valid, and it must survive the round trip through the reader unchanged: a multi-line value escapes its backslashes and reaches the closing fence with a line continuation, because a bare newline before the fence is part of the value.
