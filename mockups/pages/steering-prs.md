...
| Design | `mockups/src/wedge.js`: `stgProposalsTab`, inside `pSteering`; `mockups/src/engine.js`: `ctxprTab`, `prTable`, `recprDetail`, `recprFileText`, `recprBody`, `prLabel`, `prRun`, `prRecheck`, `ctxprMerge`, `recprMerge`, `recprDiscard`, `ciLight`, `ciFromSt`, `DLG_EXT.closepr`, `prClose`. Built into `mockups/missioncontrol.html` by `tools/build-mockup.mjs` |
| States | loaded |
...
The Steering record pull requests of the workspace, and the one place a person merges one. A Steering record is a file in `.oxagen/rules/` on the main repository, and it becomes published by being merged, never by being saved. Each pull request carries one record file and runs the same six checks. Merge is refused until every check passed, and the merge is the publication.

Two things open a pull request here: the promoter, from a proposal it argued out of runs, and a person, from the record wizard. They differ in how they argue for the record and are identical in everything that decides whether it publishes. While a pull request is open, its record steers nothing.

## What is on the page

**Header, tabs and shell.** As `steering.md`, with Proposals selected (15). New source is plain on this view, so the gold is Merge pull request's.
...
- A table with no list controls. Columns, in order: Pull request, Branch, Opened by and State.
  ...
  - **Opened by**: "the promoter", or the person who opened it from the record wizard ("Marcus Bell").
...
- **Merge effects**, five numbered rows: "write a promotion_event with the approver, the pull request and the commit"; "re-index the record from the merged commit; a hash mismatch blocks delivery"; "bump the bundle v41 → v42 and re-sign it · 1,340 → 1,386 steering tokens a turn"; "emit steering_published to the audit log"; and a fifth row on delivery. A person's pull request adds the note "Nothing above happens on the way here. The record steers nothing while this pull request is open, which is the whole reason it is a pull request." The mockup's fifth row reads "deliver it on the next model call of every run in core-platform"; the record reaches a run when its host next fetches the bundle, at the run's next session start, as the W6 scenario says, and a build says so.
...
- **promotion_event** replaces Merge effects: record_id (`rec_01K5RW2P7QH4`), lineage_id, from → to ("proposed → published"; "authored → published" for a person's), author (a person's only), approver ("Marcus Bell · workspace.owner · core-platform"), pr_url ("github.com/a-intel/platform/pull/519"), commit_sha (`7d2e91a`), merged_at, re-indexed ("from the merged commit · sha256:9a41c0e7bd238f45 verified"), bundle ("v41 → v42 · sha256:b7d05a31c9e2f644"), tokens per turn ("1,340 → 1,386"), audit ("steering_published · evt_01K5RW2Q8") and ledger ("promotions.jsonl not written · regulated mode only; this workspace is team").
- Actions under it: **See it in run_01K5RS7M2E8FJ3QW** and **Audit log** for the promoter's; **Open the record** (gold) and **See it in Records** for a person's. Open the record lands on `/steering/sources/record/<lineage>`. See it in Records opens Sources filtered to Steering records; the mockup's button re-renders the same view instead.
...
The mockup marks no field on this view with `data-future`, and the catalog gives it no future-only story. These fields are future-only in `macanderson/oxagen` all the same, and a build renders each as not recorded until its contract ships: who opened a pull request, the code owner in the merge bar, the file's bytes as GitHub holds them, the supporting-run table in the promoter's body, the steering tokens a turn before and after, and the comment Oxagen posts when it closes one.

## Functionality

- A pull request carries one record file on `context/<lineage>`: one concern, one pull request. A second pull request on a lineage fails Lineage uniqueness, and the one opened first keeps the claim.
...
- Exactly one gold action per screen: Merge pull request once every check passed, or Open the record after a person's merge. While checks run, the view has no gold.
