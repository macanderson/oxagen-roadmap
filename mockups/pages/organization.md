...
**Header**: eyebrow "Organization", h1 "<organization name>", subtext "People, roles, invitations, workspaces, cost centers, model funding and routes, and API keys."
...
- **Workspaces**: Workspace (name and slug), Main repo, Production branch, Linked repos, Agents, Owner, Governance (a chip with the workspace's mode `solo`, `team` or `regulated`, read off the workspace, with retention mode and namespace beneath); per row **Open**, **Edit** (opens `editws`), **Edit avatar** (opens `avatar` for the workspace), **Archive** (opens `archivews`); **Create a workspace**; a note that changing main is an org-owner action with approval.
- **Cost centers** (`orgCostCenters()`): panel "Cost centers" with **Add a cost center** (opens `ccadd`). A lead line: "Spend is charged to the agent’s cost center, or to its workspace’s when the agent names none. Spend with neither is shown on Spend as its own row." Columns: Label (mono), Description ("No description" when it has none), Agents, Workspaces, Added (the date and who added it); per row **Delete** (red; opens `ccdel`). Agents and Workspaces count the agents and workspaces that name the label today. With no labels the panel reads "This organization has no cost centers." A second panel, "Workspace cost centers", lists every workspace with its label ("None" when it has none) and its agent count, and per row **Change** (opens `ccws`). A note beneath it: "An agent charged to its own label keeps it whatever its workspace names. Charge an agent from its Identity tab." A viewer without an organization Owner, Admin or Billing role sees the note "You can read this list. Changing it takes an organization Owner, Admin or Billing role, which <holders> hold." above the table.
- **Model funding and routes**: **Funding source** first, in one of three states. On a customer key: Key (the saved prefix, enveloped and never returned), Billing, Key storage, Engine, a password field for the key, and **Test and save**. On a platform-minted key: Secret, Provisioned id, Name on the account, Minted, Monthly cap, Reads, Engine, with **Rotate** and **Revoke**. With no key: a note that the in-app agent cannot run and nothing has been charged, and **Mint a key for <organization name>**. Every state carries the **Source** select. Then **Model routes** for Oxagen's own work: Tier, Provider, Route, Fallback, Use, Cost, **Edit** (opens `editroute`), and a Total row with the basis "reported by harness" (`client_attested`).
- **Data plane**: a segmented control Shared, Dedicated, Behind the firewall (the current one marked); a preview note when another mode is shown; the mode's facts (shared: Binding, Database, Object storage, Key-encryption key, Attester key, Gateway; dedicated: Tenant data, Identity and billing, Object storage, Key-encryption key, Gateway, Resolver; firewall: Deployment, Bundle version, Bundle signature, Containers, Air-gapped mode, Licence, Next bundle, and the Outbound connections table); **Request a change of plane** (opens `plane`), **Rotate keys**. **Retention**: Frame bodies, Run ledger, Frame rows, Control-plane audit, `digest_only` mode. **Tenant isolation**: Rows, Workspace scoping, Cross-tenant reads, Platform catalogs, Startup guard.
...
The **avatar editor** (`avatar`) is the one editor `agent.md` specifies, the same dialog Account and the agent page open. Both records here draw as squircles. For the organization it is titled “Avatar for <organization name>” with the slug under it, and its note reads “Saved with `update_org_settings` on the organization’s record.” For a workspace it is titled “Avatar for <workspace name>” with the slug under it, and its note reads “Saved with `update_workspace_settings` on the workspace’s record.” With nothing stored, the draft opens on the name's initials. Save closes the dialog and shows the receipt “The avatar for <name> was saved. Recorded in the audit record.” The app offers Edit avatar on live workspace rows only, because `update_workspace_settings` refuses an archived workspace. Nothing yet draws a saved organization or workspace avatar outside the editor. The switchers keep their letter tiles.
