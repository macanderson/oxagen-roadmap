...
- **People**: panel "People" with the badge "two-factor required" and **Invite**; a search box over every column; filters Status (active, invited), Two-factor (TOTP, hardware key, passkey, passkey + TOTP), Role (`org.owner`, `org.billing`, `org.auditor`, `workspace.owner`, `workspace.member`), Rows; columns Person (name and email), Role (the role id alone; its workspace is in Workspaces), Workspaces, Two-factor, Last seen, Status (an invited person's chip is neutral, an active one's green); every named column sorts from its header; per row **Open** (opens `member`), **Change role** (opens `role`), **Remove** (opens `removemember`); a note that changing a role is a governed action. Nothing sits beneath the panel. The Roles tab holds the holder count of every role.
- **Roles**: Role (id and description), Kind, Scope, Permissions, Held by, Origin (built-in, or who created it and when); **Create role** (gold, small), per row **View** (built-in) or **Edit**, **Duplicate**, **Delete** (disabled for a built-in role or one with holders); the role editor `roleedit` over `PERMS`; `roledel`. Filters Kind, Scope, Origin.
...
**Dialogs this page opens:** `invite`, `member`, `role`, `removemember`, `newws`, `editws`, `archivews`, `ccadd`, `ccdel`, `ccws`, `roleedit`, `roledel`, `editroute`, `funding`, `mintkey`, `rotateorgkey`, `revokeorgkey`, `plane`, `apikey`, `rotatekey`, `revokekey`.
...
| Cost centers | `FIXTURES.COST_CENTERS` (`centers`, `workspaces`, `agents`) | `cost.cost_centers`; `cost_center` on `wrk.workspaces` and on the agent | `cost.cost_centers`, `workspace.workspaces.cost_center`, `agent.agents.cost_center`; `list_cost_centers`, `create_cost_center`, `set_cost_center`, `delete_cost_center` (ADR-142) | ✅ |
