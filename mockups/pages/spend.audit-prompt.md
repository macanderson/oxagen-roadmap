...
4. **Tiles.** Four tiles, in order: Spend (with its basis, "Observed by gateway + Reported by harness", and USD), Tokens (with "served from cache"), Observed by the gateway ("of tokens counted by the proxy"), Wasted (in the critical ink, with its share of spend). The Wasted tile opens Optimization. A tile whose figure no contract answers prints "not recorded", never a zero.
