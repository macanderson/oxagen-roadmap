...
2. **Identity panel.** Subtext "Revocable within one second." Rows in this order: Agent key, Principal, Kind, Harness, Model class, Operator, Cost center, Lifecycle state, First frame, with the values and sub-lines the spec gives. The Cost center row shows the agent's label or "None", **Change**, and the one line of why: the agent's own label, the label inherited from its workspace, or neither. Check one agent of each kind. The Principal is the `prn_…` id, never a service account name.
3. **Credentials panel.** Subtext "What this agent holds". Badge "run token only". Five pairs: API key, OAuth token, Cloud role, GitHub token (each "Not held") and Run token. The paragraph about the broker minting every secret at dispatch is present. "See the connections that mint them" opens Tools › Providers. The badge reads "run token only" when every provider credential row reads "Not held", and at no other time.
...
7. **Dialogs.** Edit identity opens "Edit identity" with Principal, Acts on behalf of, Roles held with Assign role, the approval note, Cancel and Request the change. Revoke credential opens "Revoke the credential on <key>?" with Keep it and Revoke it. Assign role reaches `assign_agent_role`. **Change** on the Cost center row opens `ccagent` with the Cost center select ("None (inherit the workspace’s)" first), the hint naming the workspace's label, Cancel and Save; saving writes `cost_center_set` to Audit, and runs already rolled up keep their label. Each write passes IAM, writes an audit event and shows a receipt or reference. A control whose contract does not exist (Edit identity, Revoke credential, see the spec) must say what the product would do or be absent; one that reports a write it did not make is a FAIL.
