...
2. **Identity panel.** Subtext "In Postgres, revocable in one second." Rows in this order: Agent key, Principal, Kind, Harness, Model tier, Operator, Lifecycle state, First frame, with the values and sub-lines the spec gives. The Principal is the `prn_…` id, never a service account name.
...
7. **Dialogs.** Change identity opens "Change identity" with Principal, Acts on behalf of, Roles held with Assign role, the approval note, Cancel and Request the change. Revoke credential opens "Revoke the credential on <key>?" with Keep it and Revoke it. Assign role reaches `assign_agent_role`. Each write passes IAM, writes an audit event and shows a receipt or reference. A control whose contract does not exist (Change identity, Revoke credential, see the spec) must say what the product would do or be absent; one that reports a write it did not make is a FAIL.
...
15. **Permissions.** Reading requires the agent read (`get_agent`). `assign_agent_role` is gated server-side, not only hidden. Verify with a role that lacks it.
