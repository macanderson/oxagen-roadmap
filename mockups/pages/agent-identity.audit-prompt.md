...
2. **Identity panel.** Subtext "In Postgres, revocable in one second." Rows in this order: Agent key, Principal, Kind, Harness, Model tier, Operator, Cost center, Lifecycle state, First frame, with the values and sub-lines the spec gives. The Cost center row shows the agent's label or "None", **Change**, and the one line of why: the agent's own label, the label inherited from its workspace, or neither. Check one agent of each kind. The Principal is the `prn_…` id, never a service account name.
3. **Credentials panel.** Subtext "The single property most of the threat model rests on." Badge "none". Five pairs: API key, OAuth token, Cloud role, GitHub token (each "none") and Run token. The paragraph about the broker minting every secret at dispatch is present. "See the connections that mint them" opens Tools › Providers. The badge reads "none" only if every provider credential row reads none.
4. **Run credential panel.** Subtext "Long-lived, purpose-locked, hashed at rest, shown to the operator once." Rows Key, Purpose lock, Issued, Last used, Run tokens and Host device key. The key shows its prefix only, never the secret or its hash. An agent with no host shows the unsigned-checkpoints sub-line. Actions Change identity and Revoke credential (danger).
5. **Trust relationships panel.** Subtext "Who this principal answers to, and what will speak for it." Rows Accountable human, Workspace, Runtime, Delegation and Tamper incidents, with Read them when the count is not zero. Open its permissions opens the Permissions tab. There is no Replay row and no replay grade anywhere on the tab (D14); a Replay row is a FAIL.
6. **Nothing moved back.** No roles list and no tier ladder on this tab. Roles are on Permissions, the tier on Runtime.
7. **Dialogs.** Change identity opens "Change identity" with Principal, Acts on behalf of, Roles held with Assign role, the approval note, Cancel and Request the change. Revoke credential opens "Revoke the credential on <key>?" with Keep it and Revoke it. Assign role reaches `assign_agent_role`. **Change** on the Cost center row opens `ccagent` with the Cost center select ("None (inherit the workspace’s)" first), the hint naming the workspace's label, Cancel and Save; saving writes `cost_center_set` to Audit, and runs already rolled up keep their label. Each write passes IAM, writes an audit event and shows a receipt or reference. A control whose contract does not exist (Change identity, Revoke credential, see the spec) must say what the product would do or be absent; one that reports a write it did not make is a FAIL.
...
9. **Future-only fields.** Run token, Purpose lock and Run tokens render as not recorded until the run-token exchange ships. Change identity and Revoke credential are absent or say they are not available. Values drawn from fixtures for any of them are a FAIL.
