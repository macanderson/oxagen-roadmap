...
9. **Tree.** The real paths under `.oxagen/` at the head, as `get_repository_tree` returns them, with the file count, and no comment beside any path and no note beneath. The count matches the Repositories tab. `workspace.json` never appears, because it is gitignored.
