...
1. **Route and shell.** The tab is a path segment and survives a reload. Repositories is lit. The Configuration tab is selected and carries no count. The header's Add Oxagen to a repository is the one gold action.
...
3. **Drift.** Heading "Drift", its subtext, See the pull request, and the columns Declared · In the file · Live · Right, with Right as a badge (`the file` or `a person decides`) and the note beneath. With no drift the panel says the reconciler found nothing, not that nobody looked. Drift from another workspace never renders here.
