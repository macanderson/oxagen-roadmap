...
3. **Panel and chips.** Heading “Work orders” with no subtext and no note under the table (the explanation is in `mockups/help/work-orders.md`, Work orders), and the chips All, Dispatched, Direct and Live, each with its count. Recompute each count from the rows: a count that disagrees is a FAIL. Each chip filters as the spec says.
