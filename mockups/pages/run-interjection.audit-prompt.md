...
1. The page spec: `mockups/pages/run-interjection.md` (read it first, in full), and `mockups/pages/skills.md` for the vocabulary it shares.
2. The design, rendered: the `run-interjection` stories in Storybook (`npm run storybook`), one per state (loaded, loading, error, access denied), desktop and mobile; or `mockups/missioncontrol.html?product=1&state=<state>&mobile=<0|1>#<route>` in a browser or with Playwright.
