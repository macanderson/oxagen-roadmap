...
8. **Approvals drawer.** While the interjection waits, the drawer's first row on every page is "<agent> is paused and needs a decision" with the repository, the workspace, the charge and timeout sentence, and "Answer", which closes the drawer and opens this page. The row is not itself a place to answer. The interjection counts one in the Approvals button and in no sidebar count or Agents tile. Once answered, the row and the count drop it.
