...
5. **The answer.** Sending either path produces a governed action with a receipt id; the frame pane fills in with the frames the spec lists for that path, in order; the agent pane shows the operator’s reply and the agent’s turn 1. After **Create**, the created workspace’s `.oxagen/workspace.toml` has no `[skills]` block and the after-panel says so; after **Link**, The skill it loaded shows the row with its token cost priced as context.
