...
1. The page spec: `mockups/pages/run-interjection.md`. Read it first, in full. Also read `mockups/pages/approvals-drawer.md`, which owns the drawer this question waits in, and `mockups/pages/run.md` for the shell and the ordinary run page.
...
3. The design authority: `docs/fleet-operations-wedge.md` (Cuts: the Skills console is cut and the question stays in the Approvals drawer; D2, D12, D16) and `docs/fleet-operations-collapse.md` (W13 retired).
...
1. **Route and shell.** A run whose loop Oxagen holds renders this page at the run path; every other run renders the ordinary run page. The sidebar lights Work. The breadcrumb names the run's direct work order before the run (D2). The Approvals button in the top bar counts this question while it waits. No assistant button in the top bar.
...
3. **The note.** Waiting, verbatim: "The loop is stopped. The harness is holding before its first model call. Nothing has been charged since 09:14:02Z." Answered: what the answer did, for link and for create. "Ask again" exists only in the mockup; in the build a sent answer is final and the control does not exist.
4. **The triptych.** Three panes in this order, with these eyebrows and labels: What the agent shows <operator's first name> (harness), What oxagen put to a person (oxagen, the one gold-bordered pane), What was written down (frames). Each pane's content matches the spec: the messages and their times, the "question" label on Oxagen's message, the two pick cards with their copy, "Send this answer" disabled with "pick one" until a pick and then "answers as <operator>"; the `control.interject` card; the config line `skills.enabled = true` · `unbound_repo = ask`; the Link and Create paths with every consequence line and its mark; "If nobody answers" with the 30-minute `deny`; the frame list. No pane carries an explanatory note.
5. **The answer.** Sending either path is one governed action with a receipt id. The frame pane fills with the frames the spec lists for that path, in order. The agent pane shows the operator's reply and the agent's turn 1. After link, Skill loaded shows the skill row with "1,840 tokens · $0.0055". After create, the created workspace's `.oxagen/workspace.toml` has no `[skills]` block, and the file's own comment says so. "See how core-platform did it" opens Steering Sources filtered to skills, never a retired Skills console route.
6. **Frames.** While waiting, the frames that have not happened render greyed with no time. Nothing renders a frame that was not written. The pane has no note, waiting or answered.
...
8. **Approvals drawer.** While the question waits, the drawer's first row on every page is "<agent> is paused and needs a decision" with the repository, the workspace, the charge and timeout sentence, and "Answer", which closes the drawer and opens this page. The row is not itself a place to answer. The question counts one in the Approvals button and in no sidebar count or Agents tile. Once answered, the row and the count drop it.
9. **Timeout.** An unanswered question times out at 30 minutes to `deny`; the run continues with no skills and the agent is told why. There is no `allow`. Verify with a clock.
...
17. **Nothing extra.** List anything on the built page that is not in the spec. Each is a finding. A sentence that explains how Oxagen works or why the page looks this way is a finding too: that text belongs in the component help, `mockups/help/run-interjection.md`.
18. **Skill dialogs.** `skill` has no note after the file's body. `skretire` reads "This opens a pull request that removes <path> on <repo>." (or its install line from `.oxagen/workspace.toml`), warns "<n> of <m> runs cited it. Each keeps the digest it recorded." or "No run has ever cited it.", and toasts "Opened <pr> to retire <id>."
