...
| Design | `mc.html` → `pRegister(r) → regName()` (the single source; `consolidated.html` is the product build of it) |
| States | loaded · loading · access denied |
| Files | `register-name-loaded.html` / `register-name-loaded-mobile.html`, `register-name-loading.html` / `register-name-loading-mobile.html`, `register-name-denied.html` / `register-name-denied-mobile.html` |
