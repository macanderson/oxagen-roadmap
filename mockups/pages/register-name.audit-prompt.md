...
You are auditing the **Register agent · Name** page of Oxagen Mission Control (`#/a-intel/core-platform/register/name`) for conformance to its design. Be exact and adversarial: the design is the spec, and “close enough” is a fail. Do not summarise what you see; compare it.

## Inputs

1. The page spec: `pages/register-name.md` (read it first, in full).
2. The design, rendered: `pages/register-name-<state>.html` and `pages/register-name-<state>-mobile.html` for each state (loaded, loading, access denied); open them in a browser or with Playwright. `pages/index.html` links all of them.
...
2. **Header.** Eyebrow “Step 1 of 3”, h1 “Name the agent”. Actions present, in order, with the same labels: Cancel · Continue. Exactly one gold (primary) action on the screen.
...
9. **Mobile.** At 390 × 844 with a touch pointer: the card fills the width with 16 px gutters, buttons are full width and ≥ 44 px, inputs are 16 px, code inputs use the numeric keypad. Compare against `pages/register-name-loaded-mobile.html`.
...
11. **Permissions.** Read requires `agent.register`; each write (agent.register) is gated server-side, not only hidden in the UI. Verify with a role that lacks the permission.
