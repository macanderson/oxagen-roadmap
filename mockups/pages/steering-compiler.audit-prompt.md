...
11. **Frame types, not kinds.** Every frame carries one of the eight types (`goal`, `invariant`, `constraint`, `delegation`, `procedure`, `context`, `invocation`, `capability`). A delegation frame appears for an agent with an active mandate. A harness's own tools are not frames. A tool the toolbelt denies is excluded, not listed.
