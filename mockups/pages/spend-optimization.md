...
|---|---|---|---|
| cache misses | 212 | $1,204.35 | system-context digest changed mid-conversation; every later turn re-read the prefix at full price |
| corrective prompts | 96 | $1,180.40 | a prompt after the first fixed or filled what the first left out, and each one re-ran the window at full price |
| retry loops | 38 | $612.08 | the same tool call repeated three or more times with the same arguments |
| context bloat | 97 | $412.77 | tool list or tool results larger than the turn used; the belt was recomputed mid-run |
| idle while parked | 19 | $96.14 | turns spent re-checking an approval that had not arrived |
| halted early | 37 | $86.18 | a policy deny at turn 1 after the prompt was already paid for |
...
- Cache hit rate: "80% · cache_read ÷ (input_uncached + cache_read), token-weighted"
...
**By harness**, with the badge "92% of tokens observed by the gateway". Columns: Harness · Agents · Tokens · Cache hit · Spend · Basis. Eight rows: Claude Code, Stella, Claude Agent SDK, Codex CLI, LangGraph, Custom (SDK-wrapped), OpenAI Agents SDK and Cursor. Basis reads `gateway_observed`, `client_attested`, or both with each share, such as "gateway_observed 60% · client_attested 40%" for Codex CLI. The note defines observed (the gateway's proxy counted the bytes) and self-reported (the harness's own telemetry), and says a class a harness does not report is marked absent, never zero.
...
|---|---|---|
| Narrow the belt | Tool definitions are over 16% of every request | Edit the grant (the agent's Toolbelt tab) |
...
| Runs with waste | `SPEND.wasteRunsList`, `RUNS` | `list_waste` run ids per cause | The run ids ship (`waste.tsx:50`). What each run wasted, its title and its steps are not on the waste record (`waste.tsx:167`) | 🟡 |
...
- **Parts in the URL.** `?part=` names the part, so each is a link. The old Tokens, Coaching and Wasted spend tabs land here.
- **Wasted** is a claim about frames, not about outcomes: the frames show the tokens bought nothing. Work a person accepted is never counted. The By cause amounts sum to the Wasted tile, and a run with two causes counts toward both run counts.
...
The thumb bar holds Work (8), Agents, Tools, Spend and More (3), with Spend lit. More holds Steering, Runtimes, Repositories, Organization, Billing, Audit, Stella, search, notifications, the account and both switchers. The tiles form a two by two grid above the tabs, and the four part buttons wrap to two rows. The By cause meters and the Runs with waste cards stack at full width. Every table (By token class, By harness, By agent and Recommendations for agents) becomes one card per row, each cell labelled with its column. The habit cards stack. Nothing scrolls sideways at 390 px.
...
- The six waste causes, and each run's wasted amount, title and steps on the waste record.
...
- Every money figure states its basis, and every enforcement claim states its tier. Self-reported tokens are labelled `client_attested`, never observed.
