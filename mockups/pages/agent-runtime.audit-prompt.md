...
5. **No host enrolled.** An agent with no enrolled host shows one panel: "No runtime is enrolled", the sentence about the `observe` tier, Enroll a runtime (gold, the Register agent gate) and Show CLI steps, which shows the CLI steps. Only an agent at the `observe` tier has no enrolled host.
