...
3. The design authority: `docs/fleet-operations-wedge.md` (D5, the Emissions row for Toolbelt, D17) and `docs/fleet-operations-ia.md` (Agents); the belt in `docs/mission-control-spec.md` §6.6.
...
1. **Route and shell.** The route opens the agent page with Toolbelt selected; its count equals the belt's width. The header and tab bar match `agent.md`.
2. **Belt computation.** Subtext as the spec gives it. The badge names the active policy and the deny generation. The wire joins grants, the delegation ceiling (the operator), the policy bundle and the kill switches into the belt with its width. The sentence names the registry's versions and servers, the belt's width, and what a call outside the belt does. Every figure matches the belt read.
3. **Model view.** The subtext states the width against the full-belt limit (40) and the presentation it earns. The Belt presentation group has Searchable belt and Full belt with `aria-pressed`. Searchable shows the note and the tools block with `search_tools`, `load_tools` and the pinned tools; Full shows every definition with its schema digest and "… N more" past six. The toggle changes nothing stored and resets when another agent opens.
4. **Belt search.** The field reads as `search_tools(…)` with Run and Enter. The five example queries are present. A hit lists belt members only, at most eight, with the "<N> of <M> belt members matched" sentence. A miss reads "Zero results.", names a registry tool that exists and why it is off the belt, and says a search never returns a tool outside the belt. Search for a tool that is off the belt: it must not appear as a hit.
5. **Per-tool decision rules.** Subtext as the spec gives it. Tool names (Labels, API names) and Layout (By category, Flat) groups with `aria-pressed`; the "N of M shown" badge; category chips with counts; the hazard and gate counts; What the categories mean. Columns in order: Tool · Category · Decision · Hazard · Egress · Financial · Schema digest. The rule that decided shows under each decision. Grouped rows have a heading per category with its meaning and count. A row opens the tool dialog.
6. **Off the belt.** Subtext as the spec gives it, the badge "not visible to the model", and the table Tool · Reason with Rows and a pager. Every tool listed here is absent from the belt table and from the tools block.
...
10. **Future-only fields.** The trailer, the pinned tools, an agent-scoped belt search, `denied` and `mandate + approval` decisions on belt rows, a `critical` hazard on a belt row, and harness tools as belt rows render as not recorded or are absent until their contracts ship. Any of them drawn with fixture values is a FAIL.
11. **Rollups.** The tab count, the wire's belt figure, the Model view width and "N of M shown" agree. The category chips, hazard counts and gate counts equal the rows. The Off the belt sample count equals the registry versions less the belt's width.
...
16. **Permissions.** Reading requires the belt read (`get_agent_toolbelt`), checked server-side. The tab has no writes.
