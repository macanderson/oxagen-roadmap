...
**Shell.** The workspace sidebar lists Work (8), Agents, Tools (29), Steering (9), Runtimes (2), Spend and Repositories (5), then Organization, Billing and Audit (3). Tools is lit. Its count is the observed output schemas waiting for approval (`proposals().length`), the one thing on this page that waits on a person. The foot carries the Stella launcher, "278 agents in this organization" and the connection badge. The top bar carries the breadcrumb "Anderson Intelligence Corp. / Core platform / Tools", **Search or run an action** with ⌘K, notifications, the approvals button (17 waiting, opening the drawer in `approvals-drawer.md`) and the account avatar.

**Header** (every Tools tab). Eyebrow: the workspace name, "Core platform" (the eyebrow style sets it in capitals). H1 "Tools", with no subtext: why the registry is the only source of tools an agent can see is in the component help (`mockups/help/tools.md`, Header). Actions, in order:

- **Add provider** (plain) opens `import`.
...
- **Stop a tool…** (danger) opens `switch` on the class switch "Every tool that moves funds".

After a switch is flipped, a banner sits between the header and the tab bar on every tab. `tools-switches.md` specifies it, and its component help is `mockups/help/tools.md`, Flip banner (`data-help="tools/flip-banner"`).
...
|---|---|---|
| Tools | 29 in the approval colour, titled "29 schemas need approval" | Observed output schemas waiting. With none waiting, the registry's version count, 703 |
...
- **Banner** (`data-help="schema-approvals"`), shown while observed schemas wait: the badge "29 awaiting approval", then "29 tools have no declared output schema." in bold. The bold line names the providers that have a waiting schema while there are three or fewer. **Review** opens `schema`. How an observed schema is inferred and what is checked until approval is in the component help (`mockups/help/tools.md`, Schema approvals).
- **Tools** panel, with no caption: why permissions apply per version is in the component help (`mockups/help/tools.md`, Tools). The panel header carries the **Labels** / **API names** toggle (`aria-pressed`), the badge "703 of 703 shown" (the versions in the chosen category, of the registry's total) and **Add provider** (plain, opens `import`).
...
- **List controls**, the ones every list in the product carries (`ltTable()`): the search field "Search this list", the filters "Any schema origin", "Any egress" and "Any financial", **Rows** (5, 10, 25, 50 or All, with 10 by default), sortable column headers, and the pager (`1–10 of 703`).
- **Table**, columns in order: Tool version · Provider · Category · Hazard · Gate · Egress · Financial · Schema origin · Digest · Toolbelts · Agents · Calls 30d.
  ...
  - *Provider*: a button with the provider's name ("GitHub"). It opens `server` (`tools-providers.md`).
  ...
  - *Gate*: Allowed, Needs approval, Mandate + approval, or Stopped by kill switch. The tooltip names the rule or the switch.
  ...
  - *Toolbelts*: one badge per toolbelt that carries the version, or a dash.
  - *Agents*: how many agents those toolbelts reach.
  - *Calls 30d*: calls in the last 30 days.
  - Each row carries a left rule in its risk colour and opens `tool`.
- No note under the table. The gate order and the difference between a toolbelt and a gate are in the component help (`mockups/help/tools.md`, Tools).
...
- `import`, in three steps. The step is the dialog's argument, so opening it always starts at Connect. A strip heads each step: "1 · Connect → 2 · Review tools/list → 3 · Classify and import", a done step marked ✓.
  1. "Import tools from a provider", no subtitle. Fields: Endpoint URL (`https://mcp.confluence.a-intel.internal/mcp`), Transport (streamable-http, sse, stdio), Connection ("Create one after import", or one of the 16 connections by id and name). Note: "Connecting calls tools/list on this endpoint." Footer: Cancel, **Connect** (gold).
  2. "Review tools/list", subtitle "8 tools returned by mcp.confluence.a-intel.internal · protocol 2025-06-18". One checkbox per tool, with the tool cell, its description, its hazard, and an "outputSchema" or "no outputSchema" badge. Warning: "2 are unchecked by default. delete_page and export_space are irreversible or bulk egress and declare no output schema." Footer: "6 of 8 selected", Back, **Classify** (gold, disabled with nothing selected).
  3. "Classify and import", no subtitle. Table: Tool version · Hazard · Egress · Financial · Output schema. Note: "These 6 versions take the registry from 703 to 709 tool versions." Footer: "Credential kind api_key · downscoping none available", Back, **Import 6 tools** (gold). The toast reads "Imported 6 tool versions from confluence. They are in the registry and on no toolbelt." What import stores, why two tools start unticked, and why import grants nothing are in the component help (`mockups/help/tools.md`, Provider import).
- `schema`, "Approve an observed output schema" (wide). An eyebrow names the version and the provider ("slack__list_channels@1 · the slack provider declares no output schema for this tool"). The design fixes the words "the slack provider" whatever the provider, and a build names the version's own provider. A note: "Approving the inferred schema turns strict validation on from the next call." Then Observations ("341 responses over 30 days, from 0 agents"), Inferred from ("the intersection of every observation · 0 outliers discarded") and Digest on approval, the inferred schema beside one recorded response, and "28 more waiting after this one." Footer: **Not yet** and **Approve schema** (gold). What happened before approval and what `approve_tool_schema` records are in the component help (`mockups/help/tools.md`, Output schema approval). With nothing left to review it reads "Every observed schema is approved", "Nothing to review.", and **Done**.
- `toolcats`, "Tool categories" (wide): the ten categories in order, each with its meaning and three example tools. Then "Hazard and gate": Risk (low, medium, high, critical), Side effect (read, write, irreversible) and Gate (Allowed, Needs approval, Mandate + approval, Blocked, Stopped by kill switch), with "dashed = a person still stands in the way". Footer: **Close**. Why category decides nothing is in the component help (`mockups/help/tools.md`, Tool categories).
- `tool` (wide). Title: the API name. The tool cell, large, with "provider <id>". A row of the category badge, the hazard, the gate, "egress <class>" and the financial badge, then the category's meaning. Schema digest, Schema origin, Credential (`github_app → installation token`), Price ("$0.00 USD", "per call, from the price book"). A financial tool adds `amount_path`, `currency_path`, `counterparty_path` and `idempotency`, and the warning "Financial tool. A call without a mandate is denied before dispatch." Then "Input schema" as JSON. The footer counts the toolbelts ("On 1 toolbelt · 63 calls in 30 days") and the 30-day calls, then **Close**. No gold. Strict validation, `schema_violation`, the canonical input digest and the credential the agent never sees are in the component help (`mockups/help/tools.md`, Tool version).
- `switch`, from Stop a tool… (`tools-switches.md`).
- `wz`, the tool creation wizard: Describe, Recommendation, Manifest, Code, Pull request (`docs/creation-spec.md`). Its step subtitles state data, and the reasoning its notes carried is in the component help (`mockups/help/tools.md`, Tool creation wizard).
...
- Every observed-schema element: the banner, Review and `schema`, the origins `observed` and `observed, approved`, `sha256:pending`, the Tools tab's waiting count ("29 schemas need approval") and the sidebar count (#3921). The Tools tab count is the registry's.
...
- Gate is today's gate, in the order `toolGateKind()` applies it: a switch on the version or on its provider (kill switch), then a financial class (mandate + approval), then an irreversible side effect (needs approval), else allowed. Flipping or clearing a switch changes the column on the next render.
- The Toolbelts and Agents cells are derived from the toolbelt catalogue and the assignment record (`beltsWithTool()`, `agentsWithTool()`). Removing a tool from a toolbelt changes the row. The fixture carries no count of either.
...
- Importing grants nothing. An imported version sits in the registry, callable by nobody, until it is on a toolbelt and a grant reaches it.
...
- **error**: "Tools could not be loaded". "The control plane answered 503 tool_registry_unavailable. Nothing was changed. Runs kept recording while this page was down." Actions: **Try again** (gold) and **Open an incident** (opens `incident`). Then "trace 01K5RSXQ7F2E · us-east-1 · 2026-09-11 09:16:04Z".
- **access denied**: "You cannot see the tool registry". "Your roles on Anderson Intelligence Corp. do not include tools.read on core-platform. An organization owner can grant it." Actions: **Request access** (gold, opens `request-access`) and **Back to Work** (`#/a-intel/core-platform/work`). Below: Signed in as "Marcus Bell · workspace.owner · core-platform", Needed "tools.read on core-platform", Decided by "pol_v41".
...
- Every enforcement claim states the tier. A gate is enforced for calls routed through Oxagen. On the `harness` tier the hook refuses a call, reported by the harness and fail-open. On `observe` nothing refuses.
...
- A Steering Source and a SteeringFrame are never shown as each other. A tool on a toolbelt emits a `capability` SteeringFrame, and this tab shows the tool.
