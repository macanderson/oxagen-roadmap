...
| `memory.json` | `MEMORY` | recalled memory as `SteeringItem`s of kind `memory`: class (`RULE`, `FACT`, `EPISODE`, `PREFERENCE`), force (`may` or `info`), scope, body, `token_cost`, provenance (run and frame), `hash`, `valid_from`, last recalled, and recalls in 30 days. `yieldsTo` names a published `must` that beats it. `supersededBy` names the record that replaced it.; `sayings` (`run`, `frame`, `by`, `text`), the words each run used, which fold into the memory; `proposedAs` on a memory the fold made a proposal |
| `md-import.json` | `MD_IMPORT` | the Markdown import sample: `root`, the files a picked directory holds (path and lines), the files it skips and why, `max_kb` (the per-file limit), and `usd_per_ktok` (the parse price the cost estimate reads) |
| `cost-centers.json` | `COST_CENTERS` | the organization's cost centers (`label`, `description`, who added it and when), the label each workspace and named agent is charged to, `assign` (how many more agents of a workspace carry a label, charged at load), and the chargeback statement's `columns` |
