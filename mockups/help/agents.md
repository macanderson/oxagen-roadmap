...
3. The footer reads "68 agents · 12 in flight · at the boundary". **Steer** is disabled with no agent selected, and empty text is refused ("A steer needs text."). With Interrupt on the button is **Send & Interrupt** (danger).
...
### Logic
1. **Describe**. One line in your own words, or one of four examples. **Draft it** stays disabled until the wand has rewritten the text (`wzDescOk()` needs `descAI`), and typing again resets it. The mockup's rewrite is deterministic. The wand runs `oxagen.assistant`, which rewrites what you wrote into the prose the file carries. Its turn is recorded with frames and a receipt and billed to Oxagen. It is not one of your runs, so it appears in neither Work nor Spend. The note under the field (`wzDesc()`) and the drafting card on step 3 (`wzDraftNote()`) are shared by every creation wizard, and the Steering group owns their copy.
...
4. **Toolbelt**. Twelve tool versions to pick from. A pick that is irreversible or high or critical risk parks: each call stops at the gateway and waits for an approver, on every call. The line under the list names the tools that park, with the verb in agreement ("1 of these parks for a person"). Auto-approval rules are cut, so every such call waits for a person. With nothing picked the definition falls back to `search_graph` and `recall_context`, a toolbelt that reads the graph and nothing else. Leaving the step writes the picks into the file unless you edited it by hand (`wzAgentSync()`).
